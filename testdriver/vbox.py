# -*- coding: utf-8 -*-
# $Id: vbox.py 53039 2014-10-13 10:49:38Z vboxsync $
...
class EventHandlerBase(object):
    ...
    def unregister(self, fWaitForThread=True):
        ...
        if self.oThread is not None and self.oThread != threading.current_thread():
            self.oThread.join();
            self.oThread = None;
...
# --lb--    # pylint--lb--: --lb--enable--lb--=--lb--C0111--lb--,--lb--R0913,W0613
...
class VirtualBoxEventHandlerBase(EventHandlerBase):
    ...
    def onGuestPropertyChange(self, sMachineId, sName, sValue, sFlags):
        pass;
# --lb--        # --lb--pylint--lb--: enable--lb--=--lb--C0111,W0613
...
#                        self.oVBoxSvcProcess.terminate(2500);
    ...
    def _setupVBoxApi(self):
        ...
        if self.oBuild.sSdkPath is not None:
            ...
            sys.path.insert(1, os.path.join(self.oBuild.sSdkPath, 'bindings', \
            'xpcom', 'python'))
        ...
        try:
            ...
            if self.sHost == 'win':
                from pythoncom import com_error as NativeComExceptionClass
...
#--lb--                # --lb--pylint--lb--: --lb--enable--lb--=F0401
        except:
            traceback.print_exc();
            return False;
    ...
    def createTestVM(self, sName, iGroup, sHd=None, cMbRam=None, cCpus=1, \
        fVirtEx=None, fNestedPaging=None, sDvdImage=None, sKind="Other", \
        fIoApic=None, fPae=None, fFastBootLogo=True, eNic0Type=None, \
        eNic0AttachType=None, sNic0NetName='default', sNic0MacAddr='grouped', \
        sFloppy=None, fNatForwardingForTxs=None, \
        sHddControllerType='IDE Controller', fVmmDevTestingPart=None, \
        ...
        try:
            if self.fpApiVer >= 4.2: # Introduces grouping (third parameter, empty for now).
                oVM = self.oVBox.createMachine("", sName, [], \
                    self.tryFindGuestOsId(sKind), "");
            elif self.fpApiVer >= 4.0:
                oVM = self.oVBox.createMachine("", sName, \
                    self.tryFindGuestOsId(sKind), "", False);
        ...
        return oVM;
        # --lb--pylint--lb--: --lb--enable=--lb--R0913,R0914
    ...
    def waitOnDirectSessionClose(self, oVM, cMsTimeout):
        ...
        msStart = base.timestampMilli();
        while iCurPid == iOrgPid and sCurType == sOrgType and sCurType != '' and \
            base.timestampMilli() - msStart < cMsTimeout and \
            (eCurState == vboxcon.SessionState_Unlocking or \
            eCurState == vboxcon.SessionState_Spawning or \
            eCurState == vboxcon.SessionState_Locked):
    ...
    def txsUnpackFile(self, oSession, oTxsSession, sRemoteFile, sRemoteDir, cMsTimeout=30000, fIgnoreErrors=False):
        return self.txsDoTask(oSession, oTxsSession, oTxsSession.asyncUnpackFile, \
            (sRemoteFile, sRemoteDir, self.adjustTimeoutMs(cMsTimeout), \
            fIgnoreErrors));   # --lb--pylint--lb--: enable=C0111
    ...
    def startVmAndConnectToTxsViaTcp(self, sVmName, fCdWait=False, cMsTimeout=15*60000, \
        ...
        if oTestVM is None or oTestVM.fSnapshotRestoreCurrent is False:
            try:
        ...
        if sUuidBefore is not False:
            ...
            if fRc is True:
                ...
                if fRc is True:
                    ...
                    sUuidAfter = self.txsDoTask(oSession, oTxsSession, \
                    oTxsSession.asyncUuid, (self.adjustTimeoutMs(cMsTimeout2, \
                    60000), False));
                    if sUuidBefore is not False:
    ...
    def txsRunTestRedirectStd(self, oTxsSession, sTestName, cMsTimeout, sExecName, asArgs=(), asAddEnv=(), sAsUser="",
        ...
        """
        reporter.testStart(sTestName);
        reporter.log2('txsRunTestRedirectStd: cMsTimeout=%u sExecName=%s asArgs=%s' \
            % (cMsTimeout, sExecName, asArgs));

        # Submit the job.
        fRc = False;
        if oTxsSession.asyncExecEx(sExecName, asArgs, asAddEnv, oStdIn, oStdOut, \
            oStdErr, oTestPipe, sAsUser, cMsTimeout=self.adjustTimeoutMs(cMsTimeout)):
            self.addTask(oTxsSession);
    ...
    def txsRunTest2(self, oTxsSession1, oTxsSession2, sTestName, cMsTimeout, \
        sExecName1, asArgs1, sExecName2, asArgs2, asAddEnv1=(), sAsUser1='', \
        fWithTestPipe1=True, asAddEnv2=(), sAsUser2='', fWithTestPipe2=True):
        """
        ...
        # Submit the jobs.
        fRc = False;
        if oTxsSession1.asyncExec(sExecName1, asArgs1, asAddEnv1, sAsUser1, \
            fWithTestPipe1, '1-', self.adjustTimeoutMs(cMsTimeout)):
            self.addTask(oTxsSession1);

            self.sleep(2); # fudge! grr

            if oTxsSession2.asyncExec(sExecName2, asArgs2, asAddEnv2, sAsUser2, \
                fWithTestPipe2, '2-', self.adjustTimeoutMs(cMsTimeout)):
...
# --lb--    # --lb--pylint--lb--: --lb--enable--lb--=--lb--R0914,--lb--R0913
