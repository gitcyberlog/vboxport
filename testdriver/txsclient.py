# -*- coding: utf-8 -*-
# $Id: txsclient.py 52776 2014-09-17 14:51:43Z vboxsync $
...
# --lb-- from common import utils;
# --lb-- from testdriver import reporter;
# --lb-- import sys
# --lb-- sys.path.append("testdriver/")
# Validation Kit imports.
...
class Session(TdTaskBase):
        ...
        if rc is True:
            ...
            while True:
                ...
                if oOut is not None:
                    ...
                    if len(abPayload) < 4:
                        sFailure = 'malformed output packet (%s, %u bytes)' \
                            % (sOpcode, cbMsg);
        ...
        if rc is True:
            ...
            while rc is True:
                ...
                if uCrc32 != (uMyCrc32 & 0xffffffff):
                    reporter.maybeErr(self.fErr, \
                        'taskDownload got a bad CRC: mycrc=%s remotecrc=%s'
    ...
    # --lb-- pylint--lb--: --lb--enable--lb--=--lb--C0111
    ...
    def syncExecEx(self, sExecName, asArgs=(), asAddEnv=(), # pylint: disable=R0913
                   ...
                   sAsUser='', cMsTimeout=3600000, fIgnoreErrors=False):
        """Synchronous version."""
        return self.asyncToSync(self.asyncExecEx, sExecName, asArgs, asAddEnv, \
            oStdIn, oStdOut, oStdErr, oTestPipe, sAsUser, cMsTimeout, fIgnoreErrors);

    def asyncExec(self, sExecName, asArgs=(), asAddEnv=(), sAsUser="", \
        fWithTestPipe=True, sPrefix='', cMsTimeout=3600000, fIgnoreErrors=False):
        """
    ...
    def syncExec(self, sExecName, asArgs=(), asAddEnv=(), sAsUser='', \
        fWithTestPipe=True, sPrefix='', cMsTimeout=3600000, fIgnoreErrors=False):
        """Synchronous version."""
        return self.asyncToSync(self.asyncExec, sExecName, asArgs, asAddEnv, \
            sAsUser, fWithTestPipe, sPrefix, cMsTimeout, fIgnoreErrors);
...
class TransportTcp(TransportBase):
    ...
    def cancelConnect(self):
        # This is bad stuff.
        self.oCv.acquire();
        reporter.log2('TransportTcp::cancelConnect: fIsConnecting=%s oSocket=%s' \
            % (self.fIsConnecting, self.oSocket));
    ...
    def _connectAsClient(self, oSocket, oWakeupR, cMsTimeout):
        ...
        try:
            ...
            if self.__isInProgressXcpt(e):
                ...
                try:
                    ttRc = select.select([oWakeupR], [oSocket], [oSocket, \
                        oWakeupR], cMsTimeout / 1000.0);
            ...
            if rc is True:
               ...
               or iRc == errno.ETIMEDOUT:
                ...
                if iRc != errno.EBADF  or  not self.fConnectCanceled:
                    reporter.fatalXcpt('socket.connect((%s,%s)) failed; iRc=%s' \
                        % (self.sHostname, self.uPort, iRc));
