# -*- coding: utf-8 -*-
# $Id: vboxwrappers.py 54432 2015-02-24 10:46:34Z vboxsync $
...
class ProgressWrapper(TdTaskBase):
    ...
    def waitForOperation(self, iOperation, cMsTimeout=60000, fErrorOnTimeout=True, \
        cMsInterval=1000, fIgnoreErrors=False):
        """
...
class SessionWrapper(TdTaskBase):
    ...
    def setNicTraceEnabled(self, fTraceEnabled, sTraceFile, iNic=0):
        ...
        try:
            ...
            try:
                ...
                reporter.errorXcpt(\
                'failed to set NIC trace flag on slot %s to %s for VM "%s"' \
    ...
    def getDefaultNicName(self, eAttachmentType):
        ...
        if eAttachmentType == vboxcon.NetworkAttachmentType_Bridged:
            if self.oTstDrv.sDefBridgedNic is not None:
                ...
                sRetName = 'eth0';
                try:
                    aoHostNics = self.oVBoxMgr.getArray(self.oVBox.host, \
                    'networkInterfaces');
                    for oHostNic in aoHostNics:
                        if   oHostNic.interfaceType == \
                            vboxcon.HostNetworkInterfaceType_Bridged and \
                            oHostNic.status == vboxcon.HostNetworkInterfaceStatus_Up:
    ...
    def setStorageControllerType(self, eType, sController="IDE Controller"):
        ...
        try:
            ...
            try:
                ...
                reporter.log('added storage controller "%s" (type %s) to %s' \
                    % (sController, iType, self.sName));
            except:
                reporter.errorXcpt('addStorageController("%s",%s) failed on "%s"' \
                    % (sController, iType, self.sName));
                return False;
        try:
            oCtl.controllerType = eType;
        except:
            reporter.errorXcpt('failed to set controller type of "%s" on "%s" to %s' \
                % (sController, self.sName, eType));
            return False;
        reporter.log('set controller type of "%s" on "%s" to %s' \
            % (sController, self.sName, eType));
        self.oTstDrv.processPendingEvents();
        return True;
    ...
    def createAndAttachHd(self, sHd, sFmt="VDI", sController="IDE Controller", \
        cb=10*1024*1024*1024, iPort=0, iDevice=0, fImmutable=True):
        """
    ...
    def teleport(self, sHostname='localhost', uPort=6500, sPassword='password', \
        cMsMaxDowntime=250):
        """
    ...
    def takeSnapshot(self, sName, sDescription='', fPause=True, \
        fFudgeOnFailure=True, cMsTimeout=30 * 1000):
        """
    ...
    def addLogsToReport(self, cReleaseLogs=1):
        ...
        for iLog in range(0, cReleaseLogs):
            try:
                ...
                if sLogFile is not None and sLogFile != '': # the None bit is for a 3.2.0 bug.
                    reporter.addLogFile(sLogFile, 'log/release/vm', '%s #%u' \
                        % (self.sName, iLog), sAltName='%s-%s' % (self.sName, \
                        os.path.basename(sLogFile),));
        ...
        try:
            ...
            if os.path.isfile(sLogFile):
                reporter.addLogFile(sLogFile, 'log/release/vm', '%s startup log' \
                    % (self.sName,), sAltName='%s-%s' % (self.sName, \
                    os.path.basename(sLogFile),));

        # Now for the debug log.
        if self.sLogFile is not None and os.path.isfile(self.sLogFile):
            reporter.addLogFile(self.sLogFile, 'log/debug/vm', '%s debug' \
                % (self.sName,), sAltName='%s-%s' % (self.sName, \
                os.path.basename(self.sLogFile),));
    ...
    def registerDerivedEventHandler(self, oSubClass, dArgs=None, fMustSucceed=True):
        ...
        return oSubClass.registerDerivedEventHandler(self.oVBoxMgr, self.fpApiVer, \
            oSubClass, dArgsCopy, oConsole, 'IConsole', 'IConsoleCallback', \
    ...
    def txsTryConnectViaTcp(self, cMsTimeout, sHostname, fReversed=False):
        ...
        oTxsSession = txsclient.tryOpenTcpSession(cMsTimeout, sHostname, \
            fReversedSetup=fReversed, cMsIdleFudge=cMsTimeout / 2);
...
class TxsConnectTask(TdTaskBase):
    ...
    class TxsConnectTaskVBoxCallback(vbox.VirtualBoxEventHandlerBase):
        ...
        def onGuestPropertyChange(self, sMachineId, sName, sValue, sFlags):
            ...
            if sMachineId == self.sMachineId and sName == \
    ...
    def _openTcpSession(self, sIpAddr, uPort=None, fReversedSetup=False, \
        cMsIdleFudge=0):
        """
        ...
        if self.oTxsSession is None:
            ...
            self.oTxsSession = txsclient.openTcpSession(self.cMsTimeout, sIpAddr, \
                uPort, fReversedSetup, cMsIdleFudge);
