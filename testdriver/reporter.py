# -*- coding: utf-8 -*-
# $Id: reporter.py 53514 2014-12-11 16:33:28Z vboxsync $
...
class ReporterBase(object):
    ...
    def __init__(self):
        ...
        import logging;
        logging.basicConfig(stream=PythonLoggingStream(), level=logging.DEBUG, \
    ...
    def testStart(self, sName, sCaller):
        ...
        return self.log(1, '%-50s: TESTING' % (self._testGetFullName()), sCaller, \
            sTsPrf);
    ...
    def testFailure(self, sDetails, sCaller):
        ...
        self._xmlWrite(['<FailureDetails timestamp="%s" text="%s"/>' \
            % (sTsIso, self._xmlEscAttr(sDetails),),]);
...
class LocalReporter(ReporterBase):
    ...
    def __init__(self):
        ...
        try:
            sLogDir = os.path.abspath(os.environ.get('TESTBOX_REPORTER_LOG_DIR', \
                self.sDefLogDir));
        ...
        self._xmlWrite(['<?xml version="1.0" encoding="UTF-8" ?>', \
            '<Test timestamp="%s" name="%s">' % (sTsIso, \
            self._xmlEscAttr(self.sName),),], fIndent=False);
    ...
    def _closeXml(self, sTsIso):
        ...
        if self.oXmlFile is not None:
            ...
            while len(self.atTests) > 0:
                sName, cErrorsStart, self.fTimedOut = self.atTests.pop();
                self._xmlWrite(['<End timestamp="%s" errors="%d"/>' % (sTsIso, \
                self.cErrors - cErrorsStart,), '</%s>' % (sName,),]);
    ...
    def addLogFile(self, oSrcFile, sSrcFilename, sAltName, sDescription, sKind, sCaller, sTsPrf):
        ...
        try:
            ...
            self._xmlWrite('<LogFile timestamp="%s" filename="%s" source="%s" kind="%s" ok="%s">%s</LogFile>\n' \
                ...
                self._xmlEscAttr(os.path.basename(sDstFilename)), \
                self._xmlEscAttr(sSrcFilename), self._xmlEscAttr(sKind), \
                fRc, self._xmlEscAttr(sDescription)));
        _ = sAltName;
        return fRc;
    ...
    def subXmlStart(self, oFileWrapper):
        ...
        try:
            ...
            self._xmlWrite('<Include timestamp="%s" filename="%s"/>\n' \
                % (utils.getIsoTimestamp(), self._xmlEscAttr(\
                os.path.basename(sSubXmlName))));
        return None;
...
class RemoteReporter(ReporterBase):
    ...
    def __init__(self):
        ...
        self._sTmServerPath = '/%s/testboxdisp.py?%s' \
            % (self._oParsedTmUrl.path.strip('/'), urllib.urlencode(dParams),);
    ...
    def _doUploadFile(self, oSrcFile, sSrcFilename, sDescription, sKind, sMime):
        ...
        sUrl = self._sTmServerPath + '&' \
             + self._fnUrlEncode({constants.tbreq.UPLOAD_PARAM_NAME: \
                os.path.basename(sSrcFilename), \
    ...
    def _xmlFlushIfNecessary(self):
        ...
        if cSecs >= self.kcSecXmlFlushMin and cSecsLast >= self.kcSecXmlFlushIdle:
            return self.xmlFlush();
    ...
    def subXmlEnd(self, oFileWrapper):
        ...
        sRawXml, '<PopHint  testdepth="%d"/>' % (len(self.atTests),),];
        self._xmlFlushIfNecessary();
        g_oLock.release();
        return None;
