    ...
    def pollTask(self, fLocked=False):
        ...
        if not fRc:
            ...
            if uPid == self.uPid:
                ...
                if self.uExitCode != 0 and (self.sKindCrashReport is not None or \
                    self.sKindCrashDump is not None):
# --lb-- reporter.error('Process "%s" returned/crashed with a non-zero status code!!
# --lb-- rc=%u sig=%u%s (raw=%#x)' % (self.sName, self.uExitCode >> 8,
# --lb-- self.uExitCode & 0x7f, ' w/ core' if self.uExitCode & 0x80 else '',
# --lb-- self.uExitCode)) utils.processCollectCrashInfo(self.uPid,
# --lb-- reporter.log,  self._addCrashFile);
                    reporter.error('Process "%s" returned rc=%u sig=%u%s (raw=%#x)' \
                        % (self.sName, self.uExitCode >> 8, self.uExitCode & 0x7f, \
                        ' w/ core', self.uExitCode));
                    utils.processCollectCrashInfo(self.uPid, \
                        reporter.log, self._addCrashFile);
            fRc = self.fSignalled;
        if not fLocked:
            self.unlockTask();
        return fRc;
