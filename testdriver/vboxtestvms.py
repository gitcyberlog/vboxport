# -*- coding: utf-8 -*-
# $Id: vboxtestvms.py 53581 2014-12-19 23:10:02Z vboxsync $
...
# --lb-- import base
# --lb-- from testdriver import reporter;
# --lb-- from testdriver import vboxcon;
# Validation Kit imports.
...
g_kasParavirtProviders = (g_ksParavirtProviderNone, g_ksParavirtProviderDefault, \
...
# --lb--pylint--: --lb--enable--lb--=--lb--C0301
...
class TestVm(object):
    ...
    def __init__(self, oSet, sVmName, sHd=None, sKind=None, acCpusSup=None, \
        asVirtModesSup=None, \
        fIoApic=None, fPae=None, sNic0AttachType=None, \
        sHddControllerType='IDE Controller', sFloppy=None, fVmmDevTestingPart=None, \
        fVmmDevTestingMmio=False, asParavirtModesSup=None, fRandomPvPMode=False):
        self.oSet = oSet;
            ...
            if self.fSnapshotRestoreCurrent is True:
                ...
                if self.is64bitRequired() and not fHostSupports64bit:
                    ...
                    if oSession is not None:
                        ...
                        fRc = fRc and oSession.enableNestedPaging(\
                            sVirtMode == 'hwvirt-np');
...
class TestVmSet(object):
    ...
    def __init__(self, oTestVmManager=None, acCpus=None, asVirtModes=None, \
        fIgnoreSkippedVm=False):
        ...
        if asArgs[iArg] == '--virt-modes':
            ...
            for sPvMode in self.asParavirtModes:
                if sPvMode not in g_kasParavirtProviders:
                    raise base.InvalidOption(\
                'The "--paravirt-modes" value "%s" is not valid; valid values are: %s' \
            ...
            for cCpus in acCpusSup:
                ...
                for sVirtMode in asVirtModesSup:
                    ...
                    for sParavirtMode in asParavirtModes:
                        ...
                        # Reconfigure the VM.
                        try:
                            (rc2, oVM) = oTestVm.getReconfiguredVm(oTestDrv, cCpus, \
                                sVirtMode, sParavirtMode=sParavirtMode);
...
class TestVmManager(object):
    ...
    def getStandardVmSet(self, sTxsTransport):
        ...
        oTestVm = TestVm(oSet, 'tst-nt4sp1', \
            sHd='4.2/' + sTxsTransport + '/nt4sp1/t-nt4sp1.vdi', \
            sKind='WindowsNT4', acCpusSup=[1]);
        oSet.aoTestVms.append(oTestVm);

        oTestVm = TestVm(oSet, 'tst-xppro', \
            sHd='4.2/' + sTxsTransport + '/xppro/t-xppro.vdi', \
    ...
    def getSmokeVmSet(self):
        ...
        oTestVm = TestVm(oSet, 'tst-rhel5', sHd='3.0/tcp/rhel5.vdi', \
            sKind='RedHat', acCpusSup=range(1, 33), fIoApic=True, \
            sNic0AttachType='nat');
        oSet.aoTestVms.append(oTestVm);

        oTestVm = TestVm(oSet, 'tst-win2k3ent', sHd='3.0/tcp/win2k3ent-acpi.vdi', \
            sKind='Windows2003', acCpusSup=range(1, 33), fPae=True, \
            sNic0AttachType='bridged');
        oSet.aoTestVms.append(oTestVm);

        oTestVm = TestVm(oSet, 'tst-sol10', sHd='3.0/tcp/solaris10.vdi', \
            sKind='Solaris', acCpusSup=range(1, 33), fPae=True, \
            sNic0AttachType='bridged');
        oSet.aoTestVms.append(oTestVm);
        ...
        oTestVm = TestVm(oSet, 'tst-xpsp2-halaacpi', \
            sHd='4.2/xpsp2/t-winxp-halaacpi.vdi', \
            sKind='WindowsXP', acCpusSup=range(1, 33), fIoApic=True);
        oSet.aoTestVms.append(oTestVm);

        oTestVm = TestVm(oSet, 'tst-xpsp2-halacpi', \
            sHd='4.2/xpsp2/t-winxp-halacpi.vdi', \
            sKind='WindowsXP', acCpusSup=range(1, 33), fIoApic=True);
        oSet.aoTestVms.append(oTestVm);

        oTestVm = TestVm(oSet, 'tst-xpsp2-halapic', \
            sHd='4.2/xpsp2/t-winxp-halapic.vdi', \
            sKind='WindowsXP', acCpusSup=range(1, 33), fIoApic=True);
        oSet.aoTestVms.append(oTestVm);

        oTestVm = TestVm(oSet, 'tst-xpsp2-halmacpi', \
            sHd='4.2/xpsp2/t-winxp-halmacpi.vdi', \
            sKind='WindowsXP', acCpusSup=range(2, 33), fIoApic=True);
        oSet.aoTestVms.append(oTestVm);

        oTestVm = TestVm(oSet, 'tst-xpsp2-halmps', \
            sHd='4.2/xpsp2/t-winxp-halmps.vdi', \
            sKind='WindowsXP', acCpusSup=range(2, 33), fIoApic=True);
        oSet.aoTestVms.append(oTestVm);
