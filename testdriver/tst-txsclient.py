# -*- coding: utf-8 -*-
# $Id: tst-txsclient.py 52776 2014-09-17 14:51:43Z vboxsync $
...
def main(asArgs): # pylint: disable=C0111,R0914,R0915
    ...
    if uPort is None:
        oSession = txsclient.openTcpSession(cMsTimeout, sAddress, \
            fReversedSetup=fReversedSetup);
    else:
        oSession = txsclient.openTcpSession(cMsTimeout, sAddress, uPort=uPort, \
            fReversedSetup=fReversedSetup);
    if oSession is None:
    ...
    if fDone is True and oSession.isSuccess():
        if fStdTests:
            ...
            if rc is True:
                ...
                rc = oSession.syncExecEx(sProg, (sProg, 'there', 'is no such', \
                    'parameter'), oStdOut='${SCRATCH}/stdout', \
            ...
            rc = oSession.syncExecEx(sProg, (sProg,), oStdIn='/dev/null', \
                oStdOut='/dev/null', oStdErr='/dev/null', oTestPipe='/dev/null', \
