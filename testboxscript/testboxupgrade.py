# -*- coding: utf-8 -*-
# $Id: testboxupgrade.py 52776 2014-09-17 14:51:43Z vboxsync $
...
def _doUpgradeUnzipAndCheck(oZip, sUpgradeDir, asMembers):
    ...
    for sMember in asMembers:
        if sMember.endswith('/'):
            os.makedirs(os.path.join(sUpgradeDir, sMember.replace(\
                '/', os.path.sep)), 0775);
        else:
...
def _doUpgradeTestRun(sUpgradeDir):
    ...
    Returns True/False.
    """
    asArgs = [os.path.join(sUpgradeDir, 'testboxscript', 'testboxscript', \
