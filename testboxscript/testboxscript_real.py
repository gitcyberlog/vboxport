#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: testboxscript_real.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class TestBoxScript(object):
    ...
    def _mountShare(self, sMountPoint, sType, sServer, sShare, sUser, sPassword, sWhat):
        ...
        if sHostOs in ('darwin', 'freebsd'):
            ...
            if sType == 'cifs':
                ...
                utils.processOutputChecked(['/sbin/mount_smbfs', '-o', \
                    'automounted,nostreams,soft,noowners,noatime,rdonly', \
                    '-f', '0555', '-d', '0555', '//%s:%s@%s/%s' % (sUser, \
                    sPassword, sServer, sShare), sMountPoint]);
            else:
            ...
            utils.sudoProcessCall(['/bin/mkdir', '-p', sMountPoint]);
            if sType == 'cifs':
                utils.sudoProcessOutputChecked(['/bin/mount', '-t', 'cifs', '-o', \
                    'user=' + sUser + ',password=' + sPassword + ',sec=ntlmv2' + \
                    ',uid=' + str(os.getuid()) + ',gid=' + str(os.getgid()) +\
                    ',nounix,file_mode=0555,dir_mode=0555,soft,ro', + \
                    '//%s/%s' % (sServer, sShare), sMountPoint]);
                    # pylint: disable=E1101
                    # pylint: disable=E1101
            elif sType == 'nfs':
                utils.sudoProcessOutputChecked(['/bin/mount', '-t', 'nfs', '-o', \
                    'soft,ro', '%s:%s' % (sServer, sShare if sShare.find('/') >= 0 \
                    else ('/export/' + sShare)), sMountPoint]);
            ...
            if sType == 'cifs':
                ...
                oPasswdFile.flush();
                utils.sudoProcessOutputChecked(['/sbin/mount', '-F', 'smbfs', '-o', \
                    'user=' + sUser + ',uid=' + str(os.getuid()) + ',gid=' + \
                    str(os.getgid()) + ',fileperms=0555,dirperms=0555,noxattr,ro', \
                    '//%s/%s' % (sServer, sShare), sMountPoint], stdin=oPasswdFile);
                oPasswdFile.close();
                    # pylint: disable=E1101
                    # pylint: disable=E1101
            elif sType == 'nfs':
                utils.sudoProcessOutputChecked(['/sbin/mount', '-F', 'nfs', '-o', \
                    'noxattr,ro', '%s:%s' % (sServer, sShare if sShare.find('/') >= 0 \
                    else ('/export/' + sShare)), sMountPoint]);
    ...
    def main():
        ...
        for sMixed, sDefault, sDesc in [('Builds', sDefBuilds, 'builds'), (\
            'TestRsrc', sDefTestRsrc, 'test resources')]:
            sLower = sMixed.lower();
            sPrefix = 's' + sMixed;
            parser.add_option('--' + sLower + '-path', dest=sPrefix + 'Path', \
                metavar='<abs-path>', default=sDefault, help='Where ' + sDesc + \
                ' can be found');
            parser.add_option('--' + sLower + '-server-type', dest=sPrefix + \
                'ServerType', metavar='<nfs|cifs>', default=None, \
                help='The type of server, cifs or nfs. If empty (default), we won\'t try mount anything.');
            parser.add_option('--' + sLower + '-server-name', dest=sPrefix + \
                'ServerName', metavar='<server>', default='solserv.de.oracle.com', \
                help='The name of the server with the builds.');
            parser.add_option('--' + sLower + '-server-share', dest=sPrefix + \
                'ServerShare', metavar='<share>', default=sLower, \
                help='The name of the builds share.');
            parser.add_option('--' + sLower + '-server-user', dest=sPrefix + \
                'ServerUser', metavar='<user>', default='guestr', \
                help='The user name to use when accessing the ' + sDesc + ' share.');
            parser.add_option('--' + sLower + '-server-passwd', '--' + sLower + \
                '-server-password', dest=sPrefix + 'ServerPasswd', \
                metavar='<password>', default='guestr', \
                help='The password to use when accessing the ' + sDesc + ' share.');

        parser.add_option("--test-manager", metavar="<url>", dest="sTestManagerUrl", \
            help="Test Manager URL", \
            default="http://tindertux.de.oracle.com/testmanager")
        parser.add_option("--scratch-root", metavar="<abs-path>", \
            dest="sScratchRoot", help="Path to the scratch directory", default=None);
        parser.add_option("--system-uuid", metavar="<uuid>", dest="sSystemUuid", \
            help="The system UUID of the testbox, used for uniquely identifiying" + \
            " the machine", default=None);
        parser.add_option("--hwvirt", dest="fHasHwVirt", action="store_true", \
            default=None, help="Hardware virtualization available in the CPU");
        parser.add_option("--no-hwvirt", dest="fHasHwVirt", action="store_false", \
            default=None, help="Hardware virtualization not available in the CPU");
        parser.add_option("--nested-paging", dest="fHasNestedPaging", \
            action="store_true", default=None, help="Nested paging is available");
        parser.add_option("--no-nested-paging", dest="fHasNestedPaging", \
            action="store_false", default=None, help="Nested paging is not available");
        parser.add_option("--64-bit-guest", dest="fCan64BitGuest", \
            action="store_true", default=None, help="Host can execute 64-bit guests");
        parser.add_option("--no-64-bit-guest", dest="fCan64BitGuest", \
            action="store_false", default=None, \
            help="Host cannot execute 64-bit guests");
        parser.add_option("--io-mmu", dest="fHasIoMmu", \
            action="store_true", default=None, help="I/O MMU available");
        parser.add_option("--no-io-mmu", dest="fHasIoMmu", \
            action="store_false", default=None, help="No I/O MMU available");
        parser.add_option("--pidfile", dest="sPidFile", default=None, \
            help="For the parent script, ignored.");
        parser.add_option("-E", "--putenv", metavar="<variable>=<value>", \
            action="append", dest="asEnvVars", default=[], \
