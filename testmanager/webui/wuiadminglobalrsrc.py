# -*- coding: utf-8 -*-
# $Id: wuiadminglobalrsrc.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiGlobalResource(WuiContentBase):
    ...
    def showAddModifyPage(self, sAction, dErrors=None):
        ...
        if sAction == WuiAdmin.ksActionGlobalRsrcAdd:
            ...
            sFormActionUrl += '&%s=%s' % (GlobalResourceData.ksParam_idGlobalRsrc, \
                self._oData.idGlobalRsrc)
        else:
            raise WuiException('Invalid paraemter "%s"' % (sAction,))

        oForm = WuiHlpForm('globalresourceform', sFormActionUrl, dErrors if \
            dErrors is not None else dict())
...
class WuiGlobalResourceList(WuiListContentBase):
    ...
    def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
        WuiListContentBase.__init__(self, aoEntries, iPage, cItemsPerPage, \
            tsEffective, sTitle='Global Resources', sId='globalResources', \
    ...
    def _formatListEntry(self, iEntry):
        ...
        return [oEntry.idGlobalRsrc, oEntry.sName, oEntry.sDescription, \
            oEntry.fEnabled, [WuiTmLink('Modify', WuiAdmin.ksScriptName, \
                ...
                GlobalResourceData.ksParam_idGlobalRsrc: oEntry.idGlobalRsrc}, \
                sConfirm='Are you sure you want to remove global resource #%d?' \
                % (oEntry.idGlobalRsrc,)),\
            ]\
        ]
