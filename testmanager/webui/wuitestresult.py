# -*- coding: utf-8 -*-
# $Id: wuitestresult.py 53039 2014-10-13 10:49:38Z vboxsync $
...
class WuiTestResult(WuiContentBase):
    ...
    def _recursivelyGenerateEvents(self, oTestResult, sParentName, sLineage, iRow,
        ...
        if oTestResult.tsElapsed is not None:
            ...
            % (WuiMain.ksParamReportSubjectIds, \
            ReportGraphModel.ksTypeElapsed, sLineage);
        ...
        if len(oTestResult.aoChildren) == 0 \
          ...
          and len(oTestResult.aoFiles) == 0:
            ...
            sHtml = \
            ' <tr class="%s tmtbl-events-leaf tmtbl-events-lvl%s tmstatusrow-%s">\n' \
            '  <td>%s</td>\n' \
            '  <td>%s</td>\n' \
            '  <td>%s</td>\n' \
            '  <td>%s</td>\n' \
            ...
            ' </tr>\n' \
                   ...
                   oTestResult.enmStatus, \
                   webutils.escapeElem(self.formatTsShort(tsEvent)), \
                   sElapsedGraph, \
                   webutils.escapeElem(str(oTestResult.tsElapsed)) if \
                    oTestResult.tsElapsed is not None else '', sDisplayName, \
                    ' id="failure-%u"' % (iFailure,) if oTestResult.isFailure() \
                    else '', webutils.escapeElem(oTestResult.enmStatus),  \
                    webutils.escapeElem(sErrCnt), sResultGraph);
            iRow += 1;
        else:
            # Multiple rows.
            sHtml = ' <tr class="%s tmtbl-events-first tmtbl-events-lvl%s ">\n' \
                     ...
                     ' </tr>\n' \
                   % ('tmodd' if iRow & 1 else 'tmeven', iDepth, \
                   webutils.escapeElem(self.formatTsShort(oTestResult.tsCreated)), \
                   sDisplayName, 'running' if oTestResult.tsElapsed is None else '',);
                   ## @todo more timeline stuff later.
            iRow += 1;

            # Depth.
            for oChild in oTestResult.aoChildren:
                (sChildHtml, iRow, iFailure) = \
                    self._recursivelyGenerateEvents(oChild, sName, sLineage, iRow, \
                    iFailure, oTestSet, iDepth + 1);
                sHtml += sChildHtml;
            ...
            for oMsg in oTestResult.aoMsgs:
                sHtml += \
                    ' <tr class="%s tmtbl-events-message tmtbl-events-lvl%s">\n' \
                    '  <td>%s</td>\n' \
                    '  <td></td>\n' \
                    '  <td></td>\n' \
                    '  <td colspan="3">%s: %s</td>\n' \
            ...
            for oValue in oTestResult.aoValues:
                sHtml += ' <tr class="%s tmtbl-events-value tmtbl-events-lvl%s">\n' \
                       ...
                       WuiMain.ksParamReportSubjectIds, \
                       ReportGraphModel.ksTypeValue, sLineage, oValue.idStrName,);
                iRow += 1;
            ...
            if oTestResult.tsElapsed is not None:
                sHtml += \
	' <tr class="%s tmtbl-events-final tmtbl-events-lvl%s tmstatusrow-%s">\n' \
	'  <td>%s</td>\n <td>%s</td>\n <td>%s</td>\n <td>%s</td>\n ' \
	'<td colspan="2"%s>%s%s</td>\n <td>%s</td>\n </tr>\n' \
	    % ('tmodd' if iRow & 1 else 'tmeven', iDepth, oTestResult.enmStatus, \
	    webutils.escapeElem(self.formatTsShort(oTestResult.tsCreated + \
	    oTestResult.tsElapsed)), sElapsedGraph, \
	    webutils.escapeElem(str(oTestResult.tsElapsed)), sDisplayName, \
    ...
    def showTestCaseResultDetails(self, oTestResultTree, oTestSet, oBuildEx, \
        oValidationKitEx, oTestBox, oTestGroup, oTestCaseEx, oTestVarEx):
        ...
        def getGrDepsHtmlList(aoGlobalResourceData):
            ...
            if len(aoGlobalResourceData) > 0:
                ...
                for oGlobalResourceData in aoGlobalResourceData:
                    sTmp += '<li>%s</li>' \
                        % (webutils.escapeElem(oGlobalResourceData.sName),);
                sTmp += '</ul>'
            else:
                sTmp = 'No items'
            return sTmp
        ...
        aoResultRows = [WuiTmLink(oTestCaseEx.sName, self.oWuiAdmin.ksScriptName, \
        ...
        cSecCfgTimeout = oTestCaseEx.cSecTimeout if oTestVarEx.cSecTimeout is None \
            else oTestVarEx.cSecTimeout;
        cSecEffTimeout = cSecCfgTimeout * oTestBox.pctScaleTimeout / 100;
        aoResultRows.append(['Timeout:', '%s (%s sec)' \
            % (utils.formatIntervalSeconds(cSecEffTimeout), cSecEffTimeout,)]);
        if cSecEffTimeout != cSecCfgTimeout:
            aoResultRows.append(['Cfg Timeout:', '%s (%s sec)' \
                % (utils.formatIntervalSeconds(cSecCfgTimeout), cSecCfgTimeout,)]);
        aoResultRows += [('Started:', WuiTmLink(\
            self.formatTsShort(oTestSet.tsCreated), WuiMain.ksScriptName, \
        ...
        if oTestVarEx.cGangMembers > 1:
            aoResultRows.append(['Member No:', '#%s (of %s)' \
                % (oTestSet.iGangMemberNo, oTestVarEx.cGangMembers)]);

        aoResultRows += [('Test Group:', \
            WuiTmLink(oTestGroup.sName, self.oWuiAdmin.ksScriptName, \
        ...
        if oBuildEx is not None:
            aoBuildRows += [WuiTmLink('Build', self.oWuiAdmin.ksScriptName, \
            ...
            aoBuildRows += [('Revision:', WuiSvnLinkWithTooltip(oBuildEx.iRevision, \
                    oBuildEx.oCat.sRepository, fBracketed=False)), \
                ('Product:', oBuildEx.oCat.sProduct), \
                ('Branch:', oBuildEx.oCat.sBranch), ('Type:', oBuildEx.oCat.sType), \
                ('Version:', oBuildEx.sVersion), ('Created:', oBuildEx.tsCreated),];
            if oBuildEx.uidAuthor is not None:
                aoBuildRows += [('Author ID:', oBuildEx.uidAuthor),];
            if oBuildEx.sLogUrl is not None:
                aoBuildRows += [('Log:', WuiBuildLogLink(oBuildEx.sLogUrl, \
                fBracketed=False)),];
        ...
        if oValidationKitEx is not None:
            ...
            aoValidationKitRows += [('Created:', oValidationKitEx.tsCreated),];
        ...
        aoTestBoxRows += [\
            ...
            ('Script Rev:', WuiSvnLink(oTestBox.iTestBoxScriptRev, \
                fBracketed=False)), \
            ('Python:', oTestBox.formatPythonVersion()), \
            ('Pending Command:', oTestBox.enmPendingCmd),];

        aoRows = [aoResultRows, aoBuildRows, aoValidationKitRows, aoTestBoxRows,];
        ...
        sHtmlEvents += ' <tr class="tmheader">\n' \
                       ...
                       ' </tr>\n';
        sPrettyCmdLine = '&nbsp;\\<br>&nbsp;&nbsp;&nbsp;&nbsp;\n'.join(\
        webutils.escapeElem(oTestCaseEx.sBaseCmd + ' ' + oTestVarEx.sArgs).split());
        ...
        sFormButton = '<button type="submit" onclick="%s">Show graphs</button>' \
            % (webutils.escapeAttr(\
            'addDynamicGraphInputs("graph-form", "main", "%s", "%s");' \
...
class WuiGroupedResultList(WuiListContentBase):
    ...
    def __init__(self, aoEntries, cEntriesCount, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
        ...
        WuiListContentBase.__init__(self, aoEntries, iPage, cItemsPerPage, \
            tsEffective, sTitle='Ungrouped (%d)' % cEntriesCount, sId='results', \
            fnDPrint=fnDPrint, oDisp=oDisp);

        self._cEntriesCount = cEntriesCount

        self._asColumnHeaders = ['Start', 'Product Build', 'Validation Kit', \
            'TestBox OS', 'TestBox Name', 'Test Case', 'Elapsed', 'Result',];
    ...
    def _formatListEntry(self, iEntry):
        ...
        if oEntry.idBuildTestSuite is not None:
            oValidationKit = WuiTmLink('#%d - r%s' % (oEntry.idBuildTestSuite, \
            oEntry.iRevisionTestSuite), WuiAdmin.ksScriptName, \
            {WuiAdmin.ksParamAction: WuiAdmin.ksActionBuildDetails, \
            BuildData.ksParam_idBuild: oEntry.idBuildTestSuite}, fBracketed=False);

        aoTestSetLinks = [WuiTmLink(oEntry.enmStatus, WuiMain.ksScriptName, \
        {WuiMain.ksParamAction: WuiMain.ksActionTestResultDetails, \
        TestSetData.ksParam_idTestSet: oEntry.idTestSet}, fBracketed=False),];
        if oEntry.cErrors > 0:
        ...
        return [oEntry.tsCreated, \
            [WuiTmLink('#%d - %s %s (%s)' % (oEntry.idBuild, oEntry.sProduct, \
                oEntry.sVersion, oEntry.sType,), \
            WuiMain.ksScriptName, self._dRevLinkParams, sTitle='%s' \
                % (oEntry.sBranch,), fBracketed=False), \
            WuiSvnLinkWithTooltip(oEntry.iRevision, 'vbox'), \
            WuiTmLink(self.ksShortDetailsLink, WuiAdmin.ksScriptName, \
            {WuiAdmin.ksParamAction: WuiAdmin.ksActionBuildDetails, \
            BuildData.ksParam_idBuild: oEntry.idBuild}, fBracketed=False),], \
            oValidationKit, '%s.%s' % (oEntry.sOs, oEntry.sArch), \
            [WuiTmLink(oEntry.sTestBoxName, WuiMain.ksScriptName, \
            self._dTestBoxLinkParams, fBracketed=False, sTitle=sTestBoxTitle), \
            WuiTmLink(self.ksShortDetailsLink, WuiAdmin.ksScriptName, \
            {WuiAdmin.ksParamAction: WuiAdmin.ksActionTestBoxDetails, \
            TestBoxData.ksParam_idTestBox: oEntry.idTestBox}, fBracketed=False)], \
            [WuiTmLink(oEntry.sTestCaseName, WuiMain.ksScriptName, \
            self._dTestCaseLinkParams, fBracketed=False, \
            sTitle=(oEntry.sBaseCmd + ' ' + oEntry.sArgs) if oEntry.sArgs \
                else oEntry.sBaseCmd), \
            WuiTmLink(self.ksShortDetailsLink, WuiAdmin.ksScriptName, \
                {WuiAdmin.ksParamAction: WuiAdmin.ksActionTestCaseDetails, \
                TestCaseData.ksParam_idTestCase: oEntry.idTestCase}, \
                fBracketed=False),], oEntry.tsElapsed, aoTestSetLinks\
        ];
