# -*- coding: utf-8 -*-
# $Id: wuibase.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiDispatcherBase(object):
    ...
    def _generatePage(self):
        ...
        if self._sDebug == '':
            if config.g_kfWebUiSqlTrace or self._fDbgSqlTrace or self._fDbgSqlExplain:
                ...
                % (utils.formatNumber(utils.timestampNano() - \
                self._oSrvGlue.tsStart,), self._oDb.debugHtmlReport(\
                self._oSrvGlue.tsStart));
    ...
    def getEffectiveDateParam(self, sParamName=None):
        ...
        sName = sParamName if sParamName is not None else \
            WuiDispatcherBase.ksParamEffectiveDate
        ...
        sValue = self._dParams[sName];
        if isinstance(sValue, list):
            raise WuiException('%s parameter "%s" is given multiple times: %s' \
                % (self._sAction, sName, sValue));
        sValue = sValue.strip();
        if sValue == '':
            return None;
    ...
    def _actionGenericListing(self, oLogicType, oListContentType):
        ...
        aoEntries = oLogicType(self._oDb).fetchForListing(iPage * cItemsPerPage, \
            cItemsPerPage + 1, tsEffective);
    ...
    def _actionGenericFormDetails(self, oDataType, oLogicType, oFormType, sIdAttr, sGenIdAttr=None): # pylint: disable=R0914
        ...
        if fChangeLog and hasattr(oLogicType, 'fetchForChangeLog'):
            (aoEntries, fMore) = oLogicType(self._oDb).fetchForChangeLog(getattr(\
                oData, sIdAttr), iChangeLogPageNo * cChangeLogEntriesPerPage, \
                cChangeLogEntriesPerPage, tsNow);
            self._sPageBody += oContent.showChangeLog(aoEntries, fMore, \
                iChangeLogPageNo, cChangeLogEntriesPerPage, tsNow);
        return True
    ...
    def _actionGenericFormAddPost(self, oDataType, oLogicType, oFormType, sRedirAction, fStrict=True):
        ...
        return self._actionGenericFormPost(WuiFormContentBase.ksMode_Add, \
            oLogic.addEntry, oDataType, oFormType, '?' + webutils.encodeUrlParams(\
            {self.ksParamAction: sRedirAction}), fStrict=fStrict)
    def _actionGenericFormEditPost(self, oDataType, oLogicType, oFormType, \
        sRedirAction, fStrict=True):
        """
        ...
        oLogic = oLogicType(self._oDb);
        from testmanager.webui.wuicontentbase import WuiFormContentBase;
        return self._actionGenericFormPost(WuiFormContentBase.ksMode_Edit, \
            oLogic.editEntry, oDataType, oFormType, '?' + webutils.encodeUrlParams(\
            {self.ksParamAction: sRedirAction}), fStrict=fStrict);
