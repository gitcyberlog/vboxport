# -*- coding: utf-8 -*-
# $Id: wuihlpform.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiHlpForm(object):
    ...
    def addTextHidden(self, sName, sValue, sExtraAttribs=''):
        ...
        '        <input name="%s" id="%s" type="text" hidden%s value="%s" class="tmform-hidden">\n'\
            ...
            sExtraAttribs, escapeElem(str(sValue))));

    #
    # Text input fields.
    #
    def addText(self, sName, sValue, sLabel, sSubClass='string', sExtraAttribs='', \
        sPostHtml=''):
        """Adds a text input."""
        if self._fReadOnly:
    ...
    def addTextRO(self, sName, sValue, sLabel, sSubClass='string', sExtraAttribs='', \
        sPostHtml=''):
        """Adds a read-only text input."""
        if sSubClass not in ('int', 'long', 'string', 'uuid', 'timestamp', 'wide'): raise Exception(sSubClass);
        self._addLabel(sName, sLabel, sSubClass);
        return self._add(\
            '          <input name="%s" id="%s" type="text" readonly%s value="%s"'\
            ' class="tmform-input-readonly">%s\n'\
            '        </div></div>\n'\
            '      </li>\n' % (escapeAttr(sName), escapeAttr(sName), \
                         sExtraAttribs, escapeElem(str(sValue)), sPostHtml));
    ...
    def addMultilineText(self, sName, sValue, sLabel, sSubClass='string', \
        sExtraAttribs=''):
        """Adds a multiline text input."""
        if self._fReadOnly:
            return self.addMultilineTextRO(sName, sValue, sLabel, sSubClass, \
                sExtraAttribs);
        ...
        return self._add('          <textarea name="%s" id="%s" %s>%s</textarea>\n'\
            '        </div></div>\n'\
            '      </li>\n' % (escapeAttr(sName), escapeAttr(sName), \
            sExtraAttribs, escapeElem(sNewValue)))

    def addMultilineTextRO(self, sName, sValue, sLabel, sSubClass='string', \
        sExtraAttribs=''):
        ...
        if fUseTable:
            ...
            for asRow in aoRows:
                ...
                self._add('            <tr>\n'
                          ...
                          '              <td>%s</td>\n'
                          '            </tr>\n'
                          % (sNameEscaped, escapeAttr(str(asRow[0])), ' checked' if \
                          fChecked else '', sExtraAttribs, escapeElem(str(asRow[2])),));
            self._add('          </table>\n');
        else:
            for asRow in aoRows:
                assert len(asRow) == 3; # Don't allow sloppy input data!
    ...
    def addListOfOsArches(self, sName, aoOsArches, sLabel, sExtraAttribs=''):
        ...
        """
        return self._addList(sName, aoOsArches, sLabel, fUseTable=False, \
            sId='tmform-checkbox-list-os-arches', sExtraAttribs=sExtraAttribs);
    ...
    def addListOfTypes(self, sName, aoTypes, sLabel, sExtraAttribs=''):
        ...
        """
        return self._addList(sName, aoTypes, sLabel, fUseTable=False, \
            sId='tmform-checkbox-list-build-types', sExtraAttribs=sExtraAttribs);
    ...
    def addListOfTestCases(self, sName, aoTestCases, sLabel, sExtraAttribs=''):
        ...
        """
        return self._addList(sName, aoTestCases, sLabel, fUseTable=False, \
            sId='tmform-checkbox-list-testcases', sExtraAttribs=sExtraAttribs);
    ...
    def addListOfResources(self, sName, aoTestCases, sLabel, sExtraAttribs=''):
        ...
        """
        return self._addList(sName, aoTestCases, sLabel, fUseTable=False, \
            sId='tmform-checkbox-list-resources', sExtraAttribs=sExtraAttribs);
    ...
    def addListOfTestGroups(self, sName, aoTestGroups, sLabel, sExtraAttribs=''):
        ...
        """
        return self._addList(sName, aoTestGroups, sLabel, fUseTable=False, \
            sId='tmform-checkbox-list-testgroups', sExtraAttribs=sExtraAttribs);
    ...
    def addListOfTestCaseArgs(self, sName, aoVariations, sLabel): # pylint: disable=R0915
        ...
        if not fReadOnly:
            ...
            sHtml += '    %s_extendListEx("%s", "%s", "%s", "%s", "%s");\n' % (sName, \
                escapeAttr(str(oVarDefaults.cGangMembers)), escapeAttr(str(\
                    oVarDefaults.cSecTimeout)), escapeAttr(oVarDefaults.sArgs), \
                escapeAttr(oVarDefaults.sTestBoxReqExpr), escapeAttr(\
                    oVarDefaults.sBuildReqExpr),);
            sHtml += '}\n';
        ...
        for iVar in range(len(aoVariations)):
            ...
            sHtml += '  <tr class="tmform-testcasevars-first-row">\n' \
                   ...
                   % (sName, iVar, TestCaseArgsData.ksParam_cGangMembers, sName, \
                   iVar, oVar.cGangMembers, sReadOnlyAttr, sName, iVar, \
                   TestCaseArgsData.ksParam_cSecTimeout, sName, iVar, \
                   utils.formatIntervalSeconds2(oVar.cSecTimeout), sReadOnlyAttr,);
            if not fReadOnly:
                sHtml += \
                    '    <td><a href="#" onclick="%s_removeEntry(\'%s\');">Remove</a></td>\n' \
                    % (sName, iVar);
            else:
                sHtml += '    <td></td>\n';
            ...
            sHtml += '  <tr class="tmform-testcasevars-inner-row">\n' \
                ...
                '    <td></td>\n' \
                '  </tr>\n' \
                % (sName, iVar, TestCaseArgsData.ksParam_sArgs, sName, iVar, \
                escapeAttr(oVar.sArgs), sReadOnlyAttr)
            ...
            sHtml += '  <tr class="tmform-testcasevars-inner-row">\n' \
                ...
                '    <td></td>\n' \
                '  </tr>\n' % (sName, iVar, TestCaseArgsData.ksParam_sTestBoxReqExpr, \
                sName, iVar, escapeAttr(oVar.sTestBoxReqExpr), sReadOnlyAttr);
            ...
            sHtml += '  <tr class="tmform-testcasevars-final-row">\n' \
                     ...
                     '    <td></td>\n' \
                     '  </tr>\n' \
                   % (sName, iVar, TestCaseArgsData.ksParam_sBuildReqExpr, sName, \
                   iVar, escapeAttr(oVar.sBuildReqExpr), sReadOnlyAttr)
  ...
  # pylint: disable=R0914
    def addListOfTestGroupMembers(self, sName, aoTestGroupMembers, aoAllTestCases, \
...
# (list)
            self._add('  <tr class="%s">\n'\
                ...
                len(oTestCase.aoTestCaseArgs), sPrefix, \
                TestGroupMemberData.ksParam_idTestCase, oTestCase.idTestCase, \
                sPrefix, TestGroupMemberData.ksParam_idTestGroup, -1 if oMember \
                    is None else oMember.idTestGroup, sPrefix, \
                TestGroupMemberData.ksParam_tsExpire, '' if oMember \
                    is None else oMember.tsExpire, sPrefix, \
                TestGroupMemberData.ksParam_tsEffective, '' if oMember \
                    is None else oMember.tsEffective, sPrefix, \
                TestGroupMemberData.ksParam_uidAuthor, '' if oMember \
                    is None else oMember.uidAuthor, \
                TestGroupDataEx.ksParam_aoMembers, '' if oMember \
                    is None else ' checked', sCheckBoxAttr, oTestCase.idTestCase, \
                oTestCase.idTestCase, escapeElem(oTestCase.sName),));
            ...
            self._add('    <td rowspan="%d" title="Include all variations (checked) or choose a set?">\n'\
                      ...
                      ' checked' if oMember is None or oMember.aidTestCaseArgs \
                      is None else '', sCheckBoxAttr,));
            ...
            self._add('    <td rowspan="%d" align="center">\n'
                      ...
                      '    </td>\n' % (len(oTestCase.aoTestCaseArgs), \
                          ...
                          (oMember if oMember is not None else \
                          oDefMember).iSchedPriority, \
                          ' readonly class="tmform-input-readonly"' if fReadOnly \
                          else '',));

            # Argument variations.
            aidTestCaseArgs = [] if oMember is None or oMember.aidTestCaseArgs \
                is None else oMember.aidTestCaseArgs;
            for iVar in range(len(oTestCase.aoTestCaseArgs)):
                oVar = oTestCase.aoTestCaseArgs[iVar];
                if iVar > 0:
                    self._add('  <tr class="%s">\n' % ('tmodd' if iTestCase & 1 \
                        else 'tmeven',));
                self._add('   <td align="center">\n'\
                    '     <input name="%s[%s]" type="checkbox"%s%s value="%d">'\
                    '   </td>\n'\
                    % (sPrefix, TestGroupMemberData.ksParam_aidTestCaseArgs, \
                    ' checked' if oVar.idTestCaseArgs in aidTestCaseArgs \
                    else '', sCheckBoxAttr, oVar.idTestCaseArgs,));
...
# pylint: disable=R0914
    ...
    aoAllTestGroups, sLabel, fReadOnly=True):
        """
        ...
        self._add('<input name="%s" type="hidden" value="%s">\n'\
            ...
            ','.join([str(oTestGroup.idTestGroup) for oTestGroup \
            in aoAllTestGroups]),));
...
# (list)
            self._add('  <tr class="%s">\n'\
                ...
                sPrefix, SchedGroupMemberData.ksParam_idTestGroup, \
                oTestGroup.idTestGroup, sPrefix, \
                SchedGroupMemberData.ksParam_idSchedGroup, -1 if oMember is  None \
                else oMember.idSchedGroup, sPrefix, \
                SchedGroupMemberData.ksParam_tsExpire, '' if oMember is None \
                else oMember.tsExpire, sPrefix, \
                SchedGroupMemberData.ksParam_tsEffective, '' if oMember is None \
                else oMember.tsEffective, sPrefix, \
                SchedGroupMemberData.ksParam_uidAuthor, '' if oMember is None \
                else oMember.uidAuthor, SchedGroupDataEx.ksParam_aoMembers, '' \
                if oMember is None else ' checked', sCheckBoxAttr, \
                oTestGroup.idTestGroup, oTestGroup.idTestGroup, escapeElem(\
                oTestGroup.sName),));
            ...
            sPrefix, SchedGroupMemberData.ksParam_idTestGroupPreReq, sComboBoxAttr, \
            ' selected' if oMember is None or oMember.idTestGroupPreReq is None \
            else '',));
            for oTestGroup2 in aoAllTestGroups:
                if oTestGroup2 != oTestGroup:
                    fSelected = oMember is not None and oTestGroup2.idTestGroup == \
                        oMember.idTestGroupPreReq;
                    self._add('        <option value="%s"%s>%s</option>\n' \
                        % (oTestGroup2.idTestGroup, ' selected' if fSelected \
                        else '', escapeElem(oTestGroup2.sName),));
            self._add('      </select>\n'
                      '    </td>\n');
