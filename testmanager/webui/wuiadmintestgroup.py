# -*- coding: utf-8 -*-
# $Id: wuiadmintestgroup.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiTestGroupList(WuiListContentBase):
    ...
    def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
        ...
        tsEffective=tsEffective, sTitle='Test Groups', fnDPrint=fnDPrint, oDisp=oDisp);
    ...
    def _formatListEntry(self, iEntry):
        ...
        if len(oEntry.aoMembers) > 0:
            for oMember in oEntry.aoMembers:
                ...
                '  <dd><strong>%s</strong> (priority: %d) %s %s</dd>\n' \
                       ...
                       oMember.iSchedPriority, WuiTmLink('Details', \
                       WuiAdmin.ksScriptName, {WuiAdmin.ksParamAction: \
                       WuiAdmin.ksActionTestCaseDetails, \
                       TestCaseData.ksParam_idGenTestCase: \
                       oMember.oTestCase.idGenTestCase,}).toHtml(), \
                       WuiTmLink('Edit', WuiAdmin.ksScriptName, \
                       {WuiAdmin.ksParamAction: WuiAdmin.ksActionTestCaseEdit, \
                       TestCaseData.ksParam_idTestCase: oMember.oTestCase.idTestCase,\
                       }).toHtml() \
                ...
                cArgsIncluded = 0;
                for oVar in oMember.oTestCase.aoTestCaseArgs:
                    if oMember.aidTestCaseArgs is None or oVar.idTestCaseArgs \
                    in oMember.aidTestCaseArgs:
                        cArgsIncluded += 1;
                        if fNoGang:
                            sHtml += '      <tr>';
                        else:
                        ...
                        % (utils.formatIntervalSeconds(oMember.oTestCase.cSecTimeout \
                            if oVar.cSecTimeout is None else oVar.cSecTimeout), \
                            webutils.escapeElem(oVar.sArgs),);
                if cArgsIncluded == 0:
                    sHtml += \
                    '      <tr><td colspan="%u">No arguments selected.</td></tr>\n' \
                    % (2 if fNoGang else 3,);
                sHtml += '    </table>\n' \
                         '  </dl>\n';
        ...
        return [oEntry.idTestGroup, oEntry.sName, oEntry.sDescription \
            if oEntry.sDescription is not None else '', oTestCases, aoActions];
