# -*- coding: utf-8 -*-
# $Id: wuimain.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiMain(WuiDispatcherBase):
    ...
    kasGraphWizImplValid = [ksGraphWizImpl_Default, ksGraphWizImpl_Matplotlib, \
        ksGraphWizImpl_Charts];
    ...
    def __init__(self, oSrvGlue):
        ...
        d[self.ksActionResultsUnGrouped] = lambda: self._actionGroupedResultsListing(\
            TestResultLogic.ksResultsGroupingTypeNone, TestResultLogic, \
            WuiGroupedResultList)

        d[self.ksActionResultsGroupedByTestGroup] = lambda: \
            self._actionGroupedResultsListing(\
            TestResultLogic.ksResultsGroupingTypeTestGroup, TestResultLogic, \
            WuiGroupedResultList)

        d[self.ksActionResultsGroupedByBuildRev] = lambda: \
            self._actionGroupedResultsListing(\
            TestResultLogic.ksResultsGroupingTypeBuildRev, TestResultLogic, \
            WuiGroupedResultList)

        d[self.ksActionResultsGroupedByTestBox] = lambda: \
            self._actionGroupedResultsListing(\
            TestResultLogic.ksResultsGroupingTypeTestBox, TestResultLogic, \
            WuiGroupedResultList)

        d[self.ksActionResultsGroupedByTestCase] = lambda: \
            self._actionGroupedResultsListing(\
            TestResultLogic.ksResultsGroupingTypeTestCase, TestResultLogic, \
            WuiGroupedResultList)

        d[self.ksActionResultsGroupedBySchedGroup] = lambda: \
            self._actionGroupedResultsListing(\
            TestResultLogic.ksResultsGroupingTypeSchedGroup, TestResultLogic, \
            WuiGroupedResultList)
    ...
    def _actionDefault(self):
        ...
        return self._actionGroupedResultsListing(\
            TestResultLogic.ksResultsGroupingTypeNone, TestResultLogic, \
                WuiGroupedResultList)
        ...
        for iGroupMemberId, sGroupMemberName in aoGroupMembers:
            if iGroupMemberId is not None:
                sHtmlSelector += '    <option value="%s"%s>%s</option>\n' \
                    % (iGroupMemberId, ' selected="selected"' if \
                    iGroupMemberId == iCurrentMember else '', sGroupMemberName)
    ...
    def _generatePagesSelector(self, dParams, cItems, cItemsPerPage, iPage):
        ...
        sHrefPtr = '<a href="?%s&%s=' % (webutils.encodeUrlParams(\
            dParams).replace('%', '%%'), WuiDispatcherBase.ksParamPageNo)
        sHrefPtr += '%d">%s</a>'
        ...
        # 1 2 3 4...
        sHtmlPager = '&nbsp;\n'.join(sHrefPtr % (x, str(x + 1)) if x != iPage \
            else str(x + 1) for x in range(cPagesRangeStart, cPagesRangeEnd))
        ...
        if iPage > 0:
            ...
            % (webutils.encodeUrlParams(dParams),)) + sHtmlPager;
        else:
            sHtmlPager = '&lt;&lt;&nbsp;&nbsp;\n' + sHtmlPager
    ...
    def _generateReportNavigation(self, tsEffective, cHoursPerPeriod, cPeriods):
        ...
        for cHours in [2, 3, 4, 5, 6, 7, 8, 9, 12, 18, 24, 48, 72, 96, 120, 144, 168]:
            sHtmlPeriodLength += '    <option value="%d"%s>%d hours</option>\n' \
                % (cHours, 'selected="selected"' if cHours == cHoursPerPeriod \
                    else '', cHours);
        ...
        % (WuiMain.ksParamReportPeriods, webutils.encodeUrlParams(dParams), \
         WuiMain.ksParamReportPeriods)
        for cCurPeriods in range(2, 43):
            sHtmlCountOfPeriods += '    <option value="%d"%s>%d</option>\n' \
                % (cCurPeriods, 'selected="selected"' if cCurPeriods == cPeriods \
                else '', cCurPeriods);
        sHtmlCountOfPeriods += '  </select>\n' \
                               '</form>\n'

        # The time walker.
        sHtmlTimeWalker = self._generateTimeWalker(self.getParameters(), \
            tsEffective, '%d hours' % (cHoursPerPeriod));
    ...
    def _actionGroupedResultsListing(self, enmResultsGroupingType, \
        oResultsLogicType, oResultsListContentType):
        """
        ...
        """
        cItemsPerPage = self.getIntParam(self.ksParamItemsPerPage, iMin=2, \
            iMax=9999, iDefault=128)
        iPage = self.getIntParam(self.ksParamPageNo, iMin=0, iMax=999999, iDefault=0)
        tsEffective = self.getEffectiveDateParam()
        iGroupMemberId = self.getIntParam(self.ksParamGroupMemberId, iMin=-1, \
            iMax=999999, iDefault=-1)
        ...
        sCurPeriod = self.getStringParam(self.ksParamEffectivePeriod, \
            asValidValues=asValidValues, sDefault=self.ksResultPeriodDefault)
        ...
        if   enmResultsGroupingType == TestResultLogic.ksResultsGroupingTypeNone \
          or enmResultsGroupingType == TestResultLogic.ksResultsGroupingTypeTestBox:
            ...
            aoGroupMembers = sorted(list(set([(x.idTestBox, '%s (%s)' \
                % (x.sName, str(x.ip))) for x in aoTmp])), reverse=False, \
                key=lambda asData: asData[1]);
            ...
            aoGroupMembers = sorted(list(set([(x.idTestGroup, x.sName) \
                for x in aoTmp])), reverse=False, key=lambda asData: asData[1]);
            ...
            aoGroupMembers = sorted(list(set([(x.iRevision, '%s.%d' \
                % (x.oCat.sBranch, x.iRevision)) for x in aoTmp])), reverse=True, \
                key=lambda asData: asData[0])
            ...
            aoGroupMembers = sorted(list(set([(x.idTestCase, '%s' \
                % x.sName) for x in aoTmp])), reverse=False, \
                key=lambda asData: asData[1])
            ...
            aoGroupMembers = sorted(list(set([(x.idSchedGroup, '%s' \
                % x.sName) for x in aoTmp])), reverse=False, \
                key=lambda asData: asData[1]);
        ...
        for idMember, sMemberName in aoGroupMembers:
            ...
            sInterval=sCurPeriod, enmResultsGroupingType=enmResultsGroupingType, \
            iResultsGroupingValue=idMember, fOnlyFailures=fOnlyFailures);
            if cEntries == 0: # Do not display empty groups
                continue
            aoEntries = oResultLogic.fetchResultsForListing(iPage * cItemsPerPage, \
                cItemsPerPage, tsNow=tsEffective, sInterval=sCurPeriod, \
            ...
            # Format them.
            #
            oContent = oResultsListContentType(aoEntries, cEntries, iPage, \
                cItemsPerPage, tsEffective, fnDPrint=self._oSrvGlue.dprint, \
                oDisp=self);
        ...
        # bottom of it.
        #
        sHtmlNavigation = self._generateResultNavigation(cEntriesMax, cItemsPerPage, \
            iPage, tsEffective, sCurPeriod, fOnlyFailures, \
        ...
        if cEntriesMax > 0:
            ...
            self._sPageBody = sHtmlNavigation + \
                '<p align="center"><i>No data to display</i></p>\n';
        return True;
    ...
    def actionTestResultDetails(self):
        ...
        oBuildDataEx = BuildDataEx().initFromDbWithId(self._oDb, \
            oTestSetData.idBuild, oTestSetData.tsCreated);
        try: oBuildValidationKitDataEx = BuildDataEx().initFromDbWithId(self._oDb, \
            oTestSetData.idBuildTestSuite, oTestSetData.tsCreated);
        except: oBuildValidationKitDataEx = None;
        oTestBoxData = TestBoxData().initFromDbWithGenId(self._oDb, \
            oTestSetData.idGenTestBox);
        oTestGroupData = TestGroupData().initFromDbWithId(self._oDb, \
            oTestSetData.idTestGroup, oTestSetData.tsCreated); ## @todo This bogus time wise. Bad DB design?
        oTestCaseDataEx = TestCaseDataEx().initFromDbWithGenId(self._oDb, \
            oTestSetData.idGenTestCase, oTestSetData.tsConfig);
        oTestCaseArgsDataEx = TestCaseArgsDataEx().initFromDbWithGenIdEx(self._oDb, \
            oTestSetData.idGenTestCaseArgs, oTestSetData.tsConfig);

        oContent = WuiTestResult(oDisp=self, fnDPrint=self._oSrvGlue.dprint);
        (self._sPageTitle, self._sPageBody) = oContent.showTestCaseResultDetails(\
            oTestResultTree, oTestSetData, oBuildDataEx, oBuildValidationKitDataEx, \
            oTestBoxData, oTestGroupData, oTestCaseDataEx, oTestCaseArgsDataEx);
        return True
    ...
    def actionViewLog(self):
        ...
        iChunk = self.getIntParam(self.ksParamLogChunkNo, iMin=0, iMax=config.g_kcMbMaxMainLog * 1048576 / cbChunk, iDefault=0);
        self._checkForUnknownParameters();
    ...
    def _actionGenericReport(self, oModelType, oReportType):
        ...
        sSubject = self.getStringParam(self.ksParamReportSubject, ReportModelBase.kasSubjects, ReportModelBase.ksSubEverything);
    ...
    def _actionGraphWiz(self):
        ...
        sSubject = self.getStringParam(self.ksParamReportSubject, \
        [ReportModelBase.ksSubEverything], ReportModelBase.ksSubEverything); # dummy
        ...
        enmGraphImpl = self.getStringParam(self.ksParamGraphWizImpl, \
            asValidValues=self.kasGraphWizImplValid, \
        ...
        oModel = ReportGraphModel(self._oDb, tsEffective, cPeriods, cHoursPerPeriod, \
            sSubject, asSubjectIds, aidTestBoxes, aidBuildCats, aidTestCases, \
            fSepTestVars);
        oContent = WuiGraphWiz(oModel, dParams, fSubReport=False, \
    ...
    def _actionVcsHistoryTooltip(self):
        ...
        iRevision = self.getIntParam(self.ksParamVcsHistoryRevision, iMin=0, \
            iMax=999999999);
        sRepository = self.getStringParam(self.ksParamVcsHistoryRepository);
        cEntries = self.getIntParam(self.ksParamVcsHistoryEntries, iMin=1, \
            iMax=1024, iDefault=8);
        self._checkForUnknownParameters();

        aoEntries = VcsRevisionLogic(self._oDb).fetchTimeline(sRepository, \
            iRevision, cEntries);
        oContent = WuiVcsHistoryTooltip(aoEntries, sRepository, iRevision, \
        cEntries, fnDPrint=self._oSrvGlue.dprint, oDisp=self);
        (self._sPageTitle, self._sPageBody) = oContent.show();
        return True;
