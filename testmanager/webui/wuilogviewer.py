# -*- coding: utf-8 -*-
# $Id: wuilogviewer.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiLogViewer(WuiContentBase):
    ...
    def _generateNavigation(self, cbFile):
        ...
        sHrefFmt = '<a href="?%s&%s=%%s" title="%%s">%%s</a>' \
            % (webutils.encodeUrlParams(dParams2).replace('%', '%%'), \
            WuiMain.ksParamLogChunkNo,);
        sHtmlWalker = self.genericPageWalker(self._iChunk, (cbFile + \
            self._cbChunk - 1) / self._cbChunk, sHrefFmt, 11, 0, 'chunk');
        ...
        sHtmlSize = '<form name="ChunkSizeForm" method="GET">\n' \
            ...
            % (WuiMain.ksParamLogChunkSize, webutils.encodeUrlParams(dParams2), \
            WuiMain.ksParamLogChunkSize,);

        for cbChunk in [256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, \
            131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216]:
            sHtmlSize += '    <option value="%d" %s>%d bytes</option>\n' \
           % (cbChunk, 'selected="selected"' \
           if cbChunk == self._cbChunk else '', cbChunk);
