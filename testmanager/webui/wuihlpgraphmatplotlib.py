...
class WuiHlpGraphMatplotlibBase(WuiHlpGraphBase):
    ...
    def _createFigure(self):
        ...
        oFigure = matplotlib.pyplot.figure(figsize=(float(self._cxGraph) / \
        self._cDpiGraph, float(self._cyGraph) / self._cDpiGraph), \
        dpi=self._cDpiGraph);
        return oFigure;
...
class WuiHlpBarGraph(WuiHlpGraphMatplotlibBase):
    ...
    def renderGraph(self): # pylint: disable=R0914
        ...
        for i in range(len(aoSeries)):
            ...
            aoBars.append(oSubPlot.bar(oXRange + self.cxBarWidth * i, aoSeries[i], \
                self.cxBarWidth, color=sColor, align='edge'));
        ...
        for i in range(len(aoBars)):
            ...
            for j in range(len(aoRects)):
                ...
                if fpValue <= fpMid:
                    ...
                    oRect.get_height() + fpPadding, aoTable[j + 1].asValues[i], \
                    ha='center', va='bottom', rotation='vertical', alpha=0.6, \
                    fontsize='small');
                else:
                    oSubPlot.text(oRect.get_x() + oRect.get_width() / 2.0, \
                        oRect.get_height() - fpPadding, aoTable[j + 1].asValues[i], \
                        ha='center', va='top', rotation='vertical', alpha=0.6, \
                        fontsize='small');
