# -*- coding: utf-8 -*-
# $Id: wuiadminbuild.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiAdminBuild(WuiFormContentBase):
    ...
    def _populateForm(self, oForm, oData):
        ...
        oForm.addComboBox(BuildData.ksParam_idBuildCategory, oData.idBuildCategory, \
        'Build category', BuildCategoryLogic(self._oDisp.getDb()).fetchForCombo());
...
class WuiAdminBuildList(WuiListContentBase):
    ...
    def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
        ...
        tsEffective, sTitle='Builds', sId='builds', fnDPrint=fnDPrint, oDisp=oDisp);

        self._asColumnHeaders = ['ID', 'Product', 'Branch', 'Version', 'Type', \
        'OS(es)', 'Author', 'Added', 'Files', 'Action'];
    ...
    def _formatListEntry(self, iEntry):
        ...
        if isDbTimestampInfinity(oEntry.tsExpire):
            aoActions += [WuiTmLink('Modify', WuiAdmin.ksScriptName, \
                ...
                BuildData.ksParam_idBuild: oEntry.idBuild}, \
                sConfirm='Are you sure you want to remove build #%d?' \
                % (oEntry.idBuild,)),\
            ];

        return [oEntry.idBuild, oEntry.oCat.sProduct, oEntry.oCat.sBranch, \
                 WuiSvnLinkWithTooltip(oEntry.iRevision, oEntry.oCat.sRepository, \
                 sName='%s r%s' % (oEntry.sVersion, oEntry.iRevision,)), \
                 oEntry.oCat.sType, ' '.join(oEntry.oCat.asOsArches), \
