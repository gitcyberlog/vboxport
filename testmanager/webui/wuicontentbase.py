# -*- coding: utf-8 -*-
# $Id: wuicontentbase.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiFormContentBase(WuiSingleContentBase): # pylint: disable=R0903
    ...
    def _formatChangeLogEntry(self, aoEntries, iEntry):
        ...
        sContent = '    <tr class="%s">\n' \
                   ...
                   '      <td colspan="3">%s%s</td>\n' \
                   '    </tr>\n' \
                   % (sRowClass, len(oEntry.aoChanges) + 1, webutils.escapeElem(\
                   self.formatTsShort(oEntry.tsEffective)), \
                   len(oEntry.aoChanges) + 1, webutils.escapeElem(\
                   self.formatTsShort(oEntry.tsExpire)), \
                   self._guessChangeLogEntryDescription(aoEntries, iEntry), \
                   ' '.join(oLink.toHtml() for oLink in \
                   self._calcChangeLogEntryLinks(aoEntries, iEntry)),);

        # Additional rows for each changed attribute.
        j = 0;
        for oChange in oEntry.aoChanges:
            sContent += \
'        <tr class="%s%s"><td>%s</td><td>%s</td><td>%s</td></tr>\n' \
            % (sRowClass, 'odd' if j & 1 else 'even', \
            webutils.escapeElem(oChange.sAttr), webutils.escapeElem(\
            oChange.sOldText), webutils.escapeElem(oChange.sNewText),);
            j += 1;
    ...
    def _showChangeLogNavi(self, fMoreEntries, iPageNo, cEntriesPerPage, tsNow, sWhere):
        ...
        for iEntriesPerPage in [2, 4, 8, 16, 32, 64, 128, 256, 384, 512, 768, 1024, 1536, 2048, 3072, 4096, 8192]:
            sNavigation += \
            '        <option value="%d" %s>%d entries per page</option>\n' \
            % (iEntriesPerPage, 'selected="selected"' if iEntriesPerPage == cEntriesPerPage else '', iEntriesPerPage);
        sNavigation += '      </select>\n';
        ...
        if fMoreEntries:
            dParams[WuiDispatcherBase.ksParamChangeLogPageNo] = iPageNo + 1;
            sNavigation += \
            '    <td align="right"><a href="?%s#tmchangelog">Next</a></td>\n' \
            % (webutils.encodeUrlParams(dParams),);
        else:
...
class WuiListContentWithActionBase(WuiListContentBase):
    ...
    def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffectiveDate, sTitle, sId=None, fnDPrint=None, oDisp=None):
        ...
        tsEffectiveDate, sTitle, sId=sId, fnDPrint=fnDPrint, oDisp=oDisp);
        ...
        self._asColumnHeaders = \
            [WuiRawHtml('<input type="checkbox" onClick="toggle%s(this)">' \
    ...
    def _getCheckBoxColumn(self, iEntry, sValue):
        ...
        % (webutils.escapeAttr(self._sCheckboxName), \
        webutils.escapeAttr(unicode(sValue))));
