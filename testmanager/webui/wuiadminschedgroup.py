# -*- coding: utf-8 -*-
# $Id: wuiadminschedgroup.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiSchedGroup(WuiFormContentBase):
    ...
    def _populateForm(self, oForm, oData):
        ...
        oForm.addComboBox(SchedGroupData.ksParam_enmScheduler, oData.enmScheduler, \
        'Scheduler type', SchedGroupData.kasSchedulerDesc)

        aoBuildSrcIds = BuildSourceLogic(self._oDisp.getDb()).fetchForCombo();
        oForm.addComboBox(SchedGroupData.ksParam_idBuildSrc, oData.idBuildSrc, \
        'Build source', aoBuildSrcIds);
...
#   def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
    def __init__(self, aoEntries, iPage, cItemsPerPage, fnDPrint, oDisp):
        ...
        self._asColumnHeaders = ['ID', 'Name', 'Enabled', 'Scheduler Type', \
            'Build Source', 'Validation Kit Source', 'Test Groups', 'TestBoxes', \
            'Actions',];
    ...
    def _formatListEntry(self, iEntry):
        ...
        if oEntry.idBuildSrc is not None:
            oBuildSrc = WuiTmLink(oEntry.oBuildSrc.sName \
            if oEntry.oBuildSrc else str(oEntry.idBuildSrc), WuiAdmin.ksScriptName, \
        ...
        if oEntry.idBuildSrcTestSuite is not None:
            oValidationKitSrc = WuiTmLink(oEntry.oBuildSrcValidationKit.sName if \
                oEntry.oBuildSrcValidationKit else str(oEntry.idBuildSrcTestSuite), \
        ...
        for oMember in oEntry.aoMembers:
            aoMembers.append(WuiTmLink(oMember.oTestGroup.sName, \
                WuiAdmin.ksScriptName, \
                ...
                % (oMember.idTestGroup, oMember.oTestGroup.sDescription,)));
        ...
        return [oEntry.idSchedGroup, oEntry.sName, oEntry.fEnabled, \
            oEntry.enmScheduler, oBuildSrc, oValidationKitSrc, aoMembers, \
            aoTestBoxes, aoActions,];
