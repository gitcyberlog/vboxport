# -*- coding: utf-8 -*-
# $Id: wuigraphwiz.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiGraphWiz(WuiReportBase):
    ...
    def __init__(self, oModel, dParams, fSubReport=False, fnDPrint=None, oDisp=None):
        WuiReportBase.__init__(self, oModel, dParams, fSubReport=fSubReport, \
    ...
    def _generateInteractiveForm(self):
        ...
        sTop = '<form action="#" method="get" id="graphwiz-form">\n' \
                ...
                WuiMain.ksParamGraphWizSrcTestSetId, \
                self._dParams[WuiMain.ksParamGraphWizSrcTestSetId],);
        ...
        sTop += '    <label for="%s">Starting:</label>\n' \
            ...
            % (WuiMain.ksParamEffectiveDate, WuiMain.ksParamEffectiveDate, WuiMain.ksParamEffectiveDate, sNow,);
        ...
        sTop += '    <label for="%s"> Going back:\n' \
            ...
            WuiMain.ksParamReportPeriodInHours, \
            WuiMain.ksParamReportPeriodInHours, \
            utils.formatIntervalHours(self._dParams[\
            WuiMain.ksParamReportPeriodInHours]));
        sTop += '   </div>\n';
        ...
        sTop += '    <label for="%s">Graph:</label>\n' \
            '    <select name="%s" id="%s">\n' \
            % (WuiMain.ksParamGraphWizImpl, WuiMain.ksParamGraphWizImpl, \
            WuiMain.ksParamGraphWizImpl,);
        for (sImpl, sDesc) in WuiMain.kaasGraphWizImplCombo:
            sTop += '     <option value="%s"%s>%s</option>\n' \
            % (sImpl, ' selected' if sImpl == self._dParams[\
            WuiMain.ksParamGraphWizImpl] else '', sDesc);
        sTop += '    </select>\n';
        ...
        sTop += '    <label for="%s">Graph size:</label>\n' \
            ...
            % (WuiMain.ksParamGraphWizWidth, WuiMain.ksParamGraphWizWidth, \
            WuiMain.ksParamGraphWizWidth, \
            self._dParams[WuiMain.ksParamGraphWizWidth], \
            WuiMain.ksParamGraphWizHeight, WuiMain.ksParamGraphWizHeight, \
            self._dParams[WuiMain.ksParamGraphWizHeight], \
            WuiMain.ksParamGraphWizDpi, WuiMain.ksParamGraphWizDpi, \
            WuiMain.ksParamGraphWizDpi, self._dParams[WuiMain.ksParamGraphWizDpi], \
            webutils.escapeAttr(\
            'return graphwizSetDefaultSizeValues("graphwiz-nav", "%s", "%s", "%s");' \
        ...
        if hasattr(self.oGraphClass, 'setXkcdStyle'):
            sTop += '    <input type="checkbox" name="%s" id="%s" value="1"%s/>\n' \
                ...
                WuiMain.ksParamGraphWizXkcdStyle, ' checked' if \
                self._dParams[WuiMain.ksParamGraphWizXkcdStyle] else '', \
                WuiMain.ksParamGraphWizXkcdStyle);
        ...
        if not hasattr(self.oGraphClass, 'kfNoErrorBarsSupport'):
            sTop += '    <input type="checkbox" name="%s" id="%s" value="1"%s title="%s"/>\n' \
                ...
                WuiMain.ksParamGraphWizErrorBarY, \
                ' checked' if self._dParams[WuiMain.ksParamGraphWizErrorBarY] \
                else '', \
        ...
        sTop += '    <label for="%s">Font size: </label>\n' \
            ...
            WuiMain.ksParamGraphWizFontSize, self._dParams[\
        ...
        sEnd += \
            '  <div id="graphwiz-testboxes" class="graphwiz-end-selection-group">\n' \
        ...
        for oTestBox in self._oModel.getEligibleTestBoxes():
            ...
            sEnd += \
            '   <li><input type="checkbox" name="%s" value="%s" id="gw-tb-%u"%s/>' \
            '<label for="gw-tb-%u">%s</label></li>\n' \
            % (WuiMain.ksParamGraphWizTestBoxIds, oTestBox.idTestBox, \
            oTestBox.idTestBox, sChecked, oTestBox.idTestBox, oTestBox.sName);
        ...
        for idTestBox in aidTestBoxes:
            ...
            sEnd += \
'   <li><input type="checkbox" name="%s" value="%s" id="gw-tb-%u" checked/>' \
                '<label for="gw-tb-%u">%s</label></li>\n' \
                % (WuiMain.ksParamGraphWizTestBoxIds, oTestBox.idTestBox, \
                oTestBox.idTestBox, oTestBox.idTestBox, oTestBox.sName);
        ...
        for oBuildCat in self._oModel.getEligibleBuildCategories():
            ...
            sEnd += \
            '    <li><input type="checkbox" name="%s" value="%s" id="gw-bc-%u" %s/>' \
            '<label for="gw-bc-%u">%s / %s / %s / %s</label></li>\n' \
            % (WuiMain.ksParamGraphWizBuildCatIds, oBuildCat.idBuildCategory, \
            oBuildCat.idBuildCategory, sChecked, oBuildCat.idBuildCategory, \
            oBuildCat.sProduct, oBuildCat.sBranch, oBuildCat.sType, \
            ' & '.join(oBuildCat.asOsArches));
        ...
        sEnd += '    <li>\n' \
            ...
            ','.join([str(i) for i in \
            self._dParams[WuiMain.ksParamGraphWizTestCaseIds]]),);
...
'    <input type="checkbox" name="%s" id="%s" value="%s:%s%s" checked class="graphwiz-src-input">\n' \
                         '    <label for="%s">%s</label>\n' \
                         '   </div>\n' \
                % (WuiMain.ksParamReportSubjectIds, WuiMain.ksParamReportSubjectIds, \
                oCollection.sType, ':'.join([str(idStr) for idStr in \
                oCollection.aidStrTests]), ':%u' % oCollection.idStrValue if \
                oCollection.idStrValue else '', WuiMain.ksParamReportSubjectIds, \
                sSampleName);
            ...
            if len(oCollection.aoSeries) > 0:
                ...
                for aoSeries in aaoSeries:
                    ...
                    for oSeries in aoSeries:
                        sSeriesName = self._getSeriesNameFromBits(oSeries, \
                            fSeriesName);
                        asHtmlTooltips = None;
                        if len(oSeries.aoRevInfo) == len(oSeries.aiRevisions):
                            ...
                            for i in range(len(oSeries.aoRevInfo)):
                                ...
                                if oSeries.acSamples[i] > 1:
                                    ...
                                    % (utils.formatNumber(\
                                    oSeries.aiErrorBarAbove[i]), utils.formatNumber(\
                                    oSeries.aiErrorBarBelow[i]), oSeries.acSamples[i]);
                                sTooltip = '<table class=\'graphwiz-tt\'><tr><td>%s:</td><td>%s %s %s</td></tr>'\
                                    '<tr><td>Rev:</td><td>r%s</td></tr>' \
                                    % (sSeriesName, utils.formatNumber(\
                                    oSeries.aiValues[i]), sYUnit, sPlusMinus, \
                                ...
                                if oRevInfo.sAuthor is not None:
                                    ...
                                    sTooltip += \
                                        ...
                                        % (oRevInfo.sAuthor, self.formatTsShort(\
                                            oRevInfo.tsCreated), \
                                        sMsg, '...' if len(oRevInfo.sMessage) > \
                                        len(sMsg) else '');
                                sTooltip += '</table>';
                                asHtmlTooltips.append(sTooltip);
                        oData.addDataSeries(sSeriesName, oSeries.aiRevisions, \
                        oSeries.aiValues, asHtmlTooltips, oSeries.aiErrorBarBelow, \
                    ...
                    oGraph.setTitle(self._calcGraphName(aoSeries[0], fSeriesName, \
                        sSampleName));
                ...
                if self._dParams[WuiMain.ksParamGraphWizTabular]:
                    sHtml += \
                        '   <div class="graphwiz-tab-div" id="graphwiz-tab-%u">\n' \
...
' <tr class="graphwiz-tab graphwiz-tab-col-hdr-row">\n' \
                                     ...
                                     ' </tr>\n' \
                                     '</thead>\n' \
                                   % (sColor, self._getSeriesNameFromBits(oSeries, \
                                   self.kfSeriesName_All & ~self.kfSeriesName_OsArchs), sUnit);

                            for i in range(len(oSeries.aiRevisions)):
                                sHtml += \
'     <tr class="%s"><td>r%s</td><td>%s</td><td>+%s</td><td>-%s</td><td>%s</td></tr>\n' \
                                    % ('tmodd' if i & 1 else 'tmeven', \
                                    oSeries.aiRevisions[i], oSeries.aiValues[i], \
                                    oSeries.aiErrorBarAbove[i], \
                                    oSeries.aiErrorBarBelow[i], oSeries.acSamples[i]);
                    sHtml += '    </table>\n' \
                             '   </div>\n';
            else:
