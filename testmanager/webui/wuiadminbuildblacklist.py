# -*- coding: utf-8 -*-
# $Id: wuiadminbuildblacklist.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiAdminBuildBlacklist(WuiFormContentBase):
    ...
    def _populateForm(self, oForm, oData):
        ...
        if len(aoFailureReasons) == 0:
            ...
            raise WuiException(\
                'Please <a href="%s?%s=%s">add</a> some Failure Reasons first.' \
                % (WuiAdmin.ksScriptName, WuiAdmin.ksParamAction, \
                WuiAdmin.ksActionFailureReasonShowAdd));

        asTypes = self.getListOfItems(self.asTypes, oData.asTypes)
        asOsArches = self.getListOfItems(self.asOsArches, oData.asOsArches)

        oForm.addIntRO(BuildBlacklistData.ksParam_idBlacklisting, \
            oData.idBlacklisting, 'Blacklist item ID')
        oForm.addTimestampRO(BuildBlacklistData.ksParam_tsEffective, \
            oData.tsEffective, 'Last changed')
        oForm.addTimestampRO(BuildBlacklistData.ksParam_tsExpire, \
            oData.tsExpire, 'Expires (excl)')
        oForm.addIntRO(BuildBlacklistData.ksParam_uidAuthor, oData.uidAuthor, \
            'Changed by UID')

        oForm.addComboBox(BuildBlacklistData.ksParam_idFailureReason, \
            oData.idFailureReason, 'Failure Reason', aoFailureReasons)
        ...
        oForm.addListOfTypes(BuildBlacklistData.ksParam_asTypes, asTypes, \
            'Build types')
        oForm.addListOfOsArches(BuildBlacklistData.ksParam_asOsArches, asOsArches, \
            'Target architectures')
        oForm.addInt(BuildBlacklistData.ksParam_iFirstRevision, \
            oData.iFirstRevision, 'First revision')
        oForm.addInt(BuildBlacklistData.ksParam_iLastRevision, \
            oData.iLastRevision, 'Last revision (incl)')
...
class WuiAdminListOfBlacklistItems(WuiListContentBase):
    ...
    def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
        ...
        tsEffective, sTitle='Build Blacklist', sId='buildsBlacklist', \
        fnDPrint=fnDPrint, oDisp=oDisp);

        self._asColumnHeaders = ['ID', 'Failure Reason', 'Product', 'Branch', \
        'Type', 'OS(es)', 'First Revision', 'Last Revision', 'Actions']
    ...
    def _formatListEntry(self, iEntry):
        ...
        sShortFailReason = \
            FailureReasonLogic(\
                TMDatabaseConnection()).getById(oEntry.idFailureReason).sShort;

        return [oEntry.idBlacklisting, sShortFailReason, oEntry.sProduct, \
        oEntry.sBranch, oEntry.asTypes, oEntry.asOsArches, oEntry.iFirstRevision, \
        oEntry.iLastRevision, \
                 ...
                 BuildBlacklistData.ksParam_idBlacklisting: oEntry.idBlacklisting}, \
                 sConfirm='Are you sure you want to remove black list entry #%d?' \
