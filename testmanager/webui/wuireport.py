# -*- coding: utf-8 -*-
# $Id: wuireport.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiReportSuccessRate(WuiReportBase):
    ...
    def generateReportBody(self):
        ...
        for i in range(len(adPeriods)):
            ...
            cSuccess = dStatuses[ReportModelBase.ksTestStatus_Success] + \
            ...
            if cTotal > 0:
                ...
                oTable.addRow(sPeriod, [iPctSuccess, iPctSkipped, iPctFailure], [\
                    '%s%% (%d)' % (iPctSuccess, dStatuses[\
                        ReportModelBase.ksTestStatus_Success]), \
                    '%s%% (%d)' % (iPctSkipped, dStatuses[\
                        ReportModelBase.ksTestStatus_Skipped]), \
                    '%s%% (%d)' % (iPctFailure, dStatuses[\
                        ReportModelBase.ksTestStatus_Failure]),\
                ]);
            else:
        ...
        if cTotalNow > 0:
            sReport += '%s%% (thereof %s%% skipped)</p>\n' \
                % (cSuccessNow * 100 / cTotalNow, \
                    adPeriods[0][ReportModelBase.ksTestStatus_Skipped] * \
                        100 / cTotalNow);
        else:
            sReport += 'N/A</p>\n'
...
class WuiReportSummary(WuiReportBase):
    ...
    def generateReportBody(self):
        ...
        oSuccessRate = WuiReportSuccessRate(self._oModel, self._dParams, \
            fSubReport=True, fnDPrint=self._fnDPrint, oDisp=self._oDisp);
