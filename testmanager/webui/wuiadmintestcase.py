# -*- coding: utf-8 -*-
# $Id: wuiadmintestcase.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiTestCaseList(WuiListContentBase):
    ...
    def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
        ...
        tsEffective, sTitle='Test Cases', fnDPrint=fnDPrint, oDisp=oDisp);
        self._asColumnHeaders = \
        [
            'Name', 'Active', 'Default Timeout', 'Base Command / Variations', \
            'Validation Kit Files', 'Test Case Prereqs', 'Global Resources', 'Actions'
        ];
    ...
    def _formatListEntry(self, iEntry):
        ...
        for oTmp in oEntry.aoTestCaseArgs:
            ...
            sHtml += '<td>%s</td><td>%s</td></tr>\n' \
                   ...
                   if oTmp.cSecTimeout is not None else 'Default', webutils.escapeElem(oTmp.sArgs.replace('-', u'\u2011')),)
        sHtml += '  </table>'
...
class WuiTestCase(WuiFormContentBase):
    ...
    def _populateForm(self, oForm, oData):
        ...
        oForm.addLong(TestCaseData.ksParam_cSecTimeout, utils.formatIntervalSeconds2(oData.cSecTimeout), 'Default timeout')
        ...
        oForm.addListOfTestCases(TestCaseDataEx.ksParam_aoDepTestCases, \
