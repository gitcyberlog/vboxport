# -*- coding: utf-8 -*-
# $Id: wuiadminbuildsource.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class WuiAdminBuildSrc(WuiFormContentBase):
    ...
    def _populateForm(self, oForm, oData):
        ...
        utils.formatIntervalSeconds2(oData.cSecMaxAge) if oData.cSecMaxAge \
        not in [-1, '', None] else '', 'Max age in seconds');
        oForm.addSubmit();
        return True;
...
class WuiAdminBuildSrcList(WuiListContentBase):
    ...
    def __init__(self, aoEntries, iPage, cItemsPerPage, tsEffective, fnDPrint, oDisp):
        WuiListContentBase.__init__(self, aoEntries, iPage, cItemsPerPage, \
           tsEffective, sTitle='Registered Build Sources', sId='build sources', \
           fnDPrint=fnDPrint, oDisp=oDisp);
        self._asColumnHeaders = ['ID', 'Name', 'Description', 'Product', 'Branch', \
            'Build Types', 'OS/ARCH', 'First Revision', 'Last Revision', 'Max Age', \
            'Actions'];
        self._asColumnAttribs = ['align="center"', 'align="center"', \
            'align="center"', 'align="center"', 'align="center"', 'align="left"', \
            'align="left"', 'align="center"', 'align="center"', 'align="center"', \
            'align="center"'];
    ...
    def _formatListEntry(self, iEntry):
        ...
        if isDbTimestampInfinity(oEntry.tsExpire):
            aoActions += [WuiTmLink('Modify', WuiAdmin.ksScriptName, \
        ...
        return [oEntry.idBuildSrc, oEntry.sName, oEntry.sDescription, \
            oEntry.sProduct, oEntry.sBranch, self._getSubList(oEntry.asTypes), \
            self._getSubList(oEntry.asOsArches), oEntry.iFirstRevision, \
            oEntry.iLastRevision, utils.formatIntervalSeconds2(oEntry.cSecMaxAge) \
            if oEntry.cSecMaxAge is not None else None, aoActions,]
