#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: filearchiver.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class FileArchiverBatchJob(object): # pylint: disable=R0903
    ...
    def _processTestSet(self, idTestSet, asFiles, sCurDir):
        ...
        if oTestSet is not None and sBaseFilename != oTestSet.sBaseFilename:
            self.warning('TestSet %d: Deleting because sBaseFilename differs: "%s" (disk) vs "%s" (db)' % (idTestSet, sBaseFilename, oTestSet.sBaseFilename,));
            oTestSet = None;
    ...
    def main():
        ...
        oParser.add_option('-v', '--verbose', dest='fVerbose', action='store_true', default=False, help='Verbose output.');
        oParser.add_option('-q', '--quiet', dest='fVerbose', action='store_false', default=False, help='Quiet operation.');
        oParser.add_option('-d', '--dry-run', dest='fDryRun', action='store_true', default=False, help='Dry run, do not make any changes.');
