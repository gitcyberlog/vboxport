# -*- coding: utf-8 -*-
# $Id: build.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class BuildCategoryData(ModelDataBase):
    ...
    def initFromValues(self, sProduct, sRepository, sBranch, sType, asOsArches, \
        idBuildCategory=None):
        """
    ...
    def _validateAndConvertAttribute(self, sAttr, sParam, oValue, aoNilValues, fAllowNull, oDb):
        ...
        if sAttr == 'sType':
            (oNewValue, sError) = ModelDataBase._validateAndConvertAttribute(self, \
            sAttr, sParam, oValue, aoNilValues, fAllowNull, oDb);
            ...
            return ModelDataBase._validateAndConvertAttribute(self, sAttr, sParam, \
...
class BuildCategoryLogic(ModelLogicBase): # pylint: disable=R0903
    ...
    def fetchForCombo(self):
        ...
        for aoRow in aaoRows:
            ...
            sInfo = '%s / %s / %s / %s' % (oData.sProduct, oData.sBranch, \
                oData.sType, ', '.join(oData.asOsArches));
    ...
    def tryFindByData(self, oData):
        ...
        self._oDb.execute('SELECT   *\n'
                          ...
                          '  AND    sType       = %s\n'
                          '  AND    asOsArches  = %s\n',
                          (oData.sProduct, oData.sRepository, oData.sBranch, \
                          oData.sType, sorted(oData.asOsArches),));
        aaoRows = self._oDb.fetchAll();
        if len(aaoRows) == 0:
            return None;
        if len(aaoRows) > 1:
...
class BuildLogic(ModelLogicBase): # pylint: disable=R0903
    ...
    def addEntry(self, oBuildData, uidAuthor=None, fCommit=False):
        ...
        self._oDb.execute('INSERT INTO Builds (uidAuthor,\n'
                          ...
                          (uidAuthor, oBuildData.idBuildCategory, \
                          oBuildData.iRevision, oBuildData.sVersion, \
                          oBuildData.sLogUrl, oBuildData.sBinaries, \
    ...
    def editEntry(self, oData, uidAuthor=None, fCommit=False):
        ...
        if not oData.isEqualEx(oOldData, ['tsEffective', 'tsExpire', 'uidAuthor']):
            ...
            self._oDb.execute('INSERT INTO Builds (uidAuthor,\n'
                              ...
                              'RETURNING idBuild, tsCreated\n',
                              (uidAuthor, oData.idBuild, oData.tsCreated, \
                              oData.idBuildCategory, oData.iRevision, \
                              oData.sVersion, oData.sLogUrl, oData.sBinaries, \
    ...
    def tryFindSameBuildForOsArch(self, oBuildEx, sOs, sCpuArch):
        ...
        self._oDb.execute('SELECT Builds.*, BuildCategories.*\n'
                          ...
                          'ORDER BY tsCreated DESC\n'
                          'LIMIT 4096\n',        # stay sane.
                          (oBuildEx.oCat.sProduct, oBuildEx.oCat.sBranch, \
                          oBuildEx.oCat.sType, '%s.%s' % (sOs, sCpuArch), \
                          '%s.noarch' % (sOs,), 'os-agnostic.%s' % (sCpuArch,), \
                          'os-agnostic.noarch', oBuildEx.iRevision, \
                          oBuildEx.sRelease,));
        aaoRows = self._oDb.fetchAll();
    ...
    def isBuildBlacklisted(self, oBuildEx):
        ...
        for i in range(len(oBuildEx.oCat.asOsArches)):
            ...
            if len(asParts) != 2 or len(asParts[0]) == 0 or len(asParts[1]) == 0:
                ...
                % (oBuildEx.asOsArches[i], oBuildEx.idBuild, \
        ...
        self._oDb.execute('SELECT COUNT(*)\n'
                          ...
                          (oBuildEx.oCat.sProduct, oBuildEx.oCat.sBranch, \
                          oBuildEx.oCat.sType, oBuildEx.oCat.asOsArches, \
                          asOsAgnosticArch, asOsNoArch, 'os-agnostic.noarch', \
                          oBuildEx.iRevision, oBuildEx.iRevision,));
