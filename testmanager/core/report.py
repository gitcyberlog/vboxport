# -*- coding: utf-8 -*-
# $Id: report.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class ReportGraphModel(ReportModelBase): # pylint: disable=R0903
    ...
    def getExtraWhereExprForTotalPeriodEx(self, sStartField='tsCreated', sEndField='tsDone', fLeadingAnd=True):
        ...
        sRet = '     AND %s >= (%s - interval \'%u hours\')\n' \
               '     AND %s <=  %s\n' % (sStartField, sNow, \
               self.cPeriods * self.cHoursPerPeriod, sEndField, sNow);
    ...
    def _getEligibleTestSetPeriod(self, sPrefix='TestSets.', fLeadingAnd=False):
        ...
        sRet = '     AND %stsCreated <= %s\n' \
             ...
             % (sPrefix, sNow, sPrefix, sNow, self.cPeriods * self.cHoursPerPeriod, \
             7, sPrefix, sNow, self.cPeriods * self.cHoursPerPeriod,);
    ...
    def fetchGraphData(self):
        ...
        for oLookup in self.aoLookups:
            ...
            for aoRow in self._oDb.fetchAll(): # Fetching all here so we can make cache queries below.
                if aoRow[1] != idLastBuildCat or aoRow[2] != idLastTestBox \
                 or aoRow[3] != idLastTestCase or aoRow[4] != iLastUnit:
                    idLastBuildCat = aoRow[1];
                    idLastTestBox = aoRow[2];
                    idLastTestCase = aoRow[3];
                    iLastUnit = aoRow[4];
                    if self.fOnTestCase:
                        oLastSeries = self.DataSeries(self.oCache, idLastBuildCat, \
                            idLastTestBox, idLastTestCase, None, iLastUnit);
                    else:
                        oLastSeries = self.DataSeries(self.oCache, idLastBuildCat, \
                            idLastTestBox, None, idLastTestCase, iLastUnit);
