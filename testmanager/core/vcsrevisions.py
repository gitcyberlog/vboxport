# -*- coding: utf-8 -*-
# $Id: vcsrevisions.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class VcsRevisionLogic(ModelLogicBase): # pylint: disable=R0903
    ...
    def addVcsRevision(self, oData, fCommit=False):
        ...
        if oOldData is None:
            # New row.
            self._oDb.execute('INSERT INTO VcsRevisions (sRepository, iRevision,' \
                ' tsCreated, sAuthor, sMessage)\nVALUES (%s, %s, %s, %s, %s)\n', \
                (oData.sRepository, oData.iRevision, oData.tsCreated, oData.sAuthor, \
                oData.sMessage,));
            ...
            self._oDb.execute('UPDATE VcsRevisions\n'
                              ...
                              'WHERE  sRepository = %s\n'
                              '   AND iRevision   = %s',
                              (oData.tsCreated, oData.sAuthor, oData.sMessage, \
                              oData.sRepository, oData.iRevision,));
