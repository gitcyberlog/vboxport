# -*- coding: utf-8 -*-
# $Id: schedulerbase.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class SchedQueueData(ModelDataBase):
    ...
    kasAllowNullAttributes = ['idItem', 'offQueue', 'aidTestGroupPreReqs', \
        'bmHourlySchedule', 'idTestSetGangLeader', 'tsConfig', 'tsLastScheduled'];
...
class SchedulerBase(object):
        ...
        if len(aoErrors) == 0:
            ...
            for oItem in aoItems:
                self._oDb.execute('INSERT INTO SchedQueues (\n'
                                  ...
                                  (oItem.idSchedGroup, oItem.offQueue, \
                                  oItem.idGenTestCaseArgs, oItem.idTestGroup, \
                                  oItem.aidTestGroupPreReqs \
                                    if len(oItem.aidTestGroupPreReqs) > 0 else None, \
                                  oItem.bmHourlySchedule, oItem.cMissingGangMembers));
    ...
    def _createTestSet(self, oTask, oTestEx, oTestBoxData, oBuild, oValidationKitBuild, tsNow):
        ...
        self._oDb.execute('INSERT INTO  TestSets (\n'
                          ...
                          '             %s,\n'      # iGangMemberNo
                          '             %s)\n'      # idTestSetGangLeader
                          , (idTestSet, oTask.tsConfig, tsNow, oBuild.idBuild, \
                          oBuild.idBuildCategory, oValidationKitBuild.idBuild \
                                if oValidationKitBuild is not None else None, \
                          oTestBoxData.idGenTestBox, oTestBoxData.idTestBox, \
                          oTask.idTestGroup, oTestEx.oTestCase.idGenTestCase, \
                          oTestEx.oTestCase.idTestCase, oTestEx.idGenTestCaseArgs, \
                          oTestEx.idTestCaseArgs, sBaseFilename, iGangMemberNo, \
        ...
        self._oDb.execute('INSERT INTO  TestResults (\n'
                          ...
                          '             0)\n'       # iNestingDepth
                          'RETURNING    idTestResult', (idTestSet, tsNow,));
            ...
            if len(sPreReqSet) > 0:
                ...
                if fDecision is None:
                    ...
                    self._oDb.execute('SELECT   COUNT(*)\n'
                                      ...
                                      (TestSetData.ksTestStatus_Success, \
                                      TestSetData.ksTestStatus_Skipped, \
                                      oEntry.oBuild.idBuild,));
                    cMissingPreReqs = self._oDb.fetchOne()[0];
                    if cMissingPreReqs > 0:
                        self.dprint(\
                            'build %s is missing %u prerequisites (out of %s)' \
    ...
    def _tryAsGangMember(self, oTask, oTestEx, oTestBoxData, tsNow, sBaseUrl):
        ...
        if oLeaderTestSet.idBuildTestSuite is not None:
            oLeaderValidationKitBit = BuildDataEx().initFromDbWithId(self._oDb, \
                oLeaderTestSet.idBuildTestSuite);
            oValidationKitBuild = \
                self._tryFindMatchingBuild(oLeaderValidationKitBit, oTestBoxData, \
                self._oSchedGrpData.idBuildSrcTestSuite);

        #
        # Create a testset and update the state(s).
        #
        idTestSet = self._createTestSet(oTask, oTestEx, oTestBoxData, oBuild, \
            oValidationKitBuild, tsNow);
        ...
        if oTask.cMissingGangMembers < 1:
            ...
            self._moveTaskToEndOfQueue(oTask, oTestEx.cGangMembers, tsNow);
            dResponse = self.composeExecResponseWorker(idTestSet, oTestEx, \
                oTestBoxData, oBuild, oValidationKitBuild, sBaseUrl);
            sTBState = TestBoxStatusData.ksTestBoxState_GangTesting;
            oTBStatusLogic.updateGangStatus(oTask.idTestSetGangLeader, sTBState, \
                fCommit=False);
        else:
    ...
    def scheduleNewTaskWorker(self, oTestBoxData, tsNow, sBaseUrl):
        ...
        for aoRow in aaoRows:
            ...
            if config.g_kfSrvGlueDebugScheduler:
                ...
                % (oTask.idItem, oTask.idGenTestCaseArgs, oTask.idTestGroup, \
                oTask.aidTestGroupPreReqs, oTask.tsLastScheduled, oTask.tsConfig,));
            ...
            oTestEx = TestCaseArgsDataEx().initFromDbWithGenIdEx(self._oDb, \
            oTask.idGenTestCaseArgs, tsConfigEff=oTask.tsConfig, \
                tsRsrcEff=oTask.tsConfig);
            if config.g_kfSrvGlueDebugScheduler:
                self.dprint('TestCase "%s": %s %s' % (oTestEx.oTestCase.sName, \
                    oTestEx.oTestCase.sBaseCmd, oTestEx.sArgs,));
