# -*- coding: utf-8 -*-
# $Id: testcase.py 52776 2014-09-17 14:51:43Z vboxsync $
...
from testmanager.core.base              import ModelDataBase, ModelDataBaseTestCase, \
                                        ModelLogicBase, TMExceptionBase, \
...
class TestCaseGlobalRsrcDepLogic(ModelLogicBase):
    ...
    def getTestCaseDeps(self, idTestCase, tsNow=None):
        ...
        """
        ## @todo This code isn't entirely kosher... Should use a DataEx with a
        ...
        if tsNow is not None:
            self._oDb.execute('SELECT   *\n' \
                              ...
                              '     AND GlobalResources.idGlobalRsrc =' \
                              ' TestCaseGlobalRsrcDeps.idGlobalRsrc\n' \
    ...
    def getDepGlobalResourceData(self, idTestCase, tsNow=None):
        ...
        if tsNow is None:
            self._oDb.execute('SELECT   GlobalResources.*\n' \
                              ...
                              '     AND GlobalResources.idGlobalRsrc =' \
                              ' TestCaseGlobalRsrcDeps.idGlobalRsrc\n' \
                              '     AND TestCaseGlobalRsrcDeps.tsExpire = ' \
                              '\'infinity\'::TIMESTAMP\n' \
                              '     AND GlobalResources.tsExpire = ' \
                              '\'infinity\'::TIMESTAMP\n' \
...
class TestCaseDependencyData(ModelDataBase):
    ...
    def initFromParams(self, oDisp, fStrict=True):
        ...
        self.idTestCase = fn(self.ksParam_idTestCase, None, None \
            if fStrict else self.idTestCase)
        self.idTestCasePreReq = fn(self.ksParam_idTestCasePreReq, None, None \
            if fStrict else self.idTestCasePreReq)
        self.tsEffective = fn(self.ksParam_tsEffective, None, None \
            if fStrict else self.tsEffective)
        self.tsExpire = fn(self.ksParam_tsExpire, None, None \
            if fStrict else self.tsExpire)
        self.uidAuthor = fn(self.ksParam_uidAuthor, None, None \
            if fStrict else self.uidAuthor)
    ...
    def validateAndConvert(self, oDb=None):
        ...
        self.idTestCase = self._validateInt(dErrors, \
            self.ksParam_idTestCase, self.idTestCase)
        self.idTestCasePreReq = self._validateInt(dErrors, \
            self.ksParam_idTestCasePreReq, self.idTestCasePreReq)
        self.tsEffective = self._validateTs(dErrors, \
            self.ksParam_tsEffective, self.tsEffective)
        self.tsExpire = self._validateTs(dErrors, \
            self.ksParam_tsExpire, self.tsExpire)
        self.uidAuthor = self._validateInt(dErrors, \
            self.ksParam_uidAuthor, self.uidAuthor)
    ...
    def convertFromParamNull(self):
        ...
        """
        if self.idTestCase in [-1, '']:
            self.idTestCase = None
        if self.idTestCasePreReq in [-1, '']:
            self.idTestCasePreReq = None
        if self.tsEffective == '':
            self.tsEffective = None
        if self.tsExpire == '':
            self.tsExpire = None
        if self.uidAuthor in [-1, '']:
            self.uidAuthor = None
        return True
    ...
    def convertToParamNull(self):
        ...
        """
        if self.idTestCase is None:
            self.idTestCase = -1
        if self.idTestCasePreReq is None:
            self.idTestCasePreReq = -1
        if self.tsEffective is None:
            self.tsEffective = ''
        if self.tsExpire is None:
            self.tsExpire = ''
        if self.uidAuthor is None:
            self.uidAuthor = -1
        return True
...
class TestCaseData(ModelDataBase):
    ...
    kasAllowNullAttributes = ['idTestCase', 'tsEffective', 'tsExpire', 'uidAuthor', \
        'idGenTestCase', 'sDescription', 'sTestBoxReqExpr', 'sBuildReqExpr', \
        'sValidationKitZips',]
    ...
    def initFromDbWithId(self, oDb, idTestCase, tsNow=None, sPeriodBack=None):
        ...
        """
        oDb.execute(self.formatSimpleNowAndPeriodQuery(oDb, 'SELECT *\n FROM ' \
        'TestCases\n WHERE  idTestCase = %s\n', (idTestCase,), tsNow, sPeriodBack))
        aoRow = oDb.fetchOne()
        if aoRow is None:
            raise TMExceptionBase(\
                'idTestCase=%s not found (tsNow=%s sPeriodBack=%s)' \
                % (idTestCase, tsNow, sPeriodBack,))
    ...
    def _validateAndConvertAttribute(self, sAttr, sParam, oValue, aoNilValues, \
        fAllowNull, oDb):
        if sAttr == 'cSecTimeout' and oValue not in aoNilValues:
        # Allow human readable interval formats.
            return utils.parseIntervalSeconds(oValue)

        (oValue, sError) = ModelDataBase._validateAndConvertAttribute(self, sAttr, \
            sParam, oValue, aoNilValues, fAllowNull, oDb)
        if sError is None:
            if sAttr == 'sTestBoxReqExpr':
                ...
                _, sError = TestCaseData.validateStr(\
                    oValue, fAllowUnicodeSymbols=False)
...
#            return None
    ...
    def matchesTestBoxPropsEx(oTestBoxData, sExpr):
        """
        Worker for TestCaseData.matchesTestBoxProps and
        TestCaseArgsDataEx.matchesTestBoxProps.
        """
        if sExpr is None:
            return True
        dLocals = \
...
class TestCaseDataEx(TestCaseData):
    ...
    def _initExtraMembersFromDb(self, oDb, tsNow=None, sPeriodBack=None):
        ...
        self.aoDepTestCases = TestCaseDependencyLogic(oDb).getDepTestCaseData(\
            self.idTestCase, tsNow)
        self.aoDepGlobalResources = TestCaseGlobalRsrcDepLogic(\
            oDb).getDepGlobalResourceData(self.idTestCase, tsNow)
        self.aoTestCaseArgs = TestCaseArgsLogic(oDb).getTestCaseArgs(\
            self.idTestCase, tsNow)
    ...
    def convertParamToAttribute(self, sAttr, sParam, oValue, oDisp, fStrict):
        ...
        if sAttr == 'aoDepTestCases':
            ...
            for sArgKey in oDisp.getStringParam(\
                TestCaseDataEx.ksParam_aoTestCaseArgs, sDefault='').split(','):
                oDispWrapper = self.DispWrapper(oDisp, '%s[%s][%%s]' % (\
                    TestCaseDataEx.ksParam_aoTestCaseArgs, sArgKey,))
                aoNewValues.append(TestCaseArgsData().initFromParams(\
                    oDispWrapper, fStrict=False))
        return aoNewValues
    ...
    def _validateAndConvertAttribute(self, sAttr, sParam, oValue, aoNilValues, fAllowNull, oDb): # pylint: disable=R0914
        ...
        """
        if sAttr not in ['aoDepTestCases', 'aoDepGlobalResources', 'aoTestCaseArgs']:
            return TestCaseData._validateAndConvertAttribute(self, sAttr, sParam, \
                oValue, aoNilValues, fAllowNull, oDb)
            ...
            for oTestCase in self.aoDepTestCases:
                if utils.isString(oTestCase.idTestCase): # Stored as string convertParamToAttribute.
                    ...
                    try:
                        ...
                        asErrors.append('Test case dependency #%s: %s' \
                            % (oTestCase.idTestCase, oXcpt))
                aoNewValues.append(oTestCase)

        elif sAttr == 'aoDepGlobalResources':
            for oGlobalRsrc in self.aoDepGlobalResources:
                if utils.isString(oGlobalRsrc.idGlobalRsrc):
# Stored as string convertParamToAttribute.
                    ...
                    try:
                        ...
                        asErrors.append('Resource dependency #%s: %s' \
                        % (oGlobalRsrc.idGlobalRsrc, oXcpt))
                aoNewValues.append(oGlobalRsrc)
            ...
            if self.aoTestCaseArgs is None or len(self.aoTestCaseArgs) == 0:
                return (None, \
                'The testcase requires at least one argument variation to be valid.')
            ...
            for iVar in range(len(self.aoTestCaseArgs)):
                ...
                if len(dCurErrors) == 0:
                    ...
                    for sKey in dCurErrors:
                        asErrors.append('%s: %s' \
                        % (sKey[len('TestCaseArgs_'):], dCurErrors[sKey]))
                    dErrors[iVar] = '<br>\n'.join(asErrors)
    ...
    def _validateAndConvertWorker(self, asAllowNullAttributes, oDb):
        dErrors = TestCaseData._validateAndConvertWorker(self, \
            asAllowNullAttributes, oDb)
...
class TestCaseLogic(ModelLogicBase):
    ...
    def fetchForChangeLog(self, idTestCase, iStart, cMaxRows, tsNow):
    # pylint: disable=R0914
        ...
        self._oDb.execute('( SELECT tsEffective, uidAuthor FROM TestCases       ' \
                          'WHERE idTestCase = %s AND tsEffective <= %s )\n' \
                          'UNION\n' \
                          '( SELECT tsEffective, uidAuthor FROM TestCaseArgs    ' \
                          'WHERE idTestCase = %s AND tsEffective <= %s )\n' \
                          'UNION\n' \
                          '( SELECT tsEffective, uidAuthor FROM TestCaseDeps    ' \
                          'WHERE idTestCase = %s AND tsEffective <= %s )\n' \
                          'UNION\n' \
                          '( SELECT tsEffective, uidAuthor FROM' \
                          ' TestCaseGlobalRsrcDeps \n' \
                          '  WHERE idTestCase = %s AND tsEffective <= %s )\n' \
                          'ORDER BY tsEffective DESC\n' \
                          'LIMIT %s OFFSET %s\n',
                          (idTestCase, tsNow, idTestCase, tsNow, idTestCase, tsNow, \
                          idTestCase, tsNow, cMaxRows + 1, iStart,))
        ...
        for i in range(0, len(aoRows) - 1):
            ...
            assert self._oDb.isTsInfinity(tsEffective) != self._oDb.isTsInfinity(\
            tsExpire) or tsEffective < tsExpire, \
                ...
                for sAttr in oNew.getDataAttributes():
                    if sAttr not in ['tsEffective', 'tsExpire', 'uidAuthor', \
                        ...
                        if oOldAttr != oNewAttr:
                            aoChanges.append(AttributeChangeEntry(sAttr, oNewAttr, \
                            oOldAttr, str(oNewAttr), str(oOldAttr)))
            ...
            for oChildNew in oNew.aoTestCaseArgs:
                ...
                while iChildOld < len(oOld.aoTestCaseArgs) \
                    and oOld.aoTestCaseArgs[iChildOld].idTestCaseArgs < \
                    oChildNew.idTestCaseArgs:
                    oChildOld = oOld.aoTestCaseArgs[iChildOld]
                    aoChanges.append(AttributeChangeEntry('Variation #%s' \
                    % (oChildOld.idTestCaseArgs,), None, oChildOld, 'Removed', \
                    str(oChildOld)))
                    iChildOld += 1
                    ...
                    if oChildNew.tsEffective != oChildOld.tsEffective:
                        for sAttr in oChildNew.getDataAttributes():
                            if sAttr not in ['tsEffective', 'tsExpire', \
                                'uidAuthor', 'idGenTestCase',]:
                                oOldAttr = getattr(oChildOld, sAttr)
                                oNewAttr = getattr(oChildNew, sAttr)
                                if oOldAttr != oNewAttr:
                                    aoChanges.append(AttributeChangeEntry(\
                                    'Variation[#%s].%s' % (oChildOld.idTestCaseArgs, \
                                    sAttr,), oNewAttr, oOldAttr, str(oNewAttr), \
                                    str(oOldAttr)))
                    iChildOld += 1
                else:
                    aoChanges.append(AttributeChangeEntry('Variation #%s' \
                    % (oChildNew.idTestCaseArgs,), oChildNew, None, \
            ...
            for oChildNew in oNew.aoDepTestCases:
                ...
                while iChildOld < len(oOld.aoDepTestCases) \
                  and oOld.aoDepTestCases[iChildOld].idTestCase < oChildNew.idTestCase:
                    ...
                    aoChanges.append(AttributeChangeEntry('Dependency #%s' \
                    % (oChildOld.idTestCase,), None, oChildOld, 'Removed', \
                ...
                if    iChildOld < len(oOld.aoDepTestCases) \
                  and oOld.aoDepTestCases[iChildOld].idTestCase == oChildNew.idTestCase:
                    ...
                    aoChanges.append(AttributeChangeEntry('Dependency #%s' \
                    % (oChildNew.idTestCase,), oChildNew, None, '%s (#%u)' \
                    % (oChildNew.sName, oChildNew.idTestCase,), 'Did not exist'))
            ...
            for oChildNew in oNew.aoDepGlobalResources:
                ...
                while iChildOld < len(oOld.aoDepGlobalResources) \
                  and oOld.aoDepGlobalResources[iChildOld].idGlobalRsrc < oChildNew.idGlobalRsrc:
                    ...
                    aoChanges.append(AttributeChangeEntry('Global Resource #%s' \
                    % (oChildOld.idGlobalRsrc,), None, oChildOld, 'Removed', \
                ...
                if    iChildOld < len(oOld.aoDepGlobalResources) \
                  and oOld.aoDepGlobalResources[iChildOld].idGlobalRsrc == oChildNew.idGlobalRsrc:
                    ...
                    aoChanges.append(AttributeChangeEntry('Global Resource #%s' \
                    % (oChildNew.idGlobalRsrc,), oChildNew, None, '%s (#%u)' \
                    % (oChildNew.sName, oChildNew.idGlobalRsrc,), 'Did not exist'))

            # Done.
            aoEntries.append(ChangeLogEntry(uidAuthor, None, tsEffective, tsExpire, \
            oNew, oOld, aoChanges))

        # If we're at the end of the log, add the initial entry.
        if len(aoRows) <= cMaxRows and len(aoRows) > 0:
            oNew = aoRows[-1]
            aoEntries.append(ChangeLogEntry(oNew.uidAuthor, None, aaoChanges[-1][0], \
            aaoChanges[-2][0] if len(aaoChanges) > 1 else oNew.tsExpire, oNew, \
            None, []))
    ...
    def addEntry(self, oData, uidAuthor, fCommit=False):
        ...
        # Add the testcase.
        #
        self._oDb.callProc('TestCaseLogic_addEntry', (uidAuthor, oData.sName, \
            oData.sDescription, oData.fEnabled, oData.cSecTimeout, \
            oData.sTestBoxReqExpr, oData.sBuildReqExpr, oData.sBaseCmd, \
            oData.sValidationKitZips))
        oData.idTestCase = self._oDb.fetchOne()[0]

        # Add testcase dependencies.
        for oDep in oData.aoDepTestCases:
            self._oDb.execute('INSERT INTO TestCaseDeps (idTestCase,' \
                ' idTestCasePreReq, uidAuthor) VALUES (%s, %s, %s)', \
                (oData.idTestCase, oDep.idTestCase, uidAuthor))

        # Add global resource dependencies.
        for oDep in oData.aoDepGlobalResources:
            self._oDb.execute('INSERT INTO TestCaseGlobalRsrcDeps (idTestCase,' \
                ' idGlobalRsrc, uidAuthor) VALUES (%s, %s, %s)', \
        ...
        for oVar in oData.aoTestCaseArgs:
            self._oDb.execute(\
                ...
                'VALUES   (%s, %s, %s, %s, %s, %s, %s)', (oData.idTestCase, \
                uidAuthor, oVar.sArgs, oVar.cSecTimeout, oVar.sTestBoxReqExpr, \
                oVar.sBuildReqExpr, oVar.cGangMembers,))
    ...
    def editEntry(self, oData, uidAuthor, fCommit=False): # pylint: disable=R0914
        ...
        if not TestCaseData().initFromOther(oOldDataEx).isEqual(oData):
            self._oDb.callProc('TestCaseLogic_editEntry', (uidAuthor, \
                oData.idTestCase, oData.sName, oData.sDescription, oData.fEnabled, \
                oData.cSecTimeout, oData.sTestBoxReqExpr, oData.sBuildReqExpr, \
                oData.sBaseCmd, oData.sValidationKitZips))
        ...
        for oNewVar in oData.aoTestCaseArgs:
            ...
            if aoRow is None:
                ...
                self._oDb.execute('INSERT INTO TestCaseArgs (\n' \
                                  ...
                                  (oData.idTestCase, uidAuthor, oNewVar.sArgs, \
                                  oNewVar.cSecTimeout, oNewVar.sTestBoxReqExpr, \
                                  oNewVar.sBuildReqExpr, oNewVar.cGangMembers,))
            else:
                ...
                if self._oDb.isTsInfinity(oCurVar.tsExpire):
                    ...
                    if    oNewVar.cSecTimeout == oCurVar.cSecTimeout \
                      ...
                      and oNewVar.cGangMembers == oCurVar.cGangMembers:
                        ...
                        continue # Unchanged.
                    self._oDb.execute('UPDATE TestCaseArgs SET tsExpire =' \
                        ' CURRENT_TIMESTAMP WHERE idGenTestCaseArgs = %s\n' \
                        , (oCurVar.idGenTestCaseArgs,))
                else:
                    # Existing old entry, re-use the ID.
                    pass
                self._oDb.execute('INSERT INTO TestCaseArgs (\n' \
                                  '         idTestCaseArgs, idTestCase, uidAuthor,' \
                                  ' sArgs, cSecTimeout,\n' \
                                  '         sTestBoxReqExpr, sBuildReqExpr,' \
                                  ' cGangMembers)\n' \
                                  'VALUES   (%s, %s, %s, %s, %s, %s, %s, %s)\n' \
                                  'RETURNING idGenTestCaseArgs\n', \
                                  (oCurVar.idTestCaseArgs, oData.idTestCase, \
                                  uidAuthor, oNewVar.sArgs, oNewVar.cSecTimeout, \
                                  oNewVar.sTestBoxReqExpr, oNewVar.sBuildReqExpr, \
                                  oNewVar.cGangMembers,))
...
class TestCaseDataTestCase(ModelDataBaseTestCase):
    ...
    def testSimpleExpr(self):
        ...
        self.assertEqual(TestCaseData.validateTestBoxReqExpr(\
            'fChipsetIoMmu is True'), None)
        self.assertEqual(TestCaseData.validateTestBoxReqExpr(\
            'fChipsetIoMmu is False'), None)
        self.assertEqual(TestCaseData.validateTestBoxReqExpr(\
            'fChipsetIoMmu is None'), None)
        self.assertEqual(TestCaseData.validateTestBoxReqExpr(\
            'isinstance(fChipsetIoMmu, bool)'), None)
        self.assertEqual(TestCaseData.validateTestBoxReqExpr(\
            'isinstance(iTestBoxScriptRev, int)'), None)
        self.assertEqual(TestCaseData.validateTestBoxReqExpr(\
            'isinstance(cMbScratch, long)'), None)

    def testBadExpr(self):
#        self.assertNotEqual(TestCaseData.validateTestBoxReqExpr(\
#            'this is an bad expression, surely it must be'), None)
#        self.assertNotEqual(TestCaseData.validateTestBoxReqExpr('x = 1 + 1'), None)
#        self.assertNotEqual(TestCaseData.validateTestBoxReqExpr(\
#            '__import__(\'os\').unlink(\'/tmp/no/such/file\')'), None)
#        self.assertNotEqual(TestCaseData.validateTestBoxReqExpr(\
#            'print "foobar"'), None)
        print 'Lincolnx = 1 + 1'
