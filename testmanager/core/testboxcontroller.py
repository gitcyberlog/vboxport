# -*- coding: utf-8 -*-
# $Id: testboxcontroller.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class TestBoxController(object): # pylint: disable=R0903
    ...
    kasAckableCmds = [constants.tbresp.CMD_EXEC, constants.tbresp.CMD_ABORT, \
        constants.tbresp.CMD_REBOOT, constants.tbresp.CMD_UPGRADE, \
        constants.tbresp.CMD_UPGRADE_AND_REBOOT, constants.tbresp.CMD_SPECIAL];
    ...
    def _getLongParam(self, sName, lMin=None, lMax=None, lDefValue=None):
        ...
        try:
            ...
            '%s parameter %s value "%s" cannot be convert to an integer (%s)' \
    ...
    def _cleanupOldTest(self, oDb, oStatusData):
        ...
        if oStatusData.idTestSet is not None:
            SystemLogLogic(oDb).addEntry(SystemLogData.ksEvent_TestSetAbandond, \
                ...
                oStatusData.enmState, self._sAction), fCommit=False);
        ...
        if oStatusData.enmState != TestBoxStatusData.ksTestBoxState_Idle:
            TestBoxStatusLogic(oDb).updateState(self._idTestBox, \
                TestBoxStatusData.ksTestBoxState_Idle, fCommit=False);
    ...
    def _actionSignOn(self):        # pylint: disable=R0914
        ...
        #
        if   self._sTestBoxAddr != oTestBox.ip or sOs != oTestBox.sOs \
          or sOsVersion != oTestBox.sOsVersion or sCpuVendor != oTestBox.sCpuVendor \
          or sCpuArch != oTestBox.sCpuArch or sCpuName != oTestBox.sCpuName \
          or lCpuRevision != oTestBox.lCpuRevision or cCpus != oTestBox.cCpus \
          ...
          or cMbMemory != oTestBox.cMbMemory or cMbScratch != oTestBox.cMbScratch \
          ...
          or iPythonHexVersion != oTestBox.iPythonHexVersion:
            oTestBoxLogic.updateOnSignOn(oTestBox.idTestBox, oTestBox.idGenTestBox, \
            sTestBoxAddr=self._sTestBoxAddr, sOs=sOs, sOsVersion=sOsVersion, \
            sCpuVendor=sCpuVendor, sCpuArch=sCpuArch, sCpuName=sCpuName, \
            lCpuRevision=lCpuRevision, cCpus=cCpus, fCpuHwVirt=fCpuHwVirt, \
            fCpuNestedPaging=fCpuNestedPaging, fCpu64BitGuest=fCpu64BitGuest, \
            fChipsetIoMmu=fChipsetIoMmu, cMbMemory=cMbMemory, cMbScratch=cMbScratch, \
            sReport=sReport, iTestBoxScriptRev=iTestBoxScriptRev, \
    ...
    def _doRequestCommand(self, fIdle):
        ...
        if oTestBoxData.enmPendingCmd != TestBoxData.ksTestBoxCmd_None:
            ...
            if oTestBoxData.enmPendingCmd == TestBoxData.ksTestBoxCmd_Abort and fIdle:
                TestBoxLogic(oDb).setCommand(self._idTestBox, \
        ...
        #
        if fIdle and oTestBoxData.fEnabled \
            ...
            if    self._sAction == constants.tbreq.COMMAND_ACK \
              and TestBoxController.kdCmdToState[sPendingCmd] is not None:
                ...
                oStatusLogic.updateState(self._idTestBox, \
                    TestBoxController.kdCmdToState[sPendingCmd], fCommit=False);
    ...
    def _actionUpload(self):
        ...
        if sMime not in ['text/plain', 'application/octet-stream', 'image/png',]:
         #'text/html', 'text/xml',
                          #'image/gif', 'image/jpeg',
                          #'video/webm', 'video/mpeg', 'video/mpeg4-generic',
            raise TestBoxControllerException('Invalid MIME type "%s"' % (sMime,));

        if sKind not in ['log/release/vm', 'log/debug/vm', 'log/release/svc', \
            'log/debug/svc', 'log/release/client', 'log/debug/client', \
            'log/installer', 'log/uninstaller', 'crash/report/vm', 'crash/dump/vm', \
            'crash/report/svc', 'crash/dump/svc', 'crash/report/client', \
            'crash/dump/client', 'misc/other', 'screenshot/failure', \
            'screenshot/success',]:
        ...
        #
        oTestSet = TestSetData().initFromDbWithId(oDb, oStatusData.idTestSet);
        oDstFile = TestSetLogic(oDb).createFile(oTestSet, sName=sName, sMime=sMime, \
            sKind=sKind, sDesc=sDesc, cbFile=cbFile, fCommit=True);
