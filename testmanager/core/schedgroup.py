# -*- coding: utf-8 -*-
# $Id: schedgroup.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class SchedGroupMemberData(ModelDataBase):
    ...
    kasAllowNullAttributes = ['idSchedGroup', 'idTestGroup', 'tsEffective', \
        'tsExpire', 'uidAuthor', 'bmHourlySchedule', 'idTestGroupPreReq'];
...
class SchedGroupData(ModelDataBase):
    ...
    kasAllowNullAttributes = ['idSchedGroup', 'tsEffective', 'tsExpire', \
    ...
    kasSchedulerDesc = \
    [
        (ksScheduler_BestEffortContinousItegration, \
            'Best-Effort-Continous-Itegration (BECI) scheduler.', ''),
    ]
...
class SchedGroupDataEx(SchedGroupData):
    ...
    def convertParamToAttribute(self, sAttr, sParam, oValue, oDisp, fStrict):
        ...
        aoNewValue = [];
        aidSelected = oDisp.getListOfIntParams(sParam, iMin=1, iMax=0x7ffffffe, \
            aiDefaults=[]);
        ...
        for idTestGroup in sIds.split(','):
            ...
            oDispWrapper = self.DispWrapper(oDisp, '%s[%s][%%s]' \
                % (SchedGroupDataEx.ksParam_aoMembers, idTestGroup,));
            oMember = SchedGroupMemberDataEx().initFromParams(oDispWrapper, \
            fStrict=False);
...
class SchedGroupLogic(ModelLogicBase): # pylint: disable=R0903
    ...
    def addEntry(self, oData, uidAuthor, fCommit=False):
        ...
        self._oDb.execute('INSERT INTO SchedGroups (\n'
                          ...
                          (uidAuthor, oData.sName, oData.sDescription, \
                          oData.fEnabled, oData.enmScheduler, oData.idBuildSrc, \
    ...
    def editEntry(self, oData, uidAuthor, fCommit=False):
        ...
        if not oData.isEqualEx(oOldData, ['tsEffective', 'tsExpire', 'uidAuthor', \
            'aoMembers', 'aoTestBoxes', 'oBuildSrc', 'oBuildSrcValidationKit',]):
    ...
    def getMembers(self, idSchedGroup, tsEffective=None):
        ...
        if tsEffective is None:
            ...
            self._oDb.execute('SELECT   *\n'
                              ...
                              (idSchedGroup, tsEffective, tsEffective, \
                              tsEffective, tsEffective,));
    ...
    def _readdEntry(self, uidAuthor, oData, tsEffective=None):
        ...
        self._oDb.execute('INSERT INTO SchedGroups (\n'
                          ...
                          (uidAuthor, tsEffective, oData.idSchedGroup, \
                          oData.sName, oData.sDescription, oData.fEnabled, \
                          oData.enmScheduler, oData.idBuildSrc, \
                          oData.idBuildSrcTestSuite,));
        return True;
    ...
    def _addSchedGroupMember(self, uidAuthor, oMember, tsEffective=None):
        ...
        self._oDb.execute('INSERT INTO SchedGroupMembers(\n'
                          ...
                          '         idTestGroupPreReq)\n'
                          'VALUES (%s, %s, %s, %s, %s, %s, %s)\n', \
                          (oMember.idSchedGroup, oMember.idTestGroup, tsEffective, \
                          uidAuthor, oMember.iSchedPriority, \
                          oMember.bmHourlySchedule, oMember.idTestGroupPreReq,));
        return True;
