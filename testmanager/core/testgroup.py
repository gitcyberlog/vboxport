# -*- coding: utf-8 -*-
# $Id: testgroup.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class TestGroupMemberData(ModelDataBase):
    ...
    def _validateAndConvertAttribute(self, sAttr, sParam, oValue, aoNilValues, fAllowNull, oDb):
        ...
        (aidVariations, sError) = self.validateListOfInts(oValue, \
            aoNilValues=aoNilValues, fAllowNull=fAllowNull, iMin=-1, iMax=0x7ffffffe);
        if sError is None:
...
class TestGroupLogic(ModelLogicBase):
    ...
    def editEntry(self, oData, uidAuthor, fCommit=False):
        ...
        if not oData.isEqualEx(oOldData, ['aoMembers', 'tsEffective', 'tsExpire', 'uidAuthor',]):
            ...
            self._oDb.execute('INSERT INTO TestGroups\n'
                              ...
                              'VALUES (%s, %s, %s, %s)\n',
                              (uidAuthor, oData.idTestGroup, oData.sName, \
                              oData.sDescription,));
    ...
    def _insertTestGroupMember(self, uidAuthor, oMember):
        ...
        self._oDb.execute(\
            'INSERT INTO TestGroupMembers\n'
            '       (uidAuthor, idTestGroup, idTestCase, '
            ' iSchedPriority, aidTestCaseArgs)\n'
            'VALUES (%s, %s, %s, %s, %s)\n', (uidAuthor, oMember.idTestGroup, \
            oMember.idTestCase, oMember.iSchedPriority, oMember.aidTestCaseArgs,));
        return True;
