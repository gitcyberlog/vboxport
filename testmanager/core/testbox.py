# -*- coding: utf-8 -*-
# $Id: testbox.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class TestBoxData(ModelDataBase):  # pylint: disable=R0902
    ...
    kasTestBoxCmdValues = [ksTestBoxCmd_None, ksTestBoxCmd_Abort, \
    ksTestBoxCmd_Reboot, ksTestBoxCmd_Upgrade, ksTestBoxCmd_UpgradeAndReboot, \
    ksTestBoxCmd_Special];
    kaoTestBoxCmdDescs = \
    [
        (ksTestBoxCmd_None, 'None', ''),
    ...
    kasAllowNullAttributes = ['idTestBox', 'tsEffective', 'tsExpire', 'uidAuthor', \
        'idGenTestBox', 'sDescription', 'ipLom', 'sOs', 'sOsVersion', 'sCpuVendor', \
        'sCpuArch', 'sCpuName', 'lCpuRevision', 'cCpus', 'fCpuHwVirt', \
        'fCpuNestedPaging', 'fCpu64BitGuest', 'fChipsetIoMmu', 'cMbMemory', \
    ...
    def formatPythonVersion(self):
        ...
        if self.iPythonHexVersion is None:
            return 'N/A';
        return 'v%d.%d.%d.%d' % (self.iPythonHexVersion >> 24, \
        (self.iPythonHexVersion >> 16) & 0xff, \
        (self.iPythonHexVersion >>  8) & 0xff, self.iPythonHexVersion & 0xff);
...
class TestBoxLogic(ModelLogicBase):
    ...
    def setCommand(self, idTestBox, sOldCommand, sNewCommand, uidAuthor=None, \
        fCommit=False):
        """
        ...
        try:
            ...
            self._oDb.execute('INSERT INTO TestBoxes (\n'
                              ...
                              '     AND enmPendingCmd = %s\n'
                              'RETURNING idGenTestBox', \
                              (idTestBox, tsEffective, uidAuthor, sNewCommand, \
                              idTestBox, tsEffective, sOldCommand,));
            idGenTestBox = self._oDb.fetchOne()[0];
            if fCommit is True:
                self._oDb.commit();
        except:
