# -*- coding: utf-8 -*-
# $Id: useraccount.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class UserAccountData(ModelDataBase):
    ...
    def initFromDbWithId(self, oDb, uid, tsNow=None, sPeriodBack=None):
        ...
        aoRow = oDb.fetchOne()
        if aoRow is None:
            raise TMExceptionBase('uid=%s not found (tsNow=%s sPeriodBack=%s)' \
                % (uid, tsNow, sPeriodBack,));
    ...
    def _validateAndConvertAttribute(self, sAttr, sParam, oValue, aoNilValues, \
        ...
        if sAttr == 'sEmail':
            return ModelDataBase.validateEmail(oValue, aoNilValues=aoNilValues, \
...
class UserAccountLogic(ModelLogicBase):
    ...
    def addEntry(self, oData, uidAuthor, fCommit=False):
        ...
        """
        self._oDb.callProc('UserAccountLogic_addEntry', (uidAuthor, \
            oData.sUsername, oData.sEmail, oData.sFullName, oData.sLoginName,));
    ...
    def editEntry(self, oData, uidAuthor, fCommit=False):
        ...
        Modify user account.
        """
        self._oDb.callProc('UserAccountLogic_editEntry', (uidAuthor, oData.uid, \
            oData.sUsername, oData.sEmail, oData.sFullName, oData.sLoginName,));
        self._oDb.maybeCommit(fCommit);
        return True;
    ...
    def tryFetchAccountByLoginName(self, sLoginName):
        ...
        if self._oDb.getRowCount() != 1:
            if self._oDb.getRowCount() != 0:
                ...
                '%u rows in Users with sLoginName="%s"' % (self._oDb.getRowCount(), \
                sLoginName));
            return None;
    ...
    def resolveChangeLogAuthors(self, aoEntries):
        ...
        for oEntry in aoEntries:
            ...
            if oEntry.sAuthor is None and oEntry.uidAuthor is not None:
                try:
                    oUser = UserAccountData().initFromDbWithId(self._oDb, \
                        oEntry.uidAuthor, oEntry.tsEffective);
                except:
                    pass;
                else:
