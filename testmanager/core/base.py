# -*- coding: utf-8 -*-
# $Id: base.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class ModelDataBase(ModelBase): # pylint: disable=R0903
    ...
    def _validateAndConvertAttribute(self, sAttr, sParam, oValue, aoNilValues, fAllowNull, oDb):
        ...
        if sPrefix in ['id', 'uid']:
            (oNewValue, sError) = self.validateInt(oValue, aoNilValues=aoNilValues, \
            fAllowNull=fAllowNull);
        elif sPrefix in ['i', 'off', 'pct']:
            (oNewValue, sError) = self.validateInt(oValue, aoNilValues=aoNilValues, \
                fAllowNull=fAllowNull, iMin=getattr(self, 'kiMin_' + sAttr, 0), \
                iMax=getattr(self, 'kiMax_' + sAttr, 0x7ffffffe));
        elif sPrefix in ['l', 'c']:
            (oNewValue, sError) = self.validateLong(oValue, \
                aoNilValues=aoNilValues, fAllowNull=fAllowNull, lMin=getattr(self, \
                'klMin_' + sAttr, 0), lMax=getattr(self, 'klMax_' + sAttr, None));
        elif sPrefix == 'f':
            ...
            (oNewValue, sError) = self.validateStr(oValue, aoNilValues=aoNilValues, \
                fAllowNull=fAllowNull, cchMin=getattr(self, 'kcchMin_' + sAttr, 0), \
                cchMax=getattr(self, 'kcchMax_' + sAttr, 4096));
        ## @todo al.
        elif sPrefix == 'aid':
            (oNewValue, sError) = self.validateListOfInts(oValue, \
                aoNilValues=aoNilValues, fAllowNull=fAllowNull, iMin=1, \
                iMax=0x7ffffffe);
        elif sPrefix == 'as':
            (oNewValue, sError) = self.validateListOfStr(oValue, \
                aoNilValues=aoNilValues, fAllowNull=fAllowNull, \
            ...
            raise TMExceptionBase('Unable to classify "%s" (prefix %s)' \
                % (sAttr, sPrefix));
    ...
    def _validateAndConvertWorker(self, asAllowNullAttributes, oDb):
        ...
        for sAttr in self.getDataAttributes():
            ...
            (oNewValue, sError) = self._validateAndConvertAttribute(sAttr, sParam, \
                oValue, aoNilValues, sAttr in asAllowNullAttributes, oDb);
            if oValue != oNewValue:
                setattr(self, sAttr, oNewValue);
    ...
    def validateTs(sValue, aoNilValues=tuple([None, '']), fAllowNull=True):
        ...
        if len(sValue) == len('2012-10-08 01:54:06.364207+02:00'):
            ...
            if    oRes is not None and (int(oRes.group(6)) > 12 or \
                int(oRes.group(7)) >= 60):
    ...
    def _validateListOfStr(dErrors, sName, asValues, asValidValues=None):
        """ Validates a list of text items."""
        (sValue, sError) = ModelDataBase.validateListOfStr(asValues, \
            asValidValues=asValidValues, fAllowNull=True);
        if sError is not None:
            dErrors[sName] = sError;
        return sValue;
    ...
    def _validateListOfStrNN(dErrors, sName, asValues, asValidValues=None):
        ...
        (sValue, sError) = ModelDataBase.validateListOfStr(asValues, \
            asValidValues=asValidValues, fAllowNull=False);
        if sError is not None:
            dErrors[sName] = sError;
        return sValue;
    ...
    @staticmethod
    def formatSimpleNowAndPeriodQuery(oDb, sQuery, aBindArgs, tsNow=None, \
        sPeriodBack=None, sTablePrefix='', sExpCol='tsExpire', sEffCol='tsEffective'):
        """
...
# --lb--pylint--lb--: --lb--enable--lb--=--lb--E1101,C0111,R0903
