# -*- coding: utf-8 -*-
# $Id: buildsource.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class BuildSourceData(ModelDataBase):
    ...
    kasAllowNullAttributes = ['idBuildSrc', 'tsEffective', 'tsExpire', 'uidAuthor', \
        'sDescription', 'asTypes', 'asOsArches', 'iFirstRevision', 'iLastRevision', \
        'cSecMaxAge'];
...
class BuildSourceLogic(ModelLogicBase): # pylint: disable=R0903
    ...
    def addEntry(self, oData, uidAuthor, fCommit=False):
        ...
        self._oDb.execute('INSERT INTO BuildSources (\n'
                          ...
                          oData.sProduct, oData.sBranch, oData.asTypes, \
                          oData.asOsArches, oData.iFirstRevision, \
                          oData.iLastRevision, oData.cSecMaxAge,));
    ...
    def editEntry(self, oData, uidAuthor, fCommit=False):
        ...
        if not oData.isEqualEx(oOldData, ['tsEffective', 'tsExpire', 'uidAuthor',]):
            ...
            self._oDb.execute('INSERT INTO BuildSources (\n'
                              ...
                              (uidAuthor, oData.idBuildSrc, oData.sName, \
                              oData.sDescription, oData.sProduct, oData.sBranch, \
                              oData.asTypes, oData.asOsArches, oData.iFirstRevision, \
                              oData.iLastRevision, oData.cSecMaxAge,));
        self._oDb.maybeCommit(fCommit);
        return True;
    ...
    def openBuildCursor(self, oBuildSource, sOs, sCpuArch, tsNow):
        ...
        if oBuildSource.asTypes is not None  and  len(oBuildSource.asTypes) > 0:
            if len(oBuildSource.asTypes) == 1:
                sExtraConditions += oCursor.formatBindArgs(\
                '   AND BuildCategories.sType = %s', (oBuildSource.asTypes[0],));
            else:
                sExtraConditions += oCursor.formatBindArgs(\
                '   AND BuildCategories.sType IN (%s', (oBuildSource.asTypes[0],));
                for i in range(1, len(oBuildSource.asTypes) - 1):
                    sExtraConditions += oCursor.formatBindArgs(\
                    ', %s', (oBuildSource.asTypes[i],));
                sExtraConditions += oCursor.formatBindArgs(\
        ...
        oCursor.execute('SELECT Builds.*, BuildCategories.*,\n'
                        ...
                        'LIMIT 256\n'
                        , (oBuildSource.sProduct, oBuildSource.sBranch, tsNow, \
    ...
    def _assertUnique(self, oData, idBuildSrcIgnore):
        ...
        if self._oDb.getRowCount() > 0:
            ...
            'A build source with name "%s" already exist.' % (oData.sName,));
        return True;
