# -*- coding: utf-8 -*-
# $Id: testresults.py 53039 2014-10-13 10:49:38Z vboxsync $
...
class TestResultData(ModelDataBase):
    ...
    kasHarmlessTestStatuses = [ksTestStatus_Skipped, ksTestStatus_BadTestBox, \
        ksTestStatus_Aborted,];
    ## List of bad statuses.
    kasBadTestStatuses = [ksTestStatus_Failure, ksTestStatus_TimedOut, \
    ...
    kasValidValues_enmStatus = [ksTestStatus_Running, ksTestStatus_Success, \
    ksTestStatus_Skipped, ksTestStatus_BadTestBox, ksTestStatus_Aborted, \
    ksTestStatus_Failure, ksTestStatus_TimedOut, ksTestStatus_Rebooted];
...
class TestResultLogic(ModelLogicBase): # pylint: disable=R0903
    ...
    def _getTimePeriodQueryPart(self, tsNow, sInterval):
        ...
        if tsNow is None:
            ...
            sTsNow = '\'%s\'::TIMESTAMP' % (tsNow,);
            # It's actually a string already. duh.
    ...
    '   AND (TestSets.tsDone IS NULL OR TestSets.tsDone >= (%s - \'%s\'::interval))\n' \
     % (sTsNow, sTsNow, sInterval, cMonthsMourningPeriod, sTsNow, sInterval);
        return sRet
        ...
        for iRow in range(1, len(aaoRows)):
            ...
            if oParent.idTestResult != oCur.idTestResultParent:
                ...
                if oParent is None:
                    ...
                    'TestResult #%d is orphaned from its parent #%s.' \
                    % (oCur.idTestResult, oCur.idTestResultParent,));
            if oParent.iNestingDepth + 1 != oCur.iNestingDepth:
                raise self._oDb.integrityException(\
                'TestResult #%d has incorrect nesting depth (%d instead of %d)' \
    ...
    def _handleElement(self, sName, dAttribs, idTestSet, aoStack, aaiHints, dCounts):
        ...
        if sName == 'Test':
            ...
            aoStack.insert(0, self._newTestResult(idTestResultParent=\
                aoStack[0].idTestResult, idTestSet=idTestSet, \
                tsCreated=dAttribs['timestamp'], sName=dAttribs['name'], \
                iNestingDepth=iNestingDepth, dCounts=dCounts, fCommit=True));

        elif sName == 'Value':
            self._newTestValue(idTestResult=aoStack[0].idTestResult, idTestSet=\
                idTestSet, tsCreated=dAttribs['timestamp'], sName=dAttribs['name'], \
                sUnit=dAttribs['unit'], lValue=long(dAttribs['value']), \
                dCounts=dCounts, fCommit=True);

        elif sName == 'FailureDetails':
            self._newFailureDetails(idTestResult=aoStack[0].idTestResult, \
                tsCreated=dAttribs['timestamp'], sText=dAttribs['text'], \
                dCounts=dCounts, fCommit=True);
            ...
            self._completeTestResults(aoStack[0], tsDone=dAttribs['timestamp'], \
                cErrors=int(dAttribs['errors']), enmStatus=\
            TestResultData.ksTestStatus_Failure, fCommit=True);

        elif sName == 'TimedOut':
            self._completeTestResults(aoStack[0], tsDone=dAttribs['timestamp'], \
                cErrors=int(dAttribs['errors']), enmStatus=\
            TestResultData.ksTestStatus_TimedOut, fCommit=True);

        elif sName == 'End':
            self._completeTestResults(aoStack[0], tsDone=dAttribs['timestamp'], \
                cErrors=int(dAttribs.get('errors', '1')), enmStatus=\
                TestResultData.ksTestStatus_Success, fCommit=True);
    ...
    def processXmlStream(self, sXml, idTestSet):
        ...
        #
        if sError is not None:
            SystemLogLogic(self._oDb).addEntry(\
            SystemLogData.ksEvent_XmlResultMalformed, \
            'idTestSet=%s idTestResult=%s XML="%s" %s' \
            % (idTestSet, aoStack[0].idTestResult if len(aoStack) > 0 else -1, \
            sXml[:30 if len(sXml) >= 30 else len(sXml)], sError,), cHoursRepeat=6, \
            fCommit=True);
        return (sError, False);
