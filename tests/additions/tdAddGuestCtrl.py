...
class tdTestDirOpen(tdTestGuestCtrlBase):
    ...
    """
    def __init__(self, sDirectory="", sUser="", sPassword="", sFilter="", aFlags=None):
        tdTestGuestCtrlBase.__init__(self);
        self.oCreds = tdCtxCreds(sUser, sPassword, sDomain="");
        self.sDirectory = sDirectory;
...
class tdTestExec(tdTestGuestCtrlBase):
    ...
    """
    def __init__(self, sCmd="", aArgs=None, aEnv=None, aFlags=None, \
        timeoutMS=5 * 60 * 1000, sUser="", sPassword="", sDomain="", \
            fWaitForExit=True):
        tdTestGuestCtrlBase.__init__(self);
        self.oCreds = tdCtxCreds(sUser, sPassword, sDomain);
        self.sCmd = sCmd;
...
class tdTestFileReadWrite(tdTestGuestCtrlBase):
    ...
    """
    def __init__(self, sFile="", sUser="", sPassword="", sOpenMode="r", \
        sDisposition="", sSharingMode="", lCreationMode=0, cbOffset=0, \
        cbToReadWrite=0, aBuf=None):
        tdTestGuestCtrlBase.__init__(self);
        self.oCreds = tdCtxCreds(sUser, sPassword, sDomain="");
        self.sFile = sFile;
...
class tdTestSession(tdTestGuestCtrlBase):
    ...
    """
    def __init__(self, sUser="", sPassword="", sDomain="", sSessionName=""):
        tdTestGuestCtrlBase.__init__(self);
...
class tdTestUpdateAdditions(tdTestGuestCtrlBase):
    ...
    """
    def __init__(self, sSrc="", aArgs=None, aFlags=None, sUser="", \
        sPassword="", sDomain=""):
        tdTestGuestCtrlBase.__init__(self);
        self.oCreds = tdCtxCreds(sUser, sPassword, sDomain);
        self.sSrc = sSrc;
...
class tdTestResultDirRead(tdTestResult):
    ...
    """
    def __init__(self, fRc=False, numFiles=0, numDirs=0):
...
class tdTestResultExec(tdTestResult):
    ...
    """
    def __init__(self, fRc=False, uExitStatus=500, iExitCode=0, sBuf=None, \
        cbBuf=0, cbStdOut=0, cbStdErr=0):
...
class tdTestResultFileStat(tdTestResult):
    ...
    """
    def __init__(self, fRc=False, cbSize=0, eFileType=0):
        tdTestResult.__init__(self, fRc=fRc);
        self.cbSize = cbSize;
        self.eFileType = eFileType;
...
class tdTestResultFileReadWrite(tdTestResult):
    ...
    """
    def __init__(self, fRc=False, cbProcessed=0, cbOffset=0, aBuf=None):
        tdTestResult.__init__(self, fRc=fRc);
...
class SubTstDrvAddGuestCtrl(base.SubTestDriverBase):
    ...
    def testIt(self, oTestVm, oSession, oTxsSession):
        ...
        if fSkip == False:
            fRc, oTxsSession = self.testGuestCtrlExecTimeout(oSession, \
                oTxsSession, oTestVm);
        reporter.testDone(fSkip);
        ...
        if fSkip == False:
            fRc, oTxsSession = self.testGuestCtrlDirCreate(oSession, \
                oTxsSession, oTestVm);
        reporter.testDone(fSkip);
        ...
        if fSkip == False:
            fRc, oTxsSession = self.testGuestCtrlDirCreateTemp(oSession, \
                oTxsSession, oTestVm);
        reporter.testDone(fSkip);
        ...
        if fSkip == False:
            fRc, oTxsSession = self.testGuestCtrlCopyFrom(oSession, \
                oTxsSession, oTestVm);
        reporter.testDone(fSkip);
        ...
        if fSkip == False:
            fRc, oTxsSession = self.testGuestCtrlFileRemove(oSession, \
                oTxsSession, oTestVm);
        reporter.testDone(fSkip);
    ...
    def gctrlCopyFrom(self, oTest, oRes, oGuestSession, subDir=''): # pylint: disable=R0914
        ...
        reporter.log2(\
            'Copying guest directory "%s" to host "%s"' % (sSrcAbs, sDstAbs));
    ...
    def gctrlExecDoTest(self, i, oTest, oRes, oGuestSession):
        ...
        if fRc is oRes.fRc:
            if fRc is True:
                ...
                if oTest.uExitStatus != oRes.uExitStatus or \
                    oTest.iExitCode != oRes.iExitCode:
                    reporter.error(\
                    'Test #%d failed: Got exit status + code %d,%d, expected %d,%d' % \
                    (i, oTest.uExitStatus, oTest.iExitCode, oRes.uExitStatus, \
                    oRes.iExitCode));
                    return False;
            if fRc is True:
              ...
              'Test #%d failed: Got buffer\n%s (%ld bytes), expected\n%s (%ld bytes)' \
                        ...
                        reporter.log2(\
                            'Test #%d passed: Buffers match (%ld bytes)' \
                            % (i, len(oRes.sBuf)));
                elif oRes.sBuf is not None and len(oRes.sBuf):
                    reporter.error(\
                    'Test #%d failed: Got no buffer data, expected\n%s (%dbytes)' \
                    % (i, map(hex, map(ord, oRes.sBuf)), len(oRes.sBuf)));
                    return False;
                elif oRes.cbStdOut > 0 and oRes.cbStdOut != oTest.cbStdOut:
                    reporter.error(\
                        'Test #%d failed: Got %ld stdout data, expected %ld' \
                        % (i, oTest.cbStdOut, oRes.cbStdOut));
                    return False;
        else:
            reporter.error('Test #%d failed: Got %s, expected %s' \
                % (i, fRc, oRes.fRc));
            return False;
        return True;
    ...
    def testGuestCtrlSessionEnvironment(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        aaTests = [
            ...
            [tdTestSessionEnv(sUser=sUser, sPassword=sPassword, aEnv=['FOO=BAR']), \
                tdTestResultSessionEnv(fRc=True, cNumVars=1)],
            [tdTestSessionEnv(sUser=sUser, sPassword=sPassword, aEnv=['FOO=BAR', \
                'BAR=BAZ']), tdTestResultSessionEnv(fRc=True, cNumVars=2)],
            ...
            # Same stuff.
            [tdTestSessionEnv(sUser=sUser, sPassword=sPassword, aEnv=['FOO=BAR', \
                'FOO=BAR']), tdTestResultSessionEnv(fRc=True, cNumVars=1)]
        ];
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            reporter.log('Testing #%d, user="%s", sPassword="%s", env="%s" (%d)...' \
                % (i, curTest.oCreds.sUser, curTest.oCreds.sPassword, curTest.aEnv, \
                len(curTest.aEnv)));
            ...
            if fRc2 is not True:
                reporter.error(\
                    'Test #%d failed: Session creation failed: Got %s, expected True' \
                    % (i, fRc2));
                fRc = False;
                break;
            # Make sure environment is empty.
            curEnv = self.oTstDrv.oVBoxMgr.getArray(curGuestSession, 'environment');
            reporter.log2('Test #%d: Environment initially has %d elements' \
            % (i, len(curEnv)));
            if len(curEnv) != 0:
                reporter.error(\
          'Test #%d failed: Initial session environment has %d vars, expected 0' \
                % (i, len(curEnv)));
                fRc = False;
                break;
            try:
                ...
                if len(curTest.aEnv):
                    ...
                    try:
                        ...
                        # Setting environment variables might fail (e.g. if empty name specified). Check.
                        curEnv = self.oTstDrv.oVBoxMgr.getArray(curGuestSession, \
                            'environment');
                        if len(curEnv) is not curRes.cNumVars:
                            reporter.error(\
        'Test #%d failed: Session environment has %d vars, expected %d (array)' \
                ...
                ## @todo Get current system environment and add it to
                ## curRes.cNumVars before comparing!
                reporter.log('Test #%d: Environment size' % (i,));
                curEnv = self.oTstDrv.oVBoxMgr.getArray(curGuestSession,\
                'environment');
                reporter.log2(\
                'Test #%d: Environment (%d) -> %s' % (i, len(curEnv), curEnv));
       ...
       % (i, len(curEnv), curEnv, curRes.cNumVars));
                    fRc = False;
                    break;
                curGuestSession.environmentClear();
                # No return (e.g. boolean) value available thru wrapper.
                curEnv = self.oTstDrv.oVBoxMgr.getArray(curGuestSession, \
                'environment');
    ...
    def testGuestCtrlSession(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        aaTests = [
            # Invalid parameters.
            [tdTestSession(), tdTestResultSession(fRc=False)],
            [tdTestSession(sUser=''), tdTestResultSession(fRc=False)],
            [tdTestSession(sPassword='bar'), tdTestResultSession(fRc=False)],
            [tdTestSession(sDomain='boo'), tdTestResultSession(fRc=False)],
            [tdTestSession(sPassword='bar', sDomain='boo'), \
                tdTestResultSession(fRc=False)],
            # User account without a passwort - forbidden.
            [tdTestSession(sUser=sUser), tdTestResultSession(fRc=False)],
            # Wrong credentials.
            ...
            [tdTestSession(sUser='foo', sPassword='bar', sDomain='boo'), \
                tdTestResultSession(fRc=False)],
            # Correct credentials.
            [tdTestSession(sUser=sUser, sPassword=sPassword), tdTestResultSession(\
                fRc=True, cNumSessions=1)]
        ];
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            reporter.log('Testing #%d, user="%s", sPassword="%s", sDomain="%s" ...' \
                % (i, curTest.oCreds.sUser, curTest.oCreds.sPassword, \
                curTest.oCreds.sDomain));
    ...
    def testGuestCtrlExec(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914,R0915
        ...
        aaInvalid = [
            ...
            [tdTestExec(sCmd="non-existent2", sUser=sUser, sPassword=sPassword, \
                fWaitForExit=True), tdTestResultExec(fRc=False)],
        ...
        if oTestVm.isWindows():
            aaExec = [
                # Basic executon.
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'c:\\windows\\system32'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True)],
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'c:\\windows\\system32\\kernel32.dll'], sUser=sUser, \
                    sPassword=sPassword), tdTestResultExec(fRc=True)],
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'c:\\windows\\system32\\nonexist.dll'], sUser=sUser, \
                    sPassword=sPassword), tdTestResultExec(fRc=True, iExitCode=1)],
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', '/wrongparam'], \
                    sUser=sUser, sPassword=sPassword), tdTestResultExec(\
                    fRc=True, iExitCode=1)],
                # Paths with spaces.
                ## @todo Get path of installed Guest Additions. Later.
                [tdTestExec(sCmd=\
            "C:\\Program Files\\Oracle\\VirtualBox Guest Additions\\VBoxControl.exe", \
                    aArgs=['version'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True)],
                # StdOut.
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'c:\\windows\\system32'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True)],
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'stdout-non-existing'], sUser=sUser, sPassword=sPassword), \
                ...
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    ...
                    tdTestResultExec(fRc=True)],
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'stderr-non-existing'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True, iExitCode=1)],
                # StdOut + StdErr.
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'c:\\windows\\system32'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True)],
                [tdTestExec(sCmd=sImageOut, aArgs=['/C', 'dir', '/S', \
                    'stdouterr-non-existing'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True, iExitCode=1)]
                # FIXME: Failing tests.
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [tdTestExec(sCmd=sImage, aArgs=['/C', 'set', 'ERRORLEVEL=0'], \
                    sUser=sUser, sPassword=sPassword), tdTestResultExec(\
                    fRc=True, iExitCode=0)],
                [tdTestExec(sCmd=sImage, aArgs=['/C', 'dir', \
                    'c:\\windows\\system32'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True, iExitCode=0)],
                [tdTestExec(sCmd=sImage, aArgs=['/C', 'dir', \
                    'c:\\windows\\system32\\kernel32.dll'], sUser=sUser, \
                    sPassword=sPassword), tdTestResultExec(fRc=True, iExitCode=0)],
                [tdTestExec(sCmd=sImage, aArgs=['/C', 'dir', \
                    'c:\\nonexisting-file'], sUser=sUser, sPassword=sPassword), \
                    tdTestResultExec(fRc=True, iExitCode=1)],
                [tdTestExec(sCmd=sImage, aArgs=['/C', 'dir', \
    ...
    def testGuestCtrlExecTimeout(self, oSession, oTxsSession, oTestVm):
        ...
        try:
            oGuest = oSession.o.console.guest;
            oGuestSession = oGuest.createSession(sUser, sPassword, \
                sDomain, "testGuestCtrlExecTimeout");
            oGuestSession.waitForArray(\
                [vboxcon.GuestSessionWaitForFlag_Start], 30 * 1000);
            ...
            try:
                ...
                waitRes = curProc.waitForArray(\
                    [vboxcon.ProcessWaitForFlag_Start], 30 * 1000);
                ...
                if fRc:
                    ...
                    waitRes = curProc.waitForArray(\
                        [vboxcon.ProcessWaitForFlag_Terminate], 1);
      ...
      'Waiting for process 2 to get killed because it ran out of execution time ...');
                        waitRes = curProc.waitForArray(\
                            [vboxcon.ProcessWaitForFlag_Terminate], 30 * 1000);
                        if waitRes != vboxcon.ProcessWaitResult_Timeout:
                            reporter.error(\
        'Waiting for process 2 did not time out when it should, got wait result %ld' \
                    ...
                    if fRc:
                        ...
                        if curProc.status != vboxcon.ProcessStatus_TimedOutKilled:
                            ...
                            reporter.log('Status of process 2 correct (%ld)' \
    ...
    def testGuestCtrlDirCreate(self, oSession, oTxsSession, oTestVm):
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory=''), tdTestResult(fRc=False)],
                # More unusual stuff.
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory='..\\..\\'), tdTestResult(fRc=False)],
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory='../../'), tdTestResult(fRc=False)],
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory='z:\\'), tdTestResult(fRc=False)],
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory='\\\\uncrulez\\foo'), tdTestResult(fRc=False)],
                # Creating directories.
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory=sScratch), tdTestResult(fRc=False)],
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory=os.path.join(sScratch, 'foo\\bar\\baz'), \
                ...
                # Long (+ random) stuff.
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory=os.path.join(sScratch, "".join(\
                ...
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory=os.path.join(sScratch, "".join(\
                    random.choice(string.ascii_lowercase) for i in range(255)))), \
                    tdTestResult(fRc=False)],
                [tdTestDirCreate(sUser=sUser, sPassword=sPassword, \
                    sDirectory=os.path.join(sScratch, "".join(\
                    random.choice(string.ascii_lowercase) for i in range(1024)))), \
                    tdTestResult(fRc=False)]
            ]);
        else:
            reporter.log('No OS-specific tests for non-Windows yet!');
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            curTest.setEnvironment(oSession, oTxsSession, oTestVm);
            fRc, curGuestSession = curTest.createSession(\
                'testGuestCtrlDirCreate: Test #%d' % (i,));
            if fRc is False:
                reporter.error('Test #%d failed: Could not create session' % (i,));
                break;
    ...
    def testGuestCtrlDirCreateTemp(self, oSession, oTxsSession, \
        oTestVm): # pylint: disable=R0914
        """
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
                    sDirectory=''), tdTestResult(fRc=False)],
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
                    sDirectory='C:\\Windows', fMode=1234), tdTestResult(fRc=False)],
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
                    sTemplate='', sDirectory='C:\\Windows', fMode=1234), \
                    tdTestResult(fRc=False)],
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
                    sTemplate='xXx', sDirectory='C:\\Windows', fMode=0700), \
                    tdTestResult(fRc=False)],
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
                    sTemplate='xxx', sDirectory='C:\\Windows', fMode=0700), \
                    tdTestResult(fRc=False)],
                # More unusual stuff.
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
                    sTemplate='foo', sDirectory='z:\\'), tdTestResult(fRc=False)],
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
                    sTemplate='foo', sDirectory='\\\\uncrulez\\foo'), \
                    tdTestResult(fRc=False)],
                # Non-existing stuff.
                [tdTestDirCreateTemp(sUser=sUser, sPassword=sPassword, \
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            reporter.log(\
                ...
                % (i, curTest.sTemplate, curTest.fMode, curTest.sDirectory, \
                curTest.fSecure));
            curTest.setEnvironment(oSession, oTxsSession, oTestVm);
            fRc, curGuestSession = curTest.createSession(\
                'testGuestCtrlDirCreateTemp: Test #%d' % (i,));
            if fRc is False:
                reporter.error('Test #%d failed: Could not create session' % (i,));
                break;
            sDirTemp = "";
            try:
                sDirTemp = curGuestSession.directoryCreateTemp(\
                    curTest.sTemplate, curTest.fMode, curTest.sDirectory, \
                    curTest.fSecure);
            except:
                if curRes.fRc is True:
                    reporter.errorXcpt(\
                    'Creating temp directory "%s" failed:' % (curTest.sDirectory,));
                    fRc = False;
                    break;
                else:
                    reporter.logXcpt(\
                    'Creating temp directory "%s" failed expectedly, skipping:' \
                    % (curTest.sDirectory,));
            curTest.closeSession();
            if sDirTemp != "":
                ...
                if fExists is False:
                    reporter.error(\
                        'Test #%d failed: Temporary directory "%s" does not exists' \
                        % (i, sDirTemp));
                    fRc = False;
                    break;
        return (fRc, oTxsSession);
    ...
    def testGuestCtrlDirRead(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                # Invalid stuff.
                [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                    sDirectory=''), tdTestResultDirRead(fRc=False)],
                [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                    sDirectory='C:\\Windows', aFlags=[1234]), \
                    tdTestResultDirRead(fRc=False)],
                [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                    sDirectory='C:\\Windows', sFilter='*.foo'), \
                    tdTestResultDirRead(fRc=False)],
                # More unusual stuff.
                [tdTestDirRead(sUser=sUser, sPassword=sPassword, sDirectory='z:\\'), \
                    tdTestResultDirRead(fRc=False)],
                [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                    sDirectory='\\\\uncrulez\\foo'), tdTestResultDirRead(fRc=False)],
                # Non-existing stuff.
                [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                    sDirectory='c:\\Apps\\nonexisting'), \
                    tdTestResultDirRead(fRc=False)],
                [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                    sDirectory='c:\\Apps\\testDirRead'), \
                    tdTestResultDirRead(fRc=False)]
            ]);
            ...
            if oTestVm.sVmName == 'tst-xppro':
                aaTests.extend([
                    # Reading directories.
                    [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                        sDirectory='../../Windows/Fonts'), \
                        tdTestResultDirRead(fRc=True, numFiles=191)],
                    [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                        sDirectory='c:\\Windows\\Help'), tdTestResultDirRead(\
                        fRc=True, numDirs=13, numFiles=569)],
                    [tdTestDirRead(sUser=sUser, sPassword=sPassword, \
                        sDirectory='c:\\Windows\\Web'), tdTestResultDirRead(\
                        fRc=True, numDirs=3, numFiles=55)]
                ]);
        else:
            reporter.log('No OS-specific tests for non-Windows yet!');
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            curTest.setEnvironment(oSession, oTxsSession, oTestVm);
            fRc, curGuestSession = curTest.createSession(\
                'testGuestCtrlDirRead: Test #%d' % (i,));
            if fRc is False:
                reporter.error('Test #%d failed: Could not create session' % (i,));
                break;
            (fRc2, cDirs, cFiles) = self.gctrlReadDir(curTest, curRes, \
                curGuestSession);
            curTest.closeSession();
            reporter.log2('Test #%d: Returned %ld directories, %ld files total' \
                % (i, cDirs, cFiles));
            if fRc2 is curRes.fRc:
                if fRc2 is True:
                    if curRes.numFiles != cFiles:
                        reporter.error(\
                            'Test #%d failed: Got %ld files, expected %ld' \
                            % (i, cFiles, curRes.numFiles));
                        fRc = False;
                        break;
                    if curRes.numDirs != cDirs:
                        reporter.error(\
                            'Test #%d failed: Got %ld directories, expected %ld' \
                            % (i, cDirs, curRes.numDirs));
                        fRc = False;
                        break;
            else:
                reporter.error(\
                    'Test #%d failed: Got %s, expected %s' % (i, fRc2, curRes.fRc));
                fRc = False;
                break;
    ...
    def testGuestCtrlFileRemove(self, oSession, oTxsSession, oTestVm):
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile=''), tdTestResult(fRc=False)],
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows'), tdTestResult(fRc=False)],
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows'), tdTestResult(fRc=False)],
                # More unusual stuff.
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='z:\\'), tdTestResult(fRc=False)],
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='\\\\uncrulez\\foo'), tdTestResult(fRc=False)],
                # Non-existing stuff.
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='c:\\Apps\\nonexisting'), tdTestResult(fRc=False)],
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='c:\\Apps\\testFileRemove'), tdTestResult(fRc=False)],
                # Try to delete system files.
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='c:\\pagefile.sys'), tdTestResult(fRc=False)],
                [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                    sFile='c:\\Windows\\kernel32.sys'), tdTestResult(fRc=False)]
            ]);
            ...
            if oTestVm.sVmName == 'tst-xppro':
                aaTests.extend([
                    ...
                    [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                        sFile='c:\\Windows\\Media\\chimes.wav'), \
                        tdTestResult(fRc=True)],
                    ...
                    [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                        ...
                        tdTestResult(fRc=False)],
                    [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                        sFile='c:\\Windows\\Media\\chord.wav'), \
                        tdTestResult(fRc=True)],
                    [tdTestFileRemove(sUser=sUser, sPassword=sPassword, \
                        sFile='c:\\Windows\\Media\\chord.wav'), \
                        tdTestResult(fRc=False)]
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            curTest.setEnvironment(oSession, oTxsSession, oTestVm);
            fRc, curGuestSession = curTest.createSession(\
                'testGuestCtrlFileRemove: Test #%d' % (i,));
            if fRc is False:
                reporter.error('Test #%d failed: Could not create session' % (i,));
                break;
            try:
                ...
                if curRes.fRc is True:
                    reporter.errorXcpt('Removing file "%s" failed:' \
                    % (curTest.sFile,));
                    fRc = False;
                    break;
                else:
                    reporter.logXcpt(\
                    'Removing file "%s" failed expectedly, skipping:' \
                    % (curTest.sFile,));
            curTest.closeSession();
        return (fRc, oTxsSession);
    ...
    def testGuestCtrlFileStat(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                # Invalid stuff.
                [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    sFile=''), tdTestResultFileStat(fRc=False)],
                [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows'), tdTestResultFileStat(fRc=False)],
                [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows'), tdTestResultFileStat(fRc=False)],
                # More unusual stuff.
                [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    sFile='z:\\'), tdTestResultFileStat(fRc=False)],
                [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    sFile='\\\\uncrulez\\foo'), tdTestResultFileStat(fRc=False)],
                # Non-existing stuff.
                [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    sFile='c:\\Apps\\nonexisting'), tdTestResultFileStat(fRc=False)],
                [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    sFile='c:\\Apps\\testDirRead'), tdTestResultFileStat(fRc=False)]
            ]);
            ...
            if oTestVm.sVmName == 'tst-xppro':
                aaTests.extend([
                    ...
                    [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                        sFile='../../Windows/Fonts'), tdTestResultFileStat(fRc=False)],
                    [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                        sFile='c:\\Windows\\Help'), tdTestResultFileStat(fRc=False)],
                    [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                    ...
                    [tdTestFileStat(sUser=sUser, sPassword=sPassword, \
                        sFile='c:\\Windows\\system32\\kernel32.dll'), \
                        tdTestResultFileStat(fRc=False, cbSize=926720, \
                        eFileType=vboxcon.FsObjType_File)]
                ]);
        else:
            reporter.log('No OS-specific tests for non-Windows yet!');
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            curTest.setEnvironment(oSession, oTxsSession, oTestVm);
            fRc, curGuestSession = curTest.createSession(\
                'testGuestCtrlFileStat: Test #%d' % (i,));
            if fRc is False:
                reporter.error('Test #%d failed: Could not create session' % (i,));
                break;
            fileObjInfo = None;
    ...
    def testGuestCtrlFileRead(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        aaTests.extend([
            ...
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, sFile=''), \
                tdTestResultFileReadWrite(fRc=False)],
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile='non-existing.file'), tdTestResultFileReadWrite(fRc=False)],
            # Wrong open mode.
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile='non-existing.file', sOpenMode='rt', sDisposition='oe'), \
                tdTestResultFileReadWrite(fRc=False)],
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile='\\\\uncrulez\\non-existing.file', sOpenMode='tr', \
                sDisposition='oe'), tdTestResultFileReadWrite(fRc=False)],
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile='../../non-existing.file', sOpenMode='wr', \
                sDisposition='oe'), tdTestResultFileReadWrite(fRc=False)],
            # Wrong disposition.
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile='non-existing.file', sOpenMode='r', sDisposition='e'), \
                tdTestResultFileReadWrite(fRc=False)],
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile='\\\\uncrulez\\non-existing.file', sOpenMode='r', \
                sDisposition='o'), tdTestResultFileReadWrite(fRc=False)],
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile='../../non-existing.file', sOpenMode='r', \
                sDisposition='c'), tdTestResultFileReadWrite(fRc=False)],
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows\\System32\\calc.exe', sOpenMode='w', \
                    sDisposition='ce'), tdTestResultFileReadWrite(fRc=False)],
                # Open a file which must exist.
                [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows\\System32\\kernel32.dll', sOpenMode='r', \
                    sDisposition='oe'), tdTestResultFileReadWrite(fRc=True)],
                # Try truncating a file which already is opened with a different
                #   sharing mode (and thus should fail).
                [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows\\System32\\kernel32.dll', sOpenMode='w', \
                    sDisposition='ot'), tdTestResultFileReadWrite(fRc=False)]
            ]);
        ...
        if oTestVm.sKind == "WindowsXP":
            aaTests.extend([
                ...
                [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows\\System32\\eula.txt', sOpenMode='r', \
                    sDisposition='oe', cbToReadWrite=33), tdTestResultFileReadWrite(\
                    fRc=True, aBuf='Microsoft Windows XP Professional', \
                    cbProcessed=33, cbOffset=33)],
                # Reading from offset.
                [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                    sFile='C:\\Windows\\System32\\eula.txt', sOpenMode='r', \
                    sDisposition='oe', cbOffset=17782, cbToReadWrite=26), \
                    tdTestResultFileReadWrite(fRc=True, \
                    aBuf='LINKS TO THIRD PARTY SITES', cbProcessed=26, \
                    cbOffset=17782 + 26)]
            ]);
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            % (i, curTest.sFile, curTest.cbToReadWrite, curTest.sOpenMode, \
            curTest.sDisposition, curTest.cbOffset));
            curTest.setEnvironment(oSession, oTxsSession, oTestVm);
            fRc, curGuestSession = curTest.createSession(\
                'testGuestCtrlFileRead: Test #%d' % (i,));
            if fRc is False:
                reporter.error('Test #%d failed: Could not create session' % (i,));
                break;
            try:
    ...
    def testGuestCtrlFileWrite(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        cScratchBuf = 512;
        aScratchBuf = array('b', [random.randint(-128, 127) \
            for i in range(cScratchBuf)]);
        aaTests.extend([
            # Write to a non-existing file.
            [tdTestFileReadWrite(sUser=sUser, sPassword=sPassword, \
                sFile=sScratch + 'testGuestCtrlFileWrite.txt', \
        ...
        aScratchBuf2 = array('b', [random.randint(-128, 127) \
            for i in range(cScratchBuf)]);
        aaTests.extend([
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            try:
                if curTest.cbOffset > 0:
                    curFile = curGuestSession.fileOpenEx(curTest.sFile, \
                    curTest.sOpenMode, curTest.sDisposition, \
                    curTest.sSharingMode, curTest.lCreationMode, \
                    curTest.cbOffset);
                    curOffset = long(curFile.offset);
                    resOffset = long(curTest.cbOffset);
                    if curOffset != resOffset:
...
'Test #%d failed: Initial offset on open does not match: Got %ld, expected %ld' \
                ...
                and curTest.cbToReadWrite > 0:
                    ...
                    if fRc:
                        ...
                        and long(curFile.offset) != curTest.cbOffset:
                            reporter.error('Test #%d failed: Initial write position \
                            does not match current position,  got %ld, expected %ld' \
            ...
            if fRc != curRes.fRc:
                reporter.error(\
                'Test #%d failed: Got %s, expected %s' % (i, fRc, curRes.fRc));
                fRc = False;
                break;
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                    sSrc='\\\\uncrulez\\foo'), tdTestResult(fRc=False)],
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, sSrc='non-exist', \
                    sDst=os.path.join(sScratch, 'non-exist.dll')), \
                    tdTestResult(fRc=False)],
                # Copying single files.
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                    sSrc=sVBoxValidationKitISO, sDst='C:\\non-exist\\'), \
                    tdTestResult(fRc=False)],
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                    sSrc=sVBoxValidationKitISO, sDst='C:\\non\\exist\\'), \
                    tdTestResult(fRc=False)],
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                    sSrc=sVBoxValidationKitISO, \
                    sDst='C:\\non\\exist\\renamedfile.dll'), tdTestResult(fRc=False)],
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                    sSrc=sVBoxValidationKitISO, sDst=os.path.join(sScratch, \
                        'HostGuestAdditions.iso')), tdTestResult(fRc=True)],
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                    sSrc=sVBoxValidationKitISO, sDst=os.path.join(\
                    sScratch, 'HostGuestAdditions.iso')), tdTestResult(fRc=True)],
                # Destination is a directory, should fail.
                [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                    sSrc=sVBoxValidationKitISO, sDst=sScratch), \
                    tdTestResult(fRc=False)]
            ...
            if self.oTstDrv.sHost == 'win':
                ...
                aaTests.extend([
                    # Copying directories with contain files we don't have \
                    #   read access to.
                    [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                        sSrc='C:\\Windows\\security', sDst=sScratch), \
                        tdTestResult(fRc=False)],
                    # Copying directories with regular files.
                    [tdTestCopyTo(sUser=sUser, sPassword=sPassword, \
                        sSrc='C:\\Windows\\Help', sDst=sScratch), \
                        tdTestResult(fRc=True)]
                ]);
        else:
            reporter.log('No OS-specific tests for non-Windows yet!');
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                    sSrc=''), tdTestResult(fRc=False)],
                [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                    sSrc='C:\\Windows', aFlags=[1234]), tdTestResult(fRc=False)],
                # Source missing.
                [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                    sDst=''), tdTestResult(fRc=False)],
                [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                    sDst='C:\\Windows', aFlags=[1234]), tdTestResult(fRc=False)],
                # Nothing to copy (sDst is empty / unreachable).
                [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                    sSrc='z:\\'), tdTestResult(fRc=False)],
                [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                    sSrc='\\\\uncrulez\\foo'), tdTestResult(fRc=False)],
                [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                    sSrc='non-exist', sDst=os.path.join(sScratch, \
                        'non-exist.dll')), tdTestResult(fRc=False)]
            ...
            if self.oTstDrv.sHost == 'win':
                aaTests.extend([
                    ...
                    # Copying directories.
                    [tdTestCopyFrom(sUser=sUser, sPassword=sPassword, \
                        sSrc='C:\\Windows\\Web', sDst=sScratch), \
                        tdTestResult(fRc=True)]
                    ## @todo Add testing the CopyFrom aFlags here!
                ]);
        else:
    ...
    def testGuestCtrlUpdateAdditions(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        # Some stupid trickery to guess the location of the iso.
        sVBoxValidationKitISO = os.path.abspath(os.path.join(\
        os.path.dirname(__file__), '../../VBoxValidationKit.iso'));
        if not os.path.isfile(sVBoxValidationKitISO):
            sVBoxValidationKitISO = os.path.abspath(os.path.join(\
                os.path.dirname(__file__), '../../VBoxTestSuite.iso'));
        if not os.path.isfile(sVBoxValidationKitISO):
            sCur = os.getcwd();
            for i in range(0, 10):
                sVBoxValidationKitISO = os.path.join(\
                    sCur, 'validationkit/VBoxValidationKit.iso');
                if os.path.isfile(sVBoxValidationKitISO):
                    break;
                sVBoxValidationKitISO = os.path.join(sCur, \
                    'testsuite/VBoxTestSuite.iso');
                if os.path.isfile(sVBoxValidationKitISO):
                    break;
                sCur = os.path.abspath(os.path.join(sCur, '..'));
                if i is None: pass; # shut up pychecker/pylint.
        if os.path.isfile(sVBoxValidationKitISO):
            reporter.log('Validation Kit .ISO found at: %s' \
                % (sVBoxValidationKitISO,));
        else:
            reporter.log(\
            'Warning: Validation Kit .ISO not found -- some tests might fail');

        sScratch = os.path.join(self.oTstDrv.sScratchPath, \
            "testGctrlUpdateAdditions");
        try:
            os.makedirs(sScratch);
        except OSError as e:
            if e.errno != errno.EEXIST:
                reporter.error(\
                    'Failed: Unable to create scratch directory \"%s\"' \
                    % (sScratch,));
                return (False, oTxsSession);
        reporter.log('Scratch path is: %s' % (sScratch,));
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                # Wrong aFlags.
                [tdTestUpdateAdditions(sUser=sUser, sPassword=sPassword, \
                    sSrc=self.oTstDrv.getGuestAdditionsIso(), aFlags=[1234]), \
                    tdTestResult(fRc=False)],
                # Non-existing .ISO.
                [tdTestUpdateAdditions(sUser=sUser, sPassword=sPassword, \
                    sSrc="non-existing.iso"), tdTestResult(fRc=False)],
                # Wrong .ISO.
                [tdTestUpdateAdditions(sUser=sUser, sPassword=sPassword, \
                    sSrc=sVBoxValidationKitISO), tdTestResult(fRc=False)],
                # The real thing.
                [tdTestUpdateAdditions(sUser=sUser, sPassword=sPassword, \
                    sSrc=self.oTstDrv.getGuestAdditionsIso()), \
                    tdTestResult(fRc=True)],
