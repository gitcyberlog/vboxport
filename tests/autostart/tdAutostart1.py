...
"""
__version__ = "$Id: tdAutostart1.py 52776 2014-09-17 14:51:43Z vboxsync $"
...
class tdAutostartOs(object):
    ...
    def _findFile(self, sRegExp, sTestBuildDir):
        ...
        for sFile in asFiles:
            if oRegExp.match(os.path.basename(sFile)) and \
                os.path.exists(sTestBuildDir + '/' + sFile):
                return sTestBuildDir + '/' + sFile;

        reporter.error('Failed to find a file matching "%s" in %s.' \
            % (sRegExp, sTestBuildDir));
        return None;

    def _createAutostartCfg(self, sDefaultPolicy='allow', asUserAllow=(), \
        asUserDeny=()):
        """
...
class tdAutostartOsLinux(tdAutostartOs):
    ...
    def installVirtualBox(self, oSession, oTxsSession):
        ...
        if self.sTestBuild is not None:
            fRc = self.oTestDriver.txsUploadFile(oSession, oTxsSession, \
                self.sTestBuild, '/tmp/' + os.path.basename(self.sTestBuild), \
                    cMsTimeout=120 * 1000);
            fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
                'Installing VBox', 10 * 1000, '/bin/chmod', ('chmod', '755', \
                    '/tmp/' + os.path.basename(self.sTestBuild)));
            fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
                'Installing VBox', 240 * 1000, '/tmp/' + \
    ...
    def configureAutostart(self, oSession, oTxsSession, sDefaultPolicy='allow',\
        ...
        fRc = self.oTestDriver.txsRunTest(oTxsSession, \
            'Creating autostart database', 10 * 1000, '/bin/mkdir', ('mkdir', \
                '-m', '1777', '/etc/vbox/autostart.d'));

        # Create /etc/default/virtualbox
        sVBoxCfg = 'VBOXAUTOSTART_CONFIG=/etc/vbox/autostart.cfg\n' \
                   + 'VBOXAUTOSTART_DB=/etc/vbox/autostart.d\n';
        fRc = fRc and self.oTestDriver.txsUploadString(oSession, oTxsSession, \
            sVBoxCfg, '/etc/default/virtualbox');
        fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
            'Setting permissions', 10 * 1000, '/bin/chmod', ('chmod', '644', \
                '/etc/default/virtualbox'));

        sVBoxCfg = self._createAutostartCfg(sDefaultPolicy, asUserAllow, asUserDeny);
        fRc = fRc and self.oTestDriver.txsUploadString(oSession, oTxsSession, \
            sVBoxCfg, '/etc/vbox/autostart.cfg');
        fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
            'Setting permissions', 10 * 1000, '/bin/chmod', ('chmod', '644', \
    ...
    def createUser(self, oTxsSession, sUser):
        ...
        fRc = self.oTestDriver.txsRunTest(oTxsSession, 'Creating new user', \
            10 * 1000, '/usr/sbin/useradd', ('useradd', '-m', '-U', sUser));

        return fRc;

    # pylint: disable=R0913

    def runProgAsUser(self, oTxsSession, sTestName, cMsTimeout, sExecName, sAsUser, \
        asArgs=(), oStdIn='/dev/null', oStdOut='/dev/null', oStdErr='/dev/null', \
            oTestPipe='/dev/null'):
        """
        ...
        fRc = self.oTestDriver.txsRunTestRedirectStd(oTxsSession, sTestName, \
            cMsTimeout, '/usr/bin/sudo', asNewArgs, (), "", oStdIn, oStdOut, \
                oStdErr, oTestPipe);

        return fRc;

# --lb--    # --lb--pylint--lb--: enable--lb--=R0913--lb--
    ...
    def createTestVM(self, oSession, oTxsSession, sUser, sVmName):
        ...
        fRc = self.runProgAsUser(oTxsSession, 'Configuring autostart database', \
            10 * 1000, '/opt/VirtualBox/VBoxManage', sUser, ('setproperty', \
                'autostartdbpath', '/etc/vbox/autostart.d'));
        fRc = fRc and self.runProgAsUser(oTxsSession, 'Create VM ' + sVmName, \
            10 * 1000, '/opt/VirtualBox/VBoxManage', sUser, ('createvm', '--name', \
                sVmName, '--register'));
        fRc = fRc and self.runProgAsUser(oTxsSession, \
            'Enabling autostart for test VM', 10 * 1000, \
                '/opt/VirtualBox/VBoxManage', sUser, ('modifyvm', sVmName, \
...
class tdAutostartOsSolaris(tdAutostartOs):
    ...
    def __init__(self, oTestDriver, sTestBuildDir):
        ...
        self.sTestBuild = self._findFile('^VirtualBox-.*-SunOS-.*\\.tar.gz$', \
            sTestBuildDir);
        self.oTestDriver = oTestDriver;
    ...
    def installVirtualBox(self, oSession, oTxsSession):
        ...
        if self.sTestBuild is not None:
            ...
            if oMatch is not None:
                ...
                fRc = self.oTestDriver.txsUploadFile(oSession, oTxsSession, \
                    self.sTestBuild, '${SCRATCH}/' + sTestBuildFilename, \
                        cMsTimeout=120 * 1000);
                fRc = fRc and self.oTestDriver.txsUnpackFile(oSession, oTxsSession, \
                    '${SCRATCH}/' + sTestBuildFilename, '${SCRATCH}', \
                        cMsTimeout=120 * 1000);
                fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
                    'Installing package', 240 * 1000, '/usr/sbin/pkgadd', \
                        ('pkgadd', '-d', '${SCRATCH}/' + sPkgFilename, '-n', '-a', \
                            '${SCRATCH}/autoresponse', 'SUNWvbox'));
        return fRc;
    ...
    def configureAutostart(self, oSession, oTxsSession, sDefaultPolicy='allow', \
        ...
        fRc = self.oTestDriver.txsRunTest(oTxsSession, \
            'Creating /etc/vbox directory', 10 * 1000, '/usr/bin/mkdir', \
        ...
        fRc = fRc and self.oTestDriver.txsUploadString(oSession, oTxsSession, \
            sVBoxCfg, '/etc/vbox/autostart.cfg');
        fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
            'Setting permissions', 10 * 1000, '/usr/bin/chmod', ('chmod', '644', \
                '/etc/vbox/autostart.cfg'));


        fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
            'Importing the service', 10 * 1000, '/usr/sbin/svccfg', ('svccfg', \
                '-s', 'svc:/application/virtualbox/autostart:default', \
                    'setprop', 'config/config=/etc/vbox/autostart.cfg'));
        fRc = fRc and self.oTestDriver.txsRunTest(oTxsSession, \
            'Enabling the service', 10 * 1000, '/usr/sbin/svcadm', ('svcadm', \
                'enable', 'svc:/application/virtualbox/autostart:default'));
    ...
    def createUser(self, oTxsSession, sUser):
        ...
        fRc = self.oTestDriver.txsRunTest(oTxsSession, 'Creating new user', \
            10 * 1000, '/usr/sbin/useradd', ('useradd', '-m', '-g', 'staff', sUser));
...
#--lb--    # --lb--pylint--lb--: --lb--enable--lb--=--lb--R0913
    ...
    def createTestVM(self, oSession, oTxsSession, sUser, sVmName):
        ...
        fRc = self.runProgAsUser(oTxsSession, 'Create VM ' + sVmName, 10 * 1000, \
            '/opt/VirtualBox/VBoxManage', sUser, ('createvm', '--name', sVmName, \
                '--register'));
        fRc = fRc and self.runProgAsUser(oTxsSession, \
            'Enabling autostart for test VM', 10 * 1000, \
                '/opt/VirtualBox/VBoxManage', sUser, ('modifyvm', sVmName, \
...
class tdAutostart(vbox.TestDriver):      # pylint: disable=R0902
    ...
    def actionConfig(self):
        ...
        # Linux VMs
        if self.ksOsLinux in self.asTestVMs:
            oVM = self.createTestVM(self.ksOsLinux, 1, \
                '4.2/autostart/tst-debian.vdi', sKind='Debian_64', fIoApic=True, \
        ...
        # Solaris VMs
        if self.ksOsSolaris in self.asTestVMs:
            oVM = self.createTestVM(self.ksOsSolaris, 1, \
                '4.2/autostart/tst-solaris.vdi', sKind='Solaris_64', fIoApic=True, \
                    eNic0AttachType=vboxcon.NetworkAttachmentType_NAT, \
    ...
    def testAutostartRunProgs(self, oSession, oTxsSession, sVmName):
        ...
        if oGuestOsHlp is not None:
            ...
            if fRc is True:
                ...
                if fRc is True:
                    fRc = oGuestOsHlp.configureAutostart(oSession, oTxsSession, \
                        'allow', (sTestUserAllow,), (sTestUserDeny,));
                    if fRc is True:
                        # Create a VM with autostart enabled in the guest for both users
                        fRc = oGuestOsHlp.createTestVM(oSession, oTxsSession, \
                            sTestUserAllow, sTestVmName);
                        fRc = fRc and oGuestOsHlp.createTestVM(oSession, \
                            oTxsSession, sTestUserDeny, sTestVmName);
                        if fRc is True:
                            # Reboot the guest
                            (fRc, oTxsSession) = self.txsRebootAndReconnectViaTcp(\
                                oSession, oTxsSession, cMsTimeout=3 * 60000, \
                                    fNatForwardingForTxs=True);
                            if fRc is True:
                                # Fudge factor - Allow the guest to finish starting up.
                                self.sleep(5);
                                fRc = oGuestOsHlp.checkForRunningVM(oSession, \
                                    oTxsSession, sTestUserAllow, sTestVmName);
                                if fRc is False:
                                    reporter.error(\
    ...
    def testAutostartOneCfg(self, sVmName):
        ...
        if fRc is True:
            self.logVmInfo(oVM);
            oSession, oTxsSession = self.startVmAndConnectToTxsViaTcp(sVmName, \
                fCdWait=False, fNatForwardingForTxs=True);
            if oSession is not None:
