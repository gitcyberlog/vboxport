#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: tdNetBenchmark1.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class tdNetBenchmark1(vbox.TestDriver):                                         # pylint: disable=R0902
    ...
    def __init__(self):
        ...
        self.asTestsDef = ['tcp-latency', 'tcp-throughput', 'udp-latency', \
            'udp-throughput', 'tbench'];
    ...
    def parseOption(self, asArgs, iArg):                                        # pylint: disable=R0912,R0915
        if asArgs[iArg] == '--remote-host':
            ...
            for s in asArgs[iArg].split(':'):
                try: c = int(s);
                except: raise base.InvalidOption(\
                'The "--cpu-counts" value "%s" is not an integer' % (s,));
                if c <= 0: raise base.InvalidOption(\
                'The "--cpu-counts" value "%s" is zero or negative' % (s,));
                self.acCpus.append(c);
        elif asArgs[iArg] == '--nic-types':
            iArg += 1;
            if iArg >= len(asArgs): raise base.InvalidOption(\
            ...
            if self.sNicAttachment not in ('bridged', 'nat'):
                ...
                'The "--nic-attachment" value "%s" is not supported. Valid values are: bridged, nat' \
                % (self.sNicAttachment));
            ...
            for s in self.asSetups:
                if s not in self.asSetupsDef:
                    raise base.InvalidOption(\
		'The "--setups" value "%s" is not valid; valid values are: %s' \
    ...
    def actionConfig(self):
        ...
        if self.sGuestName is None and 'g2g' in self.asSetups:
            oVM = self.createTestVM('tst-guest2guest', 0, '3.0/tcp/rhel5.vdi', \
                sKind='RedHat', fIoApic=True, \
        ...
        if 'tst-rhel5' in self.asTestVMs:
            oVM = self.createTestVM('tst-rhel5', 1, '3.0/tcp/rhel5.vdi', \
                sKind='RedHat', fIoApic=True, eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-rhel5-64' in self.asTestVMs:
            oVM = self.createTestVM('tst-rhel5-64', 1, '3.0/tcp/rhel5-64.vdi', \
                sKind='RedHat_64', eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-sles11' in self.asTestVMs:
            oVM = self.createTestVM('tst-sles11', 1, '3.0/tcp/sles11.vdi', \
                sKind='OpenSUSE', fIoApic=True, eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-sles11-64' in self.asTestVMs:
            oVM = self.createTestVM('tst-sles11-64', 1, '3.0/tcp/sles11-64.vdi', \
                sKind='OpenSUSE_64', eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-oel' in self.asTestVMs:
            oVM = self.createTestVM('tst-oel', 1, '3.0/tcp/oel.vdi', \
                sKind='Oracle', fIoApic=True, eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-oel-64' in self.asTestVMs:
            oVM = self.createTestVM('tst-oel-64', 1, '3.0/tcp/oel-64.vdi', \
                sKind='Oracle_64', eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;
        ...
        # Windows VMs
        if 'tst-win2k3ent' in self.asTestVMs:
            oVM = self.createTestVM('tst-win2k3ent', 1, \
                '3.0/tcp/win2k3ent-acpi.vdi', sKind='Windows2003', \
        ...
        if 'tst-win2k3ent-64' in self.asTestVMs:
            oVM = self.createTestVM('tst-win2k3ent-64', 1, \
                '3.0/tcp/win2k3ent-64.vdi', sKind='Windows2003_64', \
                eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;
        ...
        if 'tst-win2k8' in self.asTestVMs:
            oVM = self.createTestVM('tst-win2k8', 1, '3.0/tcp/win2k8.vdi', \
                sKind='Windows2008_64', eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;
        ...
        # Solaris VMs
        if 'tst-sol10' in self.asTestVMs:
            oVM = self.createTestVM('tst-sol10', 1, '3.0/tcp/solaris10.vdi', \
                sKind='Solaris_64', eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-sol11' in self.asTestVMs:
            oVM = self.createTestVM('tst-sol11', 1, '3.0/tcp/os2009-11.vdi', \
                sKind='Solaris_64', eNic0AttachType=eNic0AttachType, \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;
    ...
    def test1RunTestProgs(self, oTxsSession, fRc, sTestName, sAddr):
        ...
        if fRc and 'tcp-latency' in self.asTests and sAddr is not None:
            for cbPkt in self.acbLatencyPkts:
                fRc = self.txsRunTest(oTxsSession, '%u bytes' % (cbPkt), \
                    self.cSecsRun * 1000 * 4, \
                    '${CDROM}/${OS/ARCH}/NetPerf${EXESUFF}', \
                        ('NetPerf', '--client', sAddr, '--interval', \
                        self.cSecsRun, '--len', cbPkt, '--mode', 'latency'));
                if not fRc:
                    break;
            reporter.testDone();
        else:
        ...
        if fRc and 'tcp-throughput' in self.asTests and sAddr is not None:
            for cbPkt in self.acbThroughputPkts:
                fRc = self.txsRunTest(oTxsSession, '%u bytes' % (cbPkt), \
                    ...
                    '${CDROM}/${OS/ARCH}/NetPerf${EXESUFF}', \
                    ('NetPerf', '--client', sAddr, '--interval', \
                    self.cSecsRun, '--len', cbPkt, '--mode', 'throughput'));
                if not fRc:
                    break;
            reporter.testDone();
        else:
        ...
        if fRc is True:
            ...
            if oSession is not None:
                ...
                if 'g2h' in self.asSetups:
                    self.test1RunTestProgs(oTxsSession, fRc, 'guest <-> host', \
                        self.sLocalIP);

                # Benchmark #2 - guest <-> host.
                if 'g2r' in self.asSetups:
                    self.test1RunTestProgs(oTxsSession, fRc, 'guest <-> remote', \
                        self.sRemoteIP);

                # Benchmark #3 - guest <-> guest.
                if 'g2g' in self.asSetups:
                    self.test1RunTestProgs(oTxsSession, fRc, \
                        'guest <-> guest', self.sGuestIP);
    ...
    def test1OneVM(self, sVmName, asSkipNicTypes=(), asSupVirtModes=None, \
        rSupCpus=range(1, 256)):
        """
    ...
    def test1(self):
        ...
        if 'g2g' in self.asSetups  and  self.sGuestName is None:
            self.oGuestToGuestSess, self.oGuestToGuestTxs = \
		self.startVmAndConnectToTxsViaTcp('tst-guest2guest', \
		fCdWait=True);
            ...
            fRc = self.oGuestToGuestTxs.syncExec(\
                '${CDROM}/${OS/ARCH}/NetPerf${EXESUFF}', \
