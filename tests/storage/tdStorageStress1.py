...
"""
__version__ = "$Id: tdStorageStress1.py 54437 2015-02-24 11:08:06Z vboxsync $"
    ...
    def parseOption(self, asArgs, iArg):                                        # pylint: disable=R0912,R0915
        if asArgs[iArg] == '--virt-modes':
            ...
            for s in self.asVirtModes:
             ...
             'The "--virt-modes" value "%s" is not valid; valid values are: %s' \
...
'The "--storage-ctrls" takes a colon separated list of Storage controller types');
    ...
    def actionConfig(self):
        ...
        sVBoxValidationKit_iso = os.path.abspath(os.path.join(\
            os.path.dirname(__file__), '../../VBoxValidationKitStorIo.iso'));
        if not os.path.isfile(sVBoxValidationKit_iso):
            sVBoxValidationKit_iso = os.path.abspath(os.path.join(\
                os.path.dirname(__file__), '../../VBoxValidationKitStorIo.iso'));
        if not os.path.isfile(sVBoxValidationKit_iso):
            sVBoxValidationKit_iso = \
        '/mnt/ramdisk/vbox/svn/trunk/validationkit/VBoxValidationKitStorIo.iso';
        if not os.path.isfile(sVBoxValidationKit_iso):
            sVBoxValidationKit_iso = \
            '/mnt/ramdisk/vbox/svn/trunk/testsuite/VBoxTestSuiteStorIo.iso';
        if not os.path.isfile(sVBoxValidationKit_iso):
            sCur = os.getcwd();
            for i in range(0, 10):
                sVBoxValidationKit_iso = os.path.join(sCur, \
                    'validationkit/VBoxValidationKitStorIo.iso');
        ...
        # Linux VMs
        if 'tst-debian' in self.asTestVMs:
            oVM = self.createTestVM('tst-debian', 1, '4.2/storage/debian.vdi', \
                sKind='Debian_64', fIoApic=True, \
    ...
    def test1RunTestProgs(self, oSession, oTxsSession, fRc, sTestName, sGuestFs):
        ...
        if fRc and 'xfstests' in self.asTests:
            fRc = self.txsRunTest(oTxsSession, 'xfstests', 3600000, \
                '/bin/sh', ('sh', '-c', \
                '(cd /tmp/xfstests && ./check -g auto)'), ('TEST_DIR=/mnt/test', \
                'TEST_DEV=/dev/sdb', 'SCRATCH_MNT=/mnt/scratch', \
                'SCRATCH_DEV=/dev/sdc', 'FSTYP=' + sGuestFs));
            reporter.testDone();
        else:
    ...
    def test1OneCfg(self, sVmName, eStorageController, sDiskFormat, sDiskPath1, sDiskPath2, \
        ...
        if oSession is not None:
            ...
            if sDiskFormat == "iSCSI":
                ...
                if self.fpApiVer >= 4.4:
                    oHd = oSession.oVBox.createMedium(sDiskFormat, sDiskPath1, \
                ...
                if fRc is True:
                    try:
                        if oSession.fpApiVer >= 4.0:
                            oSession.o.machine.attachDevice(\
                        ...
                        reporter.errorXcpt(\
                        'attachDevice("%s",%s,%s,HardDisk,"%s") failed on "%s"' \
                        %(_ControllerTypeToName(eStorageController), 1, 0, \
                            oHd.id, oSession.sName));
                        fRc = False;
                    else:
                        reporter.log('attached "%s" to %s' % (sDiskPath1, oSession.sName));
            else:
                fRc = fRc and oSession.createAndAttachHd(\
                    sDiskPath1, sDiskFormat, _ControllerTypeToName(\
                        eStorageController), cb=10*1024*1024*1024, \
                        iPort=1, fImmutable=False);
                fRc = fRc and oSession.createAndAttachHd(sDiskPath2, \
                    sDiskFormat, _ControllerTypeToName(eStorageController), \
        ...
        if fRc is True:
            ...
            if oSession is not None:
                ...
                fRc = self.test1RunTestProgs(oSession, oTxsSession, fRc, \
                    'stress testing', sGuestFs);
                ...
                if oSession is not None:
                    try:
                        ...
                        reporter.errorXcpt(\
          'failed to detach/delete disks %s and %s from storage controller' % \
