#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: tdStorageBenchmark1.py 54436 2015-02-24 10:58:24Z vboxsync $
...
class tdStorageBenchmark(vbox.TestDriver):   # pylint: disable=R0902
    ...
    def test1RunTestProgs(self, oSession, oTxsSession, fRc, sTestName):
        ...
        fRc = self.txsRunTest(oTxsSession, 'Create FS', 60000, \
            '/sbin/mkfs.ext4', ('mkfs.ext4', '-F', '/dev/vboxtest'));
        reporter.testDone();

        reporter.testStart('mount');
        fRc = self.txsRunTest(oTxsSession, 'Mount FS', 30000, \
            '/bin/mount', ('mount', '/dev/vboxtest', '/mnt'));
        reporter.testDone();
        ...
        if fRc and 'iozone' in self.asTests:
            ...
            fRc = self.txsRunTestRedirectStd(oTxsSession, '2G', 3600000, \
                '/usr/bin/iozone', ('iozone', '-r', '64k', '-s', '2g', '-t', \
                '1', '-T', '-I', '-H', '32', '-F', '/mnt/iozone'), \
        ...
        if fRc and 'fio' in self.asTests:
            ...
            fRc = self.txsUploadString(oSession, oTxsSession, \
                oFioWrapper.getConfig(), '${SCRATCH}/aio-test');
        ...
        if oSession is not None:
            ...
            if sDiskFormat == "iSCSI":
                ...
                if self.fpApiVer >= 4.4:
                    oHd = oSession.oVBox.createMedium(sDiskFormat, sDiskPath, \
                    vboxcon.AccessMode_ReadWrite, vboxcon.DeviceType_HardDisk);
                else:
                ...
                if fRc is True:
                    try:
                        ...
                        reporter.log(\
                        'attached "%s" to %s' %(sDiskPath, oSession.sName));
            else:
                fRc = fRc and oSession.createAndAttachHd(sDiskPath, sDiskFormat, \
                    _ControllerTypeToName(eStorageController), \
            ...
            if oSession is not None:
                ...
                if oSession is not None:
                    try:
                        ...
                        if eStorageController is not \
                            vboxcon.StorageControllerType_PIIX3 and \
                            eStorageController is not \
                ...
                for sDir in asPaths:
                    ...
                    for cCpus in self.acCpus:
                        ...
                        for sVirtMode in self.asVirtModes:
                            ...
                            fRc = self.test1OneCfg(sVmName, eStorageCtrl, \
                                sDiskFormat, sPath, cCpus, fHwVirt, \
                                fNestedPaging) and fRc and True; # pychecker hack.
                            reporter.testDone();
