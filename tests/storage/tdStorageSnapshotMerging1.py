#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: tdStorageSnapshotMerging1.py 52776 2014-09-17 14:51:43Z vboxsync $
...
"""
__version__ = "$Id: tdStorageSnapshotMerging1.py 52776 2014-09-17 14:51:43Z vboxsync $"
...
g_ksValidationKitDir = os.path.dirname(os.path.dirname(\
...
def _ControllerTypeToName(eControllerType):
    ...
    if eControllerType == vboxcon.StorageControllerType_PIIX3 or \
    ...
    elif eControllerType == vboxcon.StorageControllerType_LsiLogic or \
        eControllerType == vboxcon.StorageControllerType_BusLogic:
...
class tdStorageSnapshot(vbox.TestDriver):                                      # pylint: disable=R0902
 ...
 'warning: The "--test-vms" value "%s" does not specify any of our test VMs.' \
    ...
    def actionConfig(self):
        ...
        # Windows VMs
        if 'tst-win7-vhd' in self.asTestVMs:
            oVM = self.createTestVM('tst-win7-vhd', 1, '4.2/storage/win7.vhd', \
                sKind='Windows7', fIoApic=True, \
        ...
        # Linux VMs
        if 'tst-debian-vhd' in self.asTestVMs:
            oVM = self.createTestVM('tst-debian-vhd', 1, \
                '4.2/storage/debian.vhd', sKind='Debian_64', fIoApic=True, \
                eNic0AttachType=vboxcon.NetworkAttachmentType_NAT, \
                eNic0Type=vboxcon.NetworkAdapterType_Am79C973);
            if oVM is None:
                return False;
        ...
        if 'tst-debian-vdi' in self.asTestVMs:
            oVM = self.createTestVM(\
            'tst-debian-vdi', 1, '4.2/storage/debian.vdi', sKind='Debian_64', \
    ...
    def test1UploadFile(self, oSession, oTxsSession):
        ...
        fRc = self.txsUploadString(oSession, oTxsSession, self.sRndData, \
            '${SCRATCH}/' + str(uuid.uuid4()), cMsTimeout=3600 * 1000);
