#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: tdTeleportLocal1.py 52776 2014-09-17 14:51:43Z vboxsync $
...
class tdTeleportLocal1(vbox.TestDriver):
    ...
    def actionConfig(self):
        ...
        # Linux VMs
        if 'tst-rhel5' in self.asTestVMs:
            oVM = self.createTestVMs('tst-rhel5', 1, '3.0/tcp/rhel5.vdi', \
                sKind='RedHat', fIoApic=True, sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-rhel5-64' in self.asTestVMs:
            oVM = self.createTestVMs('tst-rhel5-64', 1, '3.0/tcp/rhel5-64.vdi', \
                sKind='RedHat_64', sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-sles11' in self.asTestVMs:
            oVM = self.createTestVMs('tst-sles11', 1, '3.0/tcp/sles11.vdi', \
                sKind='OpenSUSE', fIoApic=True, sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-sles11-64' in self.asTestVMs:
            oVM = self.createTestVMs('tst-sles11-64', 1, \
                '3.0/tcp/sles11-64.vdi', sKind='OpenSUSE_64', \
                sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-oel' in self.asTestVMs:
            oVM = self.createTestVMs('tst-oel', 1, '3.0/tcp/oel.vdi', \
                sKind='Oracle', fIoApic=True, sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-oel-64' in self.asTestVMs:
            oVM = self.createTestVMs('tst-oel-64', 1, '3.0/tcp/oel-64.vdi', \
                sKind='Oracle_64', sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        # Windows VMs
        if 'tst-win2k3ent' in self.asTestVMs:
            oVM = self.createTestVMs('tst-win2k3ent', 1, \
        ...
        if 'tst-win2k3ent-64' in self.asTestVMs:
            oVM = self.createTestVMs('tst-win2k3ent-64', 1, \
        ...
        if 'tst-win2k8' in self.asTestVMs:
            oVM = self.createTestVMs('tst-win2k8', 1, '3.0/tcp/win2k8.vdi', \
                sKind='Windows2008_64', sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        # Solaris VMs
        if 'tst-sol10' in self.asTestVMs:
            oVM = self.createTestVMs('tst-sol10', 1, '3.0/tcp/solaris10.vdi', \
                sKind='Solaris_64', sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-sol11' in self.asTestVMs:
            oVM = self.createTestVMs('tst-sol11', 1, '3.0/tcp/os2009-11.vdi', \
                sKind='Solaris_64', sDvdImage=sVBoxValidationKit_iso);
            if oVM is None:
                return False;
    ...
    def createTestVMs(self, sName, iGroup, *tArgs, **dKeywordArgs):
        ...
        if oVM is not None:
            sName2 = sName + '-2';
            oVM = self.createTestVM(sName2, iGroup * 2 + 1, *tArgs, \
                **dKeywordArgs);
        return oVM;
    ...
    def test2OneVM(self, sVmBaseName, asSupVirtModes=None, \
    ...
    def test1Sub2(self, oVM):
        ...
        try:
            ...
            if oSession is not None:
                ...
                for uPort in range(0, 1000) + range(16000, 17000) + \
                    range(32000, 33000) + range(65000, 65536):
            ...
            try:
                if oVM.teleporterAddress != sOrgAddress: reporter.testFailure(\
                    'Rollback failed for teleporterAddress');
                if oVM.teleporterPort != uOrgPort: reporter.testFailure(\
                    'Rollback failed for teleporterPort');
                if oVM.teleporterPassword != sOrgPassword: reporter.testFailure(\
                    'Rollback failed for teleporterPassword');
                if oVM.teleporterEnabled != fOrgEnabled: reporter.testFailure(\
    ...
    def test1Sub1(self, oVM):
        ...
        if oSession:
            self.test1Sub1DoTeleport(oSession, 'localhost', 65536, 'password', \
                10000, vbox.ComError.E_INVALIDARG, 'Bad port value 65536');
            self.test1Sub1DoTeleport(oSession, 'localhost', 0, 'password', \
                10000, vbox.ComError.E_INVALIDARG, 'Bad port value 0');
            self.test1Sub1DoTeleport(oSession, 'localhost', 5000, 'password', \
                0, vbox.ComError.E_INVALIDARG, 'Bad max downtime');
            self.test1Sub1DoTeleport(oSession, '', 5000, 'password', 10000, \
                vbox.ComError.E_INVALIDARG, 'No hostname');
            self.test1Sub1DoTeleport(oSession, \
                'no.such.hostname.should.ever.exist.duh', 5000, 'password', 0, \
