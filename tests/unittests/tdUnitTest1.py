#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: tdUnitTest1.py 54450 2015-02-24 14:56:35Z vboxsync $
...
class tdUnitTest1(vbox.TestDriver):
    ...
    kdTestCasesBlackList = {
        ...
        'testcase/tstCryptoPkcs7Sign': '',
        # failed: 29330:error:02001002:lib(2):func(1):reason(2):NA:0:fopen(
        #   'server.pem': '','r')
        'testcase/tstCompressionBenchmark': '',
        # failed: error:        RTZipBlockCompress failed
        # for 'RTZipBlock/LZJB' (#4): VERR_NOT_SUPPORTED
        'testcase/tstLdr-4': '',
        # failed: Failed to get bits for
        #   '/home/vbox/test/tmp/bin/testcase/tstLdrObjR0.r0'/0,
    ...
    def _executeTestCase(self, sName, sFullPath, sTestCaseSubDir, oDevNull): # pylint: disable=R0914
        ...
        #
        reporter.log('*** Executing %s%s...' % (asArgs, ' [hardened]' if \
            fHardened else ''));
        ...
        if not self.fDryRun:
            try:
                if fHardened:
                    oChild = utils.sudoProcessPopen(asArgs, stdin=oDevNull, \
                        stdout=sys.stdout, stderr=sys.stdout);
                else:
                    oChild = subprocess.Popen(asArgs, stdin=oDevNull, \
                        stdout=sys.stdout, stderr=sys.stdout);
            except:
                # 32-bit, may fail to start on 64-bit linux. Just ignore.
                if sName in ['tstAsmStructsRC',]:
                    reporter.logXcpt();
                    fSkipped = True;
                else:
                    reporter.errorXcpt();
